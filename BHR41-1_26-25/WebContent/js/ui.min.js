...
3E3)},function(b){401==b.status?(a.restarting=!0,h.removeAll(),window.location=0==a.resetType?findCurrentPath()+"/redirectSetup.html?v=v1.0.138":findCurrentPath()+"/redirect.html?v=v1.0.138"):window.setTimeout(a.testConnection,3E3)})}}]).controller("rebootCtrl",["$scope","$route","Reboot","SystemSettings","Login","$location","$templateCache",function(a,b,c,d,e,f,g){a.tab=b.current.tab;a.section=b.current.section;a.sysSettings=d.get();a.rebooting=!1;a.restarting=!1;a.reboot=function(){a.rebooting=
!0;c.save(function(){window.setTimeout(a.testConnection,0)})};a.testConnection=function(){d.get(function(){window.setTimeout(a.testConnection,3E3)},function(b){401==b.status?(a.restarting=!0,g.removeAll(),window.location=findCurrentPath()+"/redirect.html?v=v1.0.138"):window.setTimeout(a.testConnection,3E3)})};a.keyPress=function(b){13==b.keyCode&&"/advanced/reboot"==f.path()&&a.reboot()}}]).controller("macCloningCtrl",["$scope","$route","$location","MacCloning",function(a,b,c,d){a.tab=b.current.tab;
...
d+", to="+e+").")}break;case 3:if(10==a.protocols.icmpMessage){var f=a.protocols.icmpType,p=a.protocols.icmpCode;(null==f||""===f||0>f||255<f||isNotaInt(f))&&a.errorMessages.push("ICMP Type:  Please enter a numeric value between 0 and 255.");(null==p||""===p||0>p||255<p||isNotaInt(p))&&a.errorMessages.push("ICMP Code:  Please enter a numeric value between 0 and 255.")}}0==a.errorMessages.length&&(0==a.protocols.incomingPorts&&(a.protocols.incomingExclude=!1),0==a.protocols.outgoingPorts&&(a.protocols.outgoingExclude=
!1),1==a.protocols.protocol||2==a.protocols.protocol)&&(p=f=!1,0==a.protocols.incomingPorts?f=!0:2==a.protocols.incomingPorts&&!1==a.protocols.incomingExclude&&1==b&&65535==c&&(f=!0),0==a.protocols.outgoingPorts?p=!0:2==a.protocols.outgoingPorts&&!1==a.protocols.outgoingExclude&&1==d&&65535==e&&(p=!0),f&&p&&a.errorMessages.push('Port forwarding rules can not be Any-to-Any (or "source: 1-65535" with "destination: 1-65535").'));0==a.errorMessages.length&&(b=getTempObjectContent(a.protocols),a.errorMessages=
...
for(var b=0;b<a.dnsServer.length;b++)a.editing!=b&&a.tempDnsServer.hostname==a.dnsServer[b].hostname&&a.errorMessages.push("Host Name: This name is already used by another DNS object.");0<a.errorMessages.length?(a.showErrorPage=!0,window.scrollTo(0,0)):a.save()};a.save=function(){var b={hostname:a.tempDnsServer.hostname,type:a.tempDnsServer.type};1==b.type&&(b.ipAddress=getIpString(a.tempDnsServer.ipAddress));-1==a.editing?c.save(b,function(){a.dnsServer=c.query()}):c.update({id:a.dnsServer[a.editing].id},
...
return b};a.getDeviceIp=function(b,c){for(var d=0;d<a.devices.length;d++)if(b==a.devices[d].name&&c===a.devices[d].ipAddress)return a.devices[d].ipAddress;return b};a.netObjsIPArr=[];a.showNetObj=[];a.netObjsArr=[];a.refresh=function(){a.active=[];a.dateTimeSettings=s.get(function(){a.localTimeMoment=findTimeNow(a.dateTimeSettings);a.devices=h.query(function(){a.devices=filterOutGuest(a.devices);a.accessControl=c.query(function(){a.netObjs=d.query(function(){a.netObjsIPArr=[];a.showNetObj=[];a.netObjsArr=
...
a.pf.protocol.protocol=parseInt(a.pf.protocol.protocol);var b=[],d=[],e=[],g=0,f=0,h=65535,q=0,y=0,w=65535;if(-2==a.appToForward)a.errorMessages.push("Error: Please select a pre-configured application or choose the option Custom ports to create a new appliction for port forwarding");else if(-1==a.appToForward)if(!a.showAdvanced&&0==a.errorMessages.length)for(var r=(""+a.pf.protocol.outgoingPortStart).split(","),u=0;u<r.length;u++){var n=r[u].split("-");if(2<n.length)a.errorMessages.push("Error: The input of port has illegal characters. Source port should be 111 or 111-222; Destination port should be 111, or 111-222, or 111,222-333 and so on.");
else for(var l=0;l<n.length;l++)""===n[l]||isNaN(n[l])||checkForWholeNumbers(n[l])||!checksForWholeNumber(n[l])?a.errorMessages.push("Error: The input of port has illegal characters. Source port should be 111 or 111-222; Destination port should be 111, or 111-222, or 111,222-333 and so on."):(1>n[l]||65535<n[l])&&a.errorMessages.push("Error: Port range should be between [1-65535]");1==n.length?(q=1,y=w=parseInt(n[0])):2==n.length&&(q=2,y=parseInt(n[0]),w=parseInt(n[1]));b.push(q);d.push(y);e.push(w)}else if(0==
...
(1>n[l]||65535<n[l])&&a.errorMessages.push("Error: Port range should be between [1-65535]");1==n.length?(g=1,f=h=parseInt(n[0])):2==n.length&&(g=2,f=parseInt(n[0]),h=parseInt(n[1]))}if(1==a.destPortType&&0==a.errorMessages.length)for(r=(""+a.pf.outgoingPortStart).split(","),u=0;u<r.length;u++){n=r[u].split("-");if(2<n.length)a.errorMessages.push("Error: The input of port has illegal characters. Destination port should be 111 or 111-222; Destination port should be 111, or 111-222, or 111,222-333 and so on.");
else for(l=0;l<n.length;l++)""===n[l]||isNaN(n[l])||checkForWholeNumbers(n[l])||!checksForWholeNumber(n[l])?a.errorMessages.push("Error: The input of port has illegal characters. Destination port should be 111 or 111-222; Destination port should be 111, or 111-222, or 111,222-333 and so on."):(1>n[l]||65535<n[l])&&a.errorMessages.push("Error: Port range should be between [1-65535]");1==n.length?(q=1,y=w=parseInt(n[0])):2==n.length&&(q=2,y=parseInt(n[0]),w=parseInt(n[1]));b.push(q);d.push(y);e.push(w)}}a.showAdvanced&&
1==a.pf.servicePortType&&0==a.errorMessages.length&&(""===a.pf.servicePort||isNaN(a.pf.servicePort)||0>a.pf.servicePort||65535<a.pf.servicePort||!checksForWholeNumber(a.pf.servicePort))&&a.errorMessages.push("Forward to Port: Please enter a numeric value between 0 and 65535.");0==b.length&&(b.push(q),d.push(y),e.push(w));0==a.errorMessages.length&&-1==a.appToForward&&(q=r=!1,!0==a.showAdvanced?0==a.sourcePortType?r=!0:1==a.sourcePortType&&(l=(""+a.pf.incomingPortStart).split("-"),2==l.length&&1==
l[0]&&65535==l[1]&&(r=!0)):r=!0,!0==a.showAdvanced?0==a.destPortType?q=!0:1==a.destPortType&&(l=(""+a.pf.outgoingPortStart).split("-"),2==l.length&&1==l[0]&&65535==l[1]&&(q=!0)):(l=(""+a.pf.protocol.outgoingPortStart).split("-"),2==l.length&&1==l[0]&&65535==l[1]&&(q=!0)),r&&q&&a.errorMessages.push('Port forwarding rules can not be Any-to-Any (or "source: 1-65535" with "destination: 1-65535").'));if(0==a.errorMessages.length&&0<=a.appToForward){u=null;for(l=0;l<a.pfRules.length;l++)if(a.pfRules[l].id==
a.appToForward){u=a.pfRules[l];break}if(null==u)return;for(l=0;l<u.protocols.length;l++)if(!u.protocols[l].protocolExclude&&!u.protocols[l].outgoingExclude){var q=u.protocols[l].outgoingPortStart,w=u.protocols[l].outgoingPortEnd,y=u.name,x=q,r=u.protocols[l].protocol,B=!1;void 0!==w&&q!=w&&(x+="-"+w);void 0===w&&(w=q);for(x=0;x<a.portForward.length;x++)for(n=0;n<a.portForward[x].protocols.length;n++){var t=a.portForward[x].protocols[n];if(7!=r||!(1>t.protocol||2<t.protocol))if(7==r||r==t.protocol){void 0!=
q||r!=t.protocol||0!=r&&3!=r&&4!=r&&5!=r&&6!=r||(B=!0);var A="";if(0==t.outgoingPorts)A="Any";else if(1==t.outgoingPorts&&q<=t.outgoingPortStart&&w>=t.outgoingPortStart)A=t.outgoingPortStart;else if(2==t.outgoingPorts&&q<=t.outgoingPortStart&&w>=t.outgoingPortStart||q<=t.outgoingPortEnd&&w>=t.outgoingPortEnd||q>=t.outgoingPortStart&&w<=t.outgoingPortEnd)A=t.outgoingPortStart+" -> "+t.outgoingPortEnd;if(""!=A||B){t="Conflicting Port Forwarding Rules: The "+y+" service conflict with an existing port forwarding rule offering "+
y+" service.";A=!0;if(0<a.errorMessages.length)for(l=0;l<a.errorMessages.length;l++)if(a.errorMessages[l]==t){A=!1;break}A&&a.errorMessages.push(t)}}}}}if(0==a.errorMessages.length&&-1==a.appToForward)for(u=0;u<b.length;u++)if(q=b[u],y=d[u],w=e[u],r=parseInt(a.pf.protocol.protocol),1!=r&&7!=r||4567!=y||4567!=w||a.errorMessages.push("Invalid rule: Port 4567 cannot be blocked"),0==q)for(l=0;l<a.portForward.length;l++)for(n=0;n<a.portForward[l].protocols.length;n++){if(t=a.portForward[l].protocols[n],
7!=r||!(1>t.protocol||2<t.protocol))if(7==r||r==t.protocol)A="",A=0==t.outgoingPorts?"Any":1==t.outgoingPorts?t.outgoingPortStart:t.outgoingPortStart+" -> "+t.outgoingPortEnd,a.errorMessages.push("Conflicting Port Forwarding Rules: The Destination Ports Any service conflict with an existing port forwarding rule offering Destination Ports "+A+" service.")}else for(x=q=y,r=a.pf.protocol.protocol,q!=w&&(x+="-"+w),l=0;l<a.portForward.length;l++)for(n=0;n<a.portForward[l].protocols.length;n++)if(t=a.portForward[l].protocols[n],
y=a.portForward[l].name,7!=r||!(1>t.protocol||2<t.protocol))if(7==r||r==t.protocol){A="";if(0==t.outgoingPorts)A="Any";else if(1==t.outgoingPorts&&q<=t.outgoingPortStart&&w>=t.outgoingPortStart)A=t.outgoingPortStart;else if(2==t.outgoingPorts&&q<=t.outgoingPortStart&&w>=t.outgoingPortStart||q<=t.outgoingPortEnd&&w>=t.outgoingPortEnd||q>=t.outgoingPortStart&&w<=t.outgoingPortEnd)A=t.outgoingPortStart+" -> "+t.outgoingPortEnd;""!=A&&(0<=y.indexOf("_")?a.errorMessages.push("Conflicting Port Forwarding Rules: The Destination Ports "+
x+" service conflict with an existing port forwarding rule offering  Destination Ports "+A+" service."):a.errorMessages.push("Conflicting Port Forwarding Rules: The Destination Ports "+x+" service conflict with an existing port forwarding rule offering "+y+" service."))}if(0<a.errorMessages.length)a.showErrorPage=!0,window.scrollTo(0,0);else{y=w=u=q=!1;l=parseInt(a.pf.protocol.protocol);r={enabled:!0};-1==a.ipSelect?r.deviceIp=getIpString(a.pf.specifiedIp):0<=a.ipSelect&&(r.deviceIp=a.devices[a.ipSelect].ipAddress);
if(-1==a.appToForward)if(r.name="PortForward_"+(new Date).getTime(),r.protocols=[],7==l)for(l=0;l<b.length;l++)y=w=u=q=!1,n={protocol:1},n.incomingPorts=g,n.incomingPortStart=f,!1==w&&(n.incomingPortEnd=h),n.incomingExclude=q,n.outgoingPorts=b[l],n.outgoingPortStart=d[l],!1==y&&(n.outgoingPortEnd=e[l]),n.outgoingExclude=u,r.protocols.push(n),n={protocol:2},n.incomingPorts=g,n.incomingPortStart=f,!1==w&&(n.incomingPortEnd=h),n.incomingExclude=q,n.outgoingPorts=b[l],n.outgoingPortStart=d[l],!1==y&&
(n.outgoingPortEnd=e[l]),n.outgoingExclude=u,r.protocols.push(n);else for(l=0;l<b.length;l++)y=w=u=q=!1,n={},n.protocol=a.pf.protocol.protocol,n.incomingPorts=g,n.incomingPortStart=f,!1==w&&(n.incomingPortEnd=h),n.incomingExclude=q,n.outgoingPorts=b[l],n.outgoingPortStart=d[l],!1==y&&(n.outgoingPortEnd=e[l]),n.outgoingExclude=u,r.protocols.push(n);else if(0<=a.appToForward){b=null;for(l=0;l<a.pfRules.length;l++)if(a.pfRules[l].id==a.appToForward){b=a.pfRules[l];break}null!=b&&(r.protocols=b.protocols,
r.name=escape(b.name).replace(/\+/g,"%2B"))}a.showAdvanced?(r.schedule=a.pf.schedule,r.schedule=escape(r.schedule).replace(/\+/g,"%2B"),r.servicePort=0==a.pf.servicePortType?0:parseInt(a.pf.servicePort)):(r.servicePort=0,r.schedule="Always");c.save(r,function(){a.refresh()});a.reset()}};a.remove=function(){for(var b=[],c=0;c<a.portForward.length;c++)a.removeBox[c]&&b.push(a.portForward[c].id);if(0<b.length){for(c=0;c<b.length;c++)b[c]-=c;a.removeListItems(b)}a.removeBox=[];for(c=0;c<a.portForward.length;c++)a.removeBox.push(!1)};
...
(m=k=!1,0==a.protocols.incomingPorts?k=!0:2==a.protocols.incomingPorts&&!1==a.protocols.incomingExclude&&1==b&&65535==c&&(k=!0),0==a.protocols.outgoingPorts?m=!0:2==a.protocols.outgoingPorts&&!1==a.protocols.outgoingExclude&&1==d&&65535==h&&(m=!0),k&&m&&a.errorMessages.push('Port forwarding rules can not be Any-to-Any (or "source: 1-65535" with "destination: 1-65535").'));var s={outgoing:[],incoming:[]};copyTrigger(s,a.tempTrigger);var p=m=k="",z=!1,b=!1,v,q,c=!1;q=v="";var y=!1,w=!1,r,u,n=!1;if(1==
a.protocols.protocol||2==a.protocols.protocol){1==a.protocols.protocol?k="TCP":2==a.protocols.protocol&&(k="UDP");for(var c=a.protocols.incomingPortEnd,b=a.protocols.incomingPortStart,h=a.protocols.outgoingPortEnd,d=a.protocols.outgoingPortStart,y=2==a.protocols.incomingPorts?!0:!1,z=2==a.protocols.outgoingPorts?!0:!1,s=!1==a.incoming?s.outgoing:s.incoming,l=0;l<s.length;l++)if(a.protocols.protocol==s[l].protocol&&a.tempIndex!=l&&(r=s[l].incomingPortEnd,u=s[l].incomingPortStart,w=null==r||""===r?
!1:!0,n=!1,y?w?Number(b)>=Number(u)&&Number(b)<=Number(r)?(v=u+"-"+r,q=b+"-"+c,n=!0):Number(c)>=Number(u)&&Number(c)<=Number(r)?(v=u+"-"+r,q=b+"-"+c,n=!0):Number(u)>=Number(b)&&Number(u)<=Number(c)?(v=u+"-"+r,q=b+"-"+c,n=!0):Number(r)>=Number(b)&&Number(r)<=Number(c)&&(v=u+"-"+r,q=b+"-"+c,n=!0):Number(u)>=Number(b)&&Number(u)<=Number(c)&&(v=u,q=b+"-"+c,n=!0):w?Number(b)>=Number(u)&&Number(b)<=Number(r)&&(v=u+"-"+r,q=d,n=!0):Number(b)==Number(u)&&(v=u,q=b,n=!0),n)){a.errorMessages.push("Overlapping Port Ranges: The "+
k+" source ports ("+q+") is overlapping with already defined "+k+" source ports ("+v+").");break}for(l=0;l<s.length;l++)if(a.protocols.protocol==s[l].protocol&&a.tempIndex!=l&&(v=s[l].outgoingPortEnd,q=s[l].outgoingPortStart,b=null==v||""===v?!1:!0,c=!1,z?b?Number(d)>=Number(q)&&Number(d)<=Number(v)?(m=q+"-"+v,p=d+"-"+h,c=!0):Number(h)>=Number(q)&&Number(h)<=Number(v)?(m=q+"-"+v,p=d+"-"+h,c=!0):Number(q)>=Number(d)&&Number(q)<=Number(h)?(m=q+"-"+v,p=d+"-"+h,c=!0):Number(v)>=Number(d)&&Number(v)<=
Number(h)&&(m=q+"-"+v,p=d+"-"+h,c=!0):Number(q)>=Number(d)&&Number(q)<=Number(h)&&(m=q,p=d+"-"+h,c=!0):b?Number(d)>=Number(q)&&Number(d)<=Number(v)&&(m=q+"-"+v,p=d,c=!0):Number(d)==Number(q)&&(m=q,p=d,c=!0),c)){a.errorMessages.push("Overlapping Port Ranges: The "+k+" destination ports ("+p+") is overlapping with already defined "+k+" destination ports ("+m+").");break}}0<a.errorMessages.length?(a.showErrorPage=!0,window.scrollTo(0,0)):(a.protocols.protocolNumber=parseInt(a.protocols.protocolNumber),
...
case "3":d="5";break;case "4":d="0";break;default:return!1}if(0<a.length){for(var e=0;e<a.length;e++){if(String(a[e].connectionType)==String(d)&&!0==a[e].status){c=!0;break}if("6"==String(a[e].connectionType)&&a[e].approved&&"5"==String(d)&&!0==a[e].status){c=!0;break}if("7"==String(a[e].connectionType)&&"4"==String(d)&&!0==a[e].status){c=!0;break}}return c}return!1};var m,s=h.query(function(){m=d.query(function(){a.names=[];for(var b=[],c=0;c<m.length;c++)if(!m[c].done&&(m[c].done=!0,m[c].isChild=
!1,m[c].styleName=a.getStyleName(m[c].connectionId),m[c].hasConnectedDeviceNow=a.checkForDevicesNow(s,m[c].connectionId),b.push(m[c]),a.names.push(unescape(m[c].name)),m[c].availableBridges))for(var d=0;d<m[c].availableBridges.length;d++){var e=m[c].availableBridges[d];1!=e&&m[c].bridged[d]&&(m[e].isChild=!0,m[e].done||(m[e].styleName="BridgedItem "+a.getStyleName(m[e].connectionId),b.push(m[e]),a.names.push(unescape(m[e].name)),m[e].done=!0,m[e].hasConnectedDeviceNow=a.checkForDevicesNow(s,m[e].connectionId)))}a.connections=
...
"4"==String(d)&&!0==a[e].status){c=!0;break}}return c}return!1};var s=k.query(function(){a.connection=f.get({id:c.id},function(){a.connection.styleName=a.getStyleName(c.id);if(a.connection.dhcpExpires){var b=a.connection.dhcpExpires;a.connection.dhcpExpireTime=0!=b?Math.ceil(b/60)+" minutes":"Expired"}a.name=unescape(a.connection.name);a.connection.name=unescape(a.connection.name);a.connection.hasConnectedDeviceNow=a.checkForDevicesNow(s,a.connection.connectionId);a.oldsubnet=a.connection.subnetMask;
...
n=!1,m=!0);!b&&!1==l&&2==a.connection.protocolUsed&&validateIp(a.ipAddress)&&"0.0.0.0"!=getNumIpString(a.ipAddress)&&(checkHostVsSubmask(a.ipAddress,a.subnetMask)?(s="IP Address and Subnet Mask combination: The combination of IP address and subnet mask is invalid. All of the bits in the host address portion of the IP address are set to "+checkBits(a.ipAddress)+".",a.errorMessages.push(s),k=!0):(l=compareIpMaskNetwork(a.ipAddress,a.subnetMask),"2"!=l&&(s="IP Address and Subnet Mask combination: The combination of IP address and subnet mask is invalid. All of the bits in the network address portion of the IP address are set to "+
l+".",a.errorMessages.push(s),k=!0)));if(2==a.connection.protocolUsed&&a.defaultGateway&&n&&!k&&validateIp(a.ipAddress)&&getNumIpString(a.ipAddress)==getNumIpString(a.defaultGateway)){if(""!=s){f=[];for(l=0;l<a.errorMessages.length;l++)s!=a.errorMessages[l]&&f.push(a.errorMessages[l]);a.errorMessages=f}a.errorMessages.push("Incorrect Gateway IP: The Gateway cannot have the same IP address as the specified Wireless Broadband Router IP address.");f=!0}a.connection.primaryDnsServer&&1==a.connection.dnsMode&&
...
0>a.connection.deviceMetric||255<a.connection.deviceMetric)&&a.errorMessages.push("Device Metric: Please enter a numeric value between 0 and 255.");s=getIpString(a.ipAddress);a.connection.dnsMode=parseInt(a.connection.dnsMode);x=n="";a.connection.primaryDnsServer&&(n=getIpString(a.primaryDnsServer),x=getIpString(a.secondaryDnsServer));var B=parseInt(a.connection.protocolUsed),t=a.connection.subnetMask;if(2==B||1==B)t=getIpString(a.subnetMask);var A=a.connection.defaultGateway;a.defaultGateway&&(A=
...
a.errorMessages.push("Override Subnet Mask: Subnet mask may not be 0.0.0.0. Subnet mask must consist of 4 fields. Each field's value must be between 0 and 255, constructing a contiguous string of binary ones. The default mask value is 255.255.255.0."),l=!0);if(!D)l=!0,1==a.connection.connectionId&&1==a.connection.protocolUsed&&a.connection.overrideSubnetMask&&(l=!1),m="",m=l?"Subnet Mask: Subnet mask must consist of 4 fields. Each field's value must be between 0 and 255, constructing a contiguous string of binary ones. The default mask value is 255.255.255.0.":
"Override Subnet Mask: Subnet mask must consist of 4 fields. Each field's value must be between 0 and 255, constructing a contiguous string of binary ones. The default mask value is 255.255.255.0.",a.checkMsg(a.errorMessages,m)||a.errorMessages.push(m);else if(2==B&&a.connection.defaultRoute&&"0.0.0.0"==A)a.errorMessages.push("Default Gateway: The Default Gateway must not be 0.0.0.0 while the Default Route is set.");else if(2==B&&"0.0.0.0"!=A&&!l&&validateIp(a.ipAddress)&&!(b||e||h||k||m)){for(l=
...
g=!0):(l=compareIpMaskNetwork(a.defaultGateway,a.subnetMask),"2"!=l&&(a.errorMessages.push("Wireless Broadband Router and Default Gateway Are Not on the Same Subnet: Under the given Netmask, the IP Address does not belong to the same Subnet as the Default Gateway."),g=!0)))}}}0==a.connection.protocolUsed||1!=a.connection.dnsMode||1!=a.connection.connectionId||!a.connection.primaryDnsServer||b||e||f||g||h||(s==n&&1!=a.connection.protocolUsed?a.errorMessages.push("Primary DNS Server: DNS Server cannot have the IP address of Wireless Broadband Router "+
unescape(a.connection.name)+"."):n==a.coTempIPAddress&&a.errorMessages.push("Primary DNS Server: DNS Server cannot have the IP address of Wireless Broadband Router "+a.coTempHomeName+"."),s==x&&1!=a.connection.protocolUsed?a.errorMessages.push("Secondary DNS Server: DNS Server cannot have the IP address of Wireless Broadband Router "+unescape(a.connection.name)+"."):x==a.coTempIPAddress&&a.errorMessages.push("Secondary DNS Server: DNS Server cannot have the IP address of Wireless Broadband Router "+
a.coTempHomeName+"."),"0.0.0.0"!=n&&n==x&&a.errorMessages.push("Redundant DNS Entries: 'Primary' and 'Secondary' values may not point to the same DNS server."),"255.255.255.255"==n&&a.errorMessages.push("Primary DNS Server: "+n+" is invalid."),"255.255.255.255"==x&&a.errorMessages.push("Secondary DNS Server: "+x+" is invalid."));if(0!=a.errorMessages.length){a.showErrorPage=!0;window.scrollTo(0,0);return}2==a.connection.protocolUsed?(a.connection.ipAddress=s,a.defaultGateway&&(a.connection.defaultGateway=
...
function validateIPv4DhcpSettings(a,b,c,d,e,f,g,h,k,m,s){var p=[],z=getSubnetID(a,d),v=angular.copy(z);v[3]=100;z=angular.copy(z);z[3]=150;var q=!1;validateIp(a)?s&&(validateChecksForWholeNumber(a)||(q=!0)):q=!0;q&&p.push("Invalid IP Address provided. Values must be between 0 and 255");q=!1;validateIp(b)?s&&(validateChecksForWholeNumber(b)||(q=!0)):q=!0;q&&p.push("Start IP Address: IP address must consist of 4 fields. Each field's value must be between 0 and 255, without any spaces.");q=!1;validateIp(c)?
s&&(validateChecksForWholeNumber(c)||(q=!0)):q=!0;q&&p.push("End IP Address: IP address must consist of 4 fields. Each field's value must be between 0 and 255, without any spaces.");validateIp(a)&&null!=m&&checkAllNetworks(m,a,g)&&$scope.errorMessages.push("IP Address Already Exists: IP Address address "+getNumIpString(a)+" is already in use by "+checkAllNetworks4Name(m,a,g)+".");s=m=!1;if(2==h||1==h)if(s=!0,1!=h||k||(s=!1),s){if(!validateIp(d)||checkFullListOfSubnetMaskRange(d))"0.0.0.0"!=getIpString(d)&&
p.push("Subnet Mask: Subnet mask must consist of 4 fields. Each field's value must be between 0 and 255, constructing a contiguous string of binary ones. The default mask value is 255.255.255.0."),m=!0}else validateIp(d)||("0.0.0.0"!=getIpString(d)&&p.push("Subnet Mask: Subnet mask must consist of 4 fields. Each field's value must be between 0 and 255, constructing a contiguous string of binary ones. The default mask value is 255.255.255.0."),m=!0);isIPWithinRange(b,v,z)&&isIPWithinRange(c,v,z)&&
...
function isScheduleActive(a,b){if(null===a||"Always"===a||""===a)return"Active";var c=["Active","Inactive"];a.activeDuring||(c=["Inactive","Active"]);var d=b.day();0==d&&(d=7);for(var d=d-1,e=b.hour(),f=b.minute(),g=0;g<a.timeSegments.length;g++)if(-1!=String(a.timeSegments[g].daysOfTheWeek).indexOf(d))for(var h=0;h<a.timeSegments[g].hourRanges.length;h++){var k=a.timeSegments[g].hourRanges[h].startTimeHour,m=a.timeSegments[g].hourRanges[h].endTimeHour,s=a.timeSegments[g].hourRanges[h].startTimeMinute,
p=a.timeSegments[g].hourRanges[h].endTimeMinute;if(k==m&&s<=p){if(k==e&&m==e&&s<=f&&p>f)return c[0]}else if(k<m){if(e>k&&e<m||k==e&&f>=s||m==e&&f<p)return c[0]}else if(k<e||k==e&&s<=f||m>e||m==e&&p>f)return c[0]}return c[1]}
...
b&&(0==a.wirelessType?a.wirelessUpdate():a.check5GHzBroadcast())}):a.ssidBroadcast?a.wirelessUpdate():0==a.wirelessType?a.wirelessUpdate():a.check5GHzBroadcast()}}]).controller("WirelessMode",["$scope","$route","$location","Wireless","securityTips",function(a,b,c,d,e){a.tab=b.current.tab;a.section=b.current.section;var f=d.query(function(){a.modeA=f[0].mode;a.modeB=f[1].mode;a.wirelessAEnabled=f[0].radioEnabled;a.wAtype=f[0].security.enabled?0==f[0].security.type?0:1:1});a.wirelessUpdate=function(){d.update({id:0},
{mode:parseInt(a.modeA)},function(){d.update({id:1},{mode:parseInt(a.modeB)},function(){c.path("/wireless/advanced")})})};a.enableWEP=function(){0!=a.wAtype||!a.wirelessAEnabled||1!=a.modeA&&8!=a.modeA?a.wirelessUpdate():e.showCustomMessage2("Warning",["Compatibility Mode and Performance Mode are not compatible with WEP security. You must first select WPA/WPA2 Mixed Mode or WPA2 security before enabling Compatibility or Performance mode."])};a.save=function(){a.enableWEP()}}]).controller("WirelessTransmissionCtrl",
["$scope","$route","$routeParams","$location","WirelessTransmission",function(a,b,c,d,e){a.tab=b.current.tab;a.section=b.current.section;a.errorMessages=[];a.showErrorPage=!1;a.confirmed=!(null!=c.warn&&"true"==c.warn);a.trans1=e.get({id:0});a.trans2=e.get({id:1});a.runFocus=!1;a.errorPageOk=function(){a.showErrorPage=!1;window.scrollTo(0,0)};a.isConfirmed=function(){a.confirmed=!0;a.runFocus=!0};a.keyPress=function(b){13==b.keyCode&&!a.showErrorPage&&a.confirmed&&a.save()};a.save=function(){a.errorMessages=
...
-1;a.bothDisabled=!1;a.basicGuestWifiEnabled=!1;a.guestPassWifiEnabled=!1;a.guestWifiEnum={BASIC_GUEST_WIFI:2};a.tabsEnum={1:"partials/wireless/guestDevicesTab.html?v=v1.0.138",2:"partials/wireless/basicGuestWifiTab.html?v=v1.0.138"};a.section=b.current.section;b=c.path();-1!=b.indexOf("/wireless/guestWifiSetup/devices")?a.currentTab=1:-1!=b.indexOf("/wireless/guestWifiSetup/basic")?a.currentTab=2:a.wireless1=d.query(function(){a.wirelessA1=a.wireless1[0];a.basicGuestWifi1=a.wireless1[a.guestWifiEnum.BASIC_GUEST_WIFI];
...
"Saving your settings. Please wait...");a.countDownPopup.open();a.fakeTimer=window.setTimeout(a.closeCountDownPopup1,15E3);d.update({id:a.guestWifiEnum.BASIC_GUEST_WIFI},a.basicGuestWifi,function(){b?a.refresh():a.saveWifiKey()})};a.showKeyForm=function(){g.showCreateGuestWifiPassword("Create Password","Please create a password to activate your Guest Wi-Fi.",a.wpa).then(function(b){if("cancel"==b.result)a.refresh();else{var c=!1;if(void 0==b.returnObj)c=!0;else if(a.wpa.key=b.returnObj,8>a.wpa.key.length||
63<a.wpa.key.length)c=!0;c?a.openValidationPopup():f.update({id:a.guestWifiEnum.BASIC_GUEST_WIFI},{wpaShowKeyInUI:b.wpaShowKeyInUI},function(){a.saveSettings(!1)})}})};a.openValidationPopup=function(){g.showWarningMsgGuestWifi("","Password has to be at least 8 characters long",!1,[""]).then(function(b){"ok"==b&&a.showKeyForm()})};a.openValidationPopup1=function(){g.showWarningMsgGuestWifi("","Password has to be at least 8 characters long",!1,[""]).then(function(a){})};a.openValidationPopup2=function(){g.showWarningMsgGuestWifi("",
"SSID must be 1 to 64 characters",!1,[""]).then(function(a){})};a.openValidationPopup3=function(){g.showWarningMsgGuestWifi2("","SSID must be 1 to 64 characters",!1,[""]).then(function(a){})};a.saveWifiKey=function(){a.wpa.encryptionAlgorithm=parseInt(a.wpa.encryptionAlgorithm);f.update({id:a.guestWifiEnum.BASIC_GUEST_WIFI},{type:a.wpa.type,mode:a.wpa.mode,encryptionAlgorithm:a.wpa.encryptionAlgorithm,groupKeyUpdateInterval:a.wpa.groupKeyUpdateInterval,groupKeyUpdateIntervalEnabled:a.wpa.groupKeyUpdateIntervalEnabled,
