...
(a.errorMessages=validateIPv4DhcpSettings(splitIpArr(a.connection.ipAddress),a.dhcpStartIpAddress,a.dhcpEndIpAddress,splitIpArr(a.connection.subnetMask),a.dhcpWinsServer,a.dhcpSettings.leaseTime,null,null,null,null),0!=a.errorMessages.length)){a.showErrorPage=!0;window.scrollTo(0,0);return}var b=getIpString(a.dhcpWinsServer);a.dhcpSettings.winsServer="0.0.0.0"!=b&&0!=a.dhcpSettings.mode?b:"";a.dhcpSettings.startIpAddress=getIpString(a.dhcpStartIpAddress);a.dhcpSettings.endIpAddress=getIpString(a.dhcpEndIpAddress);
...
for(var c=getMacString(a.macAddress),g=getIpString(a.ipAddress),b=splitIpArr(a.lanIp),e=0;e<a.devices.length;e++)if((a.editIndex!=e||!a.editing)&&g===a.devices[e].ipAddress){a.errorMessages.push("IP Address: IP address already exists in Wireless Broadband Router's database.");break}"00:00:00:00:00:00"==c&&a.errorMessages.push("MAC Address: MAC address must consist of 6 fields. Each field's value must be between 00 and FF (in hexadecimal representation), without any spaces. The LSB of the first byte must be zero.");
checkMacAddress(a,a.macAddress);"0.0.0.0"==g?a.errorMessages.push("IP Address: IP address may not be 0.0.0.0. IP address must consist of 4 fields. Each field's value must be between 0 and 255, without any spaces. At least one of the values must be greater than 0."):validateIp(a.ipAddress)||a.errorMessages.push("IP Address: IP address must consist of 4 fields. Each field's value must be between 0 and 255, without any spaces.");a.lanIp==g&&a.errorMessages.push("IP Address: The IP address "+g+" conflicts with Subnet connection's IP.");
if(0==a.errorMessages.length){e=!0;if((a.ipAddress[0]&a.subnetMask[0])!=(b[0]&a.subnetMask[0])||(a.ipAddress[1]&a.subnetMask[1])!=(b[1]&a.subnetMask[1])||(a.ipAddress[2]&a.subnetMask[2])!=(b[2]&a.subnetMask[2])||(a.ipAddress[3]&a.subnetMask[3])!=(b[3]&a.subnetMask[3]))e=!1;e||a.errorMessages.push("IP Address: The IP address "+g+" is illegal for any of the connections subnets.")}0!=a.errorMessages.length||validateMac(c)||a.errorMessages.push("MAC Address: MAC address must consist of 6 fields. Each field's value must be between 00 and FF (in hexadecimal representation), without any spaces. The LSB of the first byte must be zero.");
if(0==a.errorMessages.length){for(e=0;e<a.devices.length;e++)if((!a.editing||e!=a.editIndex)&&a.name===a.devices[e].name){a.errorMessages.push("Name: Name already exists in Wireless Broadband Router's database.");a.errorMessages.push('If you are trying to make changes to the lease type on this interface, please edit the host name to make it unique (e.g. "Hostname-Ethernet" or "Hostname-WiFi")');break}for(e=0;e<a.devices.length;e++)if((!a.editing||e!=a.editIndex)&&c===a.devices[e].mac){a.errorMessages.push("MAC Address: MAC Address already exists in Wireless Broadband Router's database.");
break}}0<a.errorMessages.length?(a.showErrorPage=!0,window.scrollTo(0,0)):(a.useStaticIp&&a.showSettings&&a.ipAdd!=g&&!a.useStaticIp&&h.showMessage("DHCP Connection Update Warning","Changes to the DHCP connections table IP addresses won\u2019t take effect on the actual host table until the client device renews its DHCP lease. You can either wait until the lease expires and is renewed by the client, or you can manually renew the lease on the client."),a.editing&&c!=a.devices[a.editIndex].mac?d.remove({clientId:a.editIndex},
function(){d.save({},{name:a.name,ipAddress:g,mac:c,staticAddress:a.useStaticIp},function(){a.adding=!1;a.refresh()})}):d.save({},{name:a.name,ipAddress:g,mac:c,staticAddress:a.useStaticIp},function(){a.adding=!1;a.refresh()}))};a.doPingTest=function(b){f.save({destination:a.devices[b].ipAddress,count:4},function(){c.path("/advanced/diagnostics")})};a.editConnection=function(b){a.adding=!0;a.editing=!0;a.editIndex=b;b=a.devices[b];a.name=b.name;a.ipAddress=splitIpArr(b.ipAddress);a.ipAdd=b.ipAddress;
a.macAddress=splitMacArr(b.mac);a.macAdd=b.mac;a.useStaticIp=b.staticCheckbox;a.showSettings=b.staticCheckbox};a.cancelStaticConnection=function(){a.adding=!1;a.showSettings=!1};a.deleteConnection=function(b){d.remove({id:0,clientId:b},function(){a.refresh()})};a.refresh=function(){var b=g.get(function(){a.startAddress=splitIpArr(b.startIpAddress);a.endAddress=splitIpArr(b.endIpAddress)});a.conn=e.get({id:0},function(){a.lanIp=a.conn.ipAddress;a.subnetMask=splitIpArr(a.conn.subnetMask);a.conn.displayName=
unescape(a.conn.name)});a.devices=d.query(function(){a.devices=filterOutGuest(a.devices)})};a.keyPress=function(b){13==b.keyCode&&"/advanced/dhcp/connections"==c.path()&&(a.adding||a.showErrorPage?a.adding&&!a.showErrorPage&&a.saveStaticConnection():c.path("/advanced/dhcp"))};a.refresh()}]).controller("fwUpgradeCtrl",["$scope","$route","FirmwareUpgrade","Reboot","SystemSettings","$location","$templateCache","$timeout",function(a,b,c,d,e,f,g,h){a.tab=b.current.tab;a.section=b.current.section;a.page=
0;a.restarting=!1;a.inputfile=null;a.runFocus=!1;a.fakeTimer=null;a.fakeCount=0;a.fake_percent=0;a.$on("$destroy",function(){clearTimeout(a.fakeTimer)});a.fakeTimerFunction=function(){clearTimeout(a.fakeTimer);a.fakeCount++;60<a.fakeCount&&(a.fakeCount=0);a.fake_percent=a.fakeCount/60*100;a.fake_percent=Math.round(a.fake_percent);a.fakeTimer=window.setTimeout(a.fakeTimerFunction,1E3);a.$apply()};a.refresh=function(){a.stats=c.get()};a.refresh();a.errorPageOk=function(){a.runFocus=!0;a.showErrorPage=
!1;a.page=0;window.scrollTo(0,0)};window.uploadDone=function(){a.isFileSubmitted&&(a.stats=c.get(function(){""==a.stats.standbyVersion||null==a.stats.standbyVersion?(a.errorMessages=["Invalid Firmware"],a.showErrorPage=!0):a.page=2;window.scrollTo(0,0)}))};a.applyFirmware=function(){c.save({executeUpdate:!0});d.save(function(){window.setTimeout(a.testConnection,0)});a.page=3;a.fakeTimerFunction();window.scrollTo(0,0)};a.cancelFirmwareUpdate=function(){a.page=0;window.scrollTo(0,0)};a.submittingFile=
function(){a.isFileSubmitted=!0};a.fileUpgradePage=function(){h(function(){document.getElementById("submitting").focus()},1E3);a.inputfile=null;a.page=1;window.scrollTo(0,0)};a.testConnection=function(){e.get(function(){window.setTimeout(a.testConnection,3E3)},function(b){401==b.status?(a.restarting=!0,g.removeAll(),clearTimeout(a.fakeTimer),window.location="/redirect.html?v=v1.0.125"):window.setTimeout(a.testConnection,3E3)})};a.chooseFile=function(){h(function(){document.getElementById("submitting").focus()},
1E3)};a.setFocus=function(){h(function(){document.getElementById("submitting").focus()},500)};a.keyPress=function(b){13==b.keyCode&&"/advanced/fwupgrade"==f.path()&&0==a.page&&a.fileUpgradePage()}}]).controller("fwRestoreCtrl",["$scope","$route","FirmwareUpgrade","Reboot","SystemSettings","$location","$templateCache",function(a,b,c,d,e,f,g){a.tab=b.current.tab;a.section=b.current.section;a.valid=!1;a.restarting=!1;a.stats=c.get(function(){a.valid=""!==a.stats.restoreVersion});a.apply=function(){c.save({executeRestore:!0});
d.save(function(){window.setTimeout(a.testConnection,0)});a.page=3};a.testConnection=function(){e.get(function(){window.setTimeout(a.testConnection,3E3)},function(b){401==b.status?(a.restarting=!0,g.removeAll(),window.location="/redirect.html?v=v1.0.125"):window.setTimeout(a.testConnection,3E3)})}}]).controller("localAdminCtrl",["$scope","$route","$location","LocalAdmin","SystemSettings",function(a,b,c,d,e){a.tab=b.current.tab;a.section=b.current.section;a.sysSettings=e.get();a.locAdmin=d.get();
a.save=function(){a.locAdmin.$update(function(){c.path("/advanced")})};a.keyPress=function(b){13==b.keyCode&&"/advanced/localadmin"==c.path()&&a.save()}}]).controller("remoteAdminCtrl",["$scope","$route","$routeParams","$location","RemoteAdmin","SystemSettings",function(a,b,c,d,e,f){a.tab=b.current.tab;a.section=b.current.section;a.sysSettings=f.get();a.remAdmin=e.get();a.fromString="?from=ad";b="";a.prev="";a.back="/advanced";a.comingFrom="";if(null!=c.from){a.comingFrom=c.from;a.fromString="?from="+
c.from+"-ad";a.back="/advanced/settings";c=c.from.split("-");for(e=0;e<c.length-1;e++)0!=e&&(b+="-"),b+=c[e];""!==b&&(a.prev="?from="+b)}else a.back="/advanced";a.save=function(){a.remAdmin.$update(function(){""==a.oldFrom?d.path(a.back).search({}):"settings"==a.comingFrom?location.reload():d.path(a.back).search({from:a.oldFrom})})};a.keyPress=function(b){13==b.keyCode&&"/advanced/remoteadmin"==d.path()&&a.save()}}]).controller("systemSettingsCtrl",["$scope","$route","$routeParams","$location","SystemSettings",
"securityTips",function(a,b,c,d,e,f){a.tab=b.current.tab;a.section=b.current.section;a.showErrorPage=!1;a.fromString="?from=settings";a.oldFrom="";a.prev="";a.oldDomainName="";if(null!=c.from){a.fromString="?from="+c.from+"-settings";b=c.from.split("-");"fw"==b[b.length-1]?a.back="/firewall/remoteadmin":"ad"==b[b.length-1]&&(a.back="/advanced/remoteadmin");for(c=0;c<b.length-1;c++)0!=c&&(a.oldFrom+="-"),a.oldFrom+=b[c];""!==a.oldFrom&&(a.prev="?from="+a.oldFrom)}else a.back="/advanced";a.sysSettings=
e.get(function(){a.systemIpAddress=splitIpArr(a.sysSettings.systemRemoteHost);a.securityIpAddress=splitIpArr(a.sysSettings.securityRemoteHost);a.oldDomainName=a.sysSettings.localDomain});a.errorPageOk=function(){a.showErrorPage=!1;window.scrollTo(0,0)};a.save=function(){a.errorMessages=[];validateLocalDomains(a.sysSettings.localDomain)||a.errorMessages.push("Domain Name: Name may not contain spaces. Only letters, digits and the special characters dash (-), underscore (_) and dot (.) may be used. These special characters may not appear at the beginning or at the end of a name. The maximum length of a label (text between two dots) is 63");
...
b&&a.redirect()})}))};a.redirect=function(){""==a.oldFrom?d.path(a.back).search({}):d.path(a.back).search({from:a.oldFrom})};a.systemLowCapacityChanged=function(){a.sysSettings.systemLowCapacityNotificationEnabled||(a.sysSettings.securityLowCapacityNotificationEnabled=!0)};a.securityLowCapacityChanged=function(){a.sysSettings.securityLowCapacityNotificationEnabled||(a.sysSettings.systemLowCapacityNotificationEnabled=!0)};a.keyPress=function(b){13==b.keyCode&&"/advanced/settings"==d.path()&&a.save()}}]).controller("dateTimeCtrl",
["$scope","$route","$routeParams","$location","DateTimeSettings",function(a,b,c,d,e){a.tab=b.current.tab;a.section=b.current.section;var f=function(){a.dateTimeSettings=e.get(function(){a.checkTime();a.checkTempDateObj();for(var b=0;b<a.dateTimeSettings.servers.length;b++){var c={};c.name=a.dateTimeSettings.servers[b];c.id=b;a.dateTimeSettings.servers[b]=c}a.timezone=parseInt(a.dateTimeSettings.timeZone)})};f();a.pageToShow=0;a.serverName="Name";a.showErrorPage=!1;a.tempDateObj=null;a.errorPageOk=
function(){a.showErrorPage=!1;window.scrollTo(0,0)};a.syncNow=function(){};a.refresh=function(){void 0!=window?window.location.reload(!0):location.reload(!0)};a.togglePage=function(b){a.pageToShow=b;window.scrollTo(0,0)};a.save=function(){for(var b=0;b<a.dateTimeSettings.servers.length;b++)a.dateTimeSettings.servers[b].hasOwnProperty("name")&&(a.dateTimeSettings.servers[b]=a.dateTimeSettings.servers[b].name);a.dateTimeSettings.timeZone=parseInt(a.dateTimeSettings.timeZone);b={autoTimeUpdate:a.dateTimeSettings.autoTimeUpdate,
localTime:a.dateTimeSettings.localTime,timeZone:parseInt(a.dateTimeSettings.timeZone),offset:parseInt(a.dateTimeSettings.offset),servers:a.dateTimeSettings.servers};e.update(b,function(){a.dateTimeSettings=e.get(function(){for(var b=0;b<a.dateTimeSettings.servers.length;b++){var c={};c.name=a.dateTimeSettings.servers[b];c.id=b;a.dateTimeSettings.servers[b]=c}})})};a.offsetToZone=function(){var b=a.offset/60;0==b?a.dateTimeSettings.timeZone=0:-5>=b&&-10<=b&&(a.dateTimeSettings.timeZone=b)};a.checkTime=
function(){null!=c.tz&&(a.dateTimeSettings.timeZone=c.tz);a.offset=0;a.localTimeMoment=findTimeNow(a.dateTimeSettings);a.localTimeFormatted=a.localTimeMoment.format("MMM DD, YYYY HH:mm:ss")};a.checkTempDateObj=function(){null!=c.tmplocalTime&&(a.tempDateObj={localTime:c.tmplocalTime},a.tempDateObj.timeZone=a.dateTimeSettings.timeZone,a.dateTimeSettings.autoTimeUpdate=!1,a.localTimeMoment=findTimeNow(a.tempDateObj),a.localTimeFormatted=a.localTimeMoment.format("MMM DD, YYYY HH:mm:ss"),a.tempDateObj.month=
a.localTimeMoment.month(),a.tempDateObj.day=a.localTimeMoment.date(),a.tempDateObj.year=a.localTimeMoment.year(),a.tempDateObj.hours=a.localTimeMoment.hour(),a.tempDateObj.minutes=a.localTimeMoment.minute(),a.tempDateObj.seconds=a.localTimeMoment.second())};a.checkTempObj=function(){a.dateTimeSettings.autoTimeUpdate&&(a.tempDateObj=null)};a.setTimeZone=function(){null==a.tempDateObj?d.path("/advanced/datetime").search({tz:a.dateTimeSettings.timeZone}):d.path("/advanced/datetime").search({tz:a.dateTimeSettings.timeZone,
...
d=moment.unix(moment.utc(moment(d,"YYYY-MM-DD-HH-mm-ss")).unix()-60*(new Date).getTimezoneOffset()-3600*e),f={};f.localTime=d.unix();f.timeZone=a.tempDateObj.timeZone;var f=findTimeNow(f),e=f.format("HH"),f=f.format("mm"),q=!1,z=0,q=0;if(q=Number(e)==Number(b)&&Number(f)==Number(c)?!1:!0)q=4>b&&20<e?Number(b)-Number(e-24):20<b&&4>e?Number(b-24)-Number(e):Number(b)-Number(e),0!=q&&(z=3600*q),q=Number(c)-Number(f),0!=q&&(z+=60*q);a.tempDateObj.localTime=d.unix()+z;a.localTimeMoment=findTimeNow(a.tempDateObj);
a.localTimeFormatted=a.localTimeMoment.format("MMM DD, YYYY HH:mm:ss")};a.applySetClock=function(){a.errorMessages=[];var b=a.tempDateObj.hours,c=a.tempDateObj.minutes,d=a.tempDateObj.seconds,f=moment(a.tempDateObj.year+"-"+(parseInt(a.tempDateObj.month)+1)+"-"+a.tempDateObj.day,"YYYY-MM-DD");(""===b||isNaN(b)||0>b||23<b)&&a.errorMessages.push("Hours: Please enter a numeric value between 0 and 23.");(""===c||isNaN(c)||0>c||59<c)&&a.errorMessages.push("Minutes: Please enter a numeric value between 0 and 59.");
(""===d||isNaN(d)||0>d||59<d)&&a.errorMessages.push("Seconds: Please enter a numeric value between 0 and 59.");f.isValid()||a.errorMessages.push("Local Date and Time:  Entered time or date value are incorrect.");0<a.errorMessages.length?(a.showErrorPage=!0,window.scrollTo(0,0)):(a.calDate(),e.update({localTime:a.tempDateObj.localTime},function(){a.togglePage(0)}))};a.cancelSetClock=function(){a.tempDateObj=null;a.togglePage(0)};a.saveServer=function(){a.errorMessages=[];a.showError=!1;validateIpDomainDatetime(a.serverName)||
isAddressValidIPPattern(a.serverName)||(a.showError=!0);if(a.showError)a.errorMessages.push("Server Address: The value must be a valid IP address or a valid domain name."),a.showErrorPage=!0,window.scrollTo(0,0);else{for(var b=0;b<a.dateTimeSettings.servers.length;b++)a.dateTimeSettings.servers[b]=a.dateTimeSettings.servers[b].name;-1==a.editing?a.dateTimeSettings.servers.push(a.serverName):a.dateTimeSettings.servers[a.editing]=a.serverName;e.update({autoTimeUpdate:a.dateTimeSettings.autoTimeUpdate,
servers:a.dateTimeSettings.servers},function(){f()});a.togglePage(0)}};a.addServer=function(){a.serverName="";a.editing=-1;a.togglePage(1);window.scrollTo(0,0)};a.editServer=function(b){a.serverName=a.dateTimeSettings.servers[b].name;a.editing=b;a.togglePage(1);window.scrollTo(0,0)};a.removeServer=function(b){for(var c=0;c<a.dateTimeSettings.servers.length;c++)a.dateTimeSettings.servers[c]=a.dateTimeSettings.servers[c].name;a.dateTimeSettings.servers.splice(b,1);e.update({autoTimeUpdate:a.dateTimeSettings.autoTimeUpdate,
servers:a.dateTimeSettings.servers},function(){f()})};a.keyPress=function(b){13==b.keyCode&&"/advanced/datetime"==d.path()&&(0!=a.pageToShow||a.showErrorPage?1!=a.pageToShow||a.showErrorPage||a.saveServer():a.save())}}]).controller("configFileCtrl",["$scope","$route","$location","ConfigInfo","Reboot","Login","SystemSettings","$templateCache","$timeout",function(a,b,c,d,e,f,g,h,k){a.tab=b.current.tab;a.section=b.current.section;a.loadFile=!1;a.completed=!1;a.showErrorPage=!1;a.inputFile=null;a.restarting=
!1;window.uploadDone=function(){if(a.loadFile)var b=d.get(function(){b.successful?(e.save(function(){window.setTimeout(a.testConnection,0)}),a.completed=!0):(a.errorMessages=["Configuration File: Bad format of configuration file."],a.showErrorPage=!0)})};a.testConnection=function(){g.get(function(){window.setTimeout(a.testConnection,3E3)},function(b){401==b.status?(a.restarting=!0,h.removeAll(),window.location="/redirect.html?v=v1.0.125"):window.setTimeout(a.testConnection,3E3)})};a.toggleShow=
function(b){k(function(){document.getElementById("submitting").focus()},1E3);a.loadFile=!a.loadFile;0==b&&(a.inputFile=null)};a.errorPageOk=function(){a.showErrorPage=!1;a.loadFile=!1;a.completed=!1;a.isFileSubmitted=!1;window.scrollTo(0,0)};a.submittingFile=function(){a.isFileSubmitted=!0};a.saving=function(){f.get(function(a){window.location="api/settings/configfile"})};a.loadConfig=function(){document.getElementById("submitting").focus()};a.setFocus=function(){k(function(){document.getElementById("submitting").focus()},
500)};a.keyPress=function(b){13==b.keyCode&&"/advanced/configfile"==c.path()&&(a.loadFile||c.path("/advanced"))}}]).controller("diagnosticsCtrl",["$scope","$route","$rootScope","Diagnostics","$location",function(a,b,c,d,e){a.tab=b.current.tab;a.section=b.current.section;a.destination="";a.pingCount=4;a.showErrorPage=!1;a.previousPath=c.pre_path;a.isCancelled=!1;a.errorPageOk=function(){a.showErrorPage=!1;window.scrollTo(0,0)};a.refresh=function(){var b=d.get(function(){a.destination=b.destination;
a.pingCount=b.count;a.total=b.count;a.status=b.running?1:0;a.testRan=!0;null==b.transmitted?b.running?window.setTimeout(a.refresh,1E3):a.testRan=!1:(a.sent=b.transmitted,a.received=b.received,0==a.received&&0==a.sent&&0==a.status&&(a.status=5),0==a.status&&a.sent!=a.total&&(a.status=4),0==a.status&&a.sent==a.total&&a.received!=a.sent&&(a.status=2),a.loss=0<a.sent?Math.floor((a.sent-a.received)/a.sent*100):0,a.isCancelled&&(a.status=4),0<a.received?(a.min=b.min.toFixed(2),a.max=b.max.toFixed(2),a.average=
b.average.toFixed(2),a.hasResults=!0):a.hasResults=!1,1==a.status&&b.transmitted!=b.count&&window.setTimeout(a.refresh,1E3))})};a.goButton=function(){a.isCancelled=!1;a.errorMessages=[];var b=a.destination.split("."),c=!1;(c=validateIp(b))||(c=validateIPv6(a.destination));b=validateGivenHostName(a.destination);c||b||a.errorMessages.push("Server Address: The value must be a valid IP address or a valid domain name.");(isNaN(a.pingCount)||1>a.pingCount||999<a.pingCount)&&a.errorMessages.push("Number of pings: Please enter a numeric value between 1 and 999.");
0<a.errorMessages.length?(a.showErrorPage=!0,window.scrollTo(0,0)):(a.status=-1,a.total=4,a.sent=0,a.received=0,a.loss=0,a.hasResults=!1,d.save({destination:a.destination,count:parseInt(a.pingCount)},function(){a.status=-1;a.refresh()}))};a.cancel=function(){a.isCancelled=!0;d.remove({})};a.keyPress=function(b){13==b.keyCode&&"/advanced/diagnostics"==e.path()&&a.goButton()};a.refresh()}]).controller("defaultsCtrl",["$scope","$route","Defaults","Login","Reboot","SystemSettings","$location","$templateCache",
function(a,b,c,d,e,f,g,h){a.tab=b.current.tab;a.section=b.current.section;a.restDefaults=0;a.resetType=0;a.restarting=!1;a.defaults=function(){a.restDefaults=1};a.cancel=function(){a.restDefaults=0};a.restoreDefaults=function(){a.restDefaults=2;c.save({resetType:parseInt(a.resetType)},function(){e.save(function(){window.setTimeout(a.testConnection,0)})})};a.saving=function(){d.get(function(a){window.location="api/settings/configfile"})};a.testConnection=function(){f.get(function(){window.setTimeout(a.testConnection,
3E3)},function(b){401==b.status?(a.restarting=!0,h.removeAll(),window.location=0==a.resetType?"/redirectSetup.html?v=v1.0.125":"/redirect.html?v=v1.0.125"):window.setTimeout(a.testConnection,3E3)})}}]).controller("rebootCtrl",["$scope","$route","Reboot","SystemSettings","Login","$location","$templateCache",function(a,b,c,d,e,f,g){a.tab=b.current.tab;a.section=b.current.section;a.sysSettings=d.get();a.rebooting=!1;a.restarting=!1;a.reboot=function(){a.rebooting=!0;c.save(function(){window.setTimeout(a.testConnection,
0)})};a.testConnection=function(){d.get(function(){window.setTimeout(a.testConnection,3E3)},function(b){401==b.status?(a.restarting=!0,g.removeAll(),window.location="/redirect.html?v=v1.0.125"):window.setTimeout(a.testConnection,3E3)})};a.keyPress=function(b){13==b.keyCode&&"/advanced/reboot"==f.path()&&a.reboot()}}]).controller("macCloningCtrl",["$scope","$route","$location","MacCloning",function(a,b,c,d){a.tab=b.current.tab;a.section=b.current.section;a.showErrorPage=!1;a.macCloning=d.get(function(){a.macAddress=
splitMacArr(a.macCloning.mac)});a.errorPageOk=function(){a.showErrorPage=!1;window.scrollTo(0,0)};a.save=function(){a.errorMessages=[];a.macCloning.mac=getMacString(a.macAddress);if(!validateMac(a.macCloning.mac))a.errorMessages=["MAC Address: MAC address must consist of 6 fields. Each field's value must be between 00 and FF (in hexadecimal representation), without any spaces. The LSB of the first byte must be zero."];else if(/[f\:]{17}$/i.test(a.macCloning.mac))a.errorMessages.push("MAC Address: MAC FF:FF:FF:FF:FF:FF is reserved for broadcast.");
else if(!validateMac(a.macCloning.mac)||/[0\:]{17}$/i.test(a.macCloning.mac)||1==(parseInt(a.macAddress[0],16)&1))a.errorMessages=["MAC Address: MAC address must consist of 6 fields. Each field's value must be between 00 and FF (in hexadecimal representation), without any spaces. The LSB of the first byte must be zero."];0<a.errorMessages.length?(a.showErrorPage=!0,window.scrollTo(0,0)):a.macCloning.$update(function(){c.path("/advanced")})};a.restore=function(){a.macAddress=splitMacArr(a.macCloning.defaultMac)};
a.keyPress=function(b){13==b.keyCode&&"/advanced/maccloning"==c.path()&&a.save()}}]).controller("usersCtrl",["$scope","$route","$routeParams","$location","Users","SystemSettings",function(a,b,c,d,e,f){a.tab=b.current.tab;a.section=b.current.section;a.sysSettings=f.get();a.updateContent=function(){a.userObj=e.get(function(){a.user=a.userObj.users[0];a.userObj.users[0].fullName=unescape(a.userObj.users[0].fullName);null!=c.edit&&a.edit(0)})};a.updateContent();a.pageToShow=0;a.editing=-1;a.showErrorPage=
!1;a.setPassword=!1;a.showPassword=!1;a.userObjs=null;a.newPass=!1;a.errorPageOk=function(){a.showErrorPage=!1;window.scrollTo(0,0)};a.save=function(){a.errorMessages=[];var b=Number(a.userObj.maxLoginAttempts);(isNaN(b)||1>b||99<b)&&a.errorMessages.push("Maximum Unsuccessful Login Attempts: Please enter a numeric value between 1 and 99.");0<a.errorMessages.length?(a.showErrorPage=!0,window.scrollTo(0,0)):e.update({maxLoginAttempts:b},function(){void 0!=window?window.location.reload(!0):location.reload(!0)})};
a.togglePage=function(b){0==b?null!=c.frommain?d.url(d.path("/main")):(a.pageToShow=b,a.updateContent()):a.pageToShow=b;window.scrollTo(0,0)};a.edit=function(b){a.newName=a.userObj.users[0].fullName;a.newPassword="";a.rtNewPassword="";a.setPassword=!1;a.editing=b;a.togglePage(1)};a.saveUserCfm=function(){e.update({users:[a.userObjs]},function(){a.userObj.users[0].fullName=unescape(a.userObj.users[0].fullName);a.newPass?d.path("/logout"):a.togglePage(0)})};a.saveUser=function(){a.userObjs={id:0};a.errorMessages=
[];""==a.newName&&a.errorMessages.push("Full Name: A string must be specified.");a.newPass=!1;a.setPassword&&(0==a.newPassword.length&&0==a.rtNewPassword.length&&a.errorMessages.push("New Password: A password must be specified."),a.newPassword!=a.rtNewPassword&&a.errorMessages.push("Incorrect New Password: Entered passwords mismatch."),0==a.errorMessages.length&&(a.user.password=a.newPassword),a.userObjs.salt=uuid(),a.userObjs.password=CryptoJS.SHA512(a.newPassword+a.userObjs.salt).toString(),a.newPass=
!0);0<a.errorMessages.length?(a.showErrorPage=!0,window.scrollTo(0,0)):(a.userObjs.fullName=escape(a.newName).replace(/\+/g,"%2B"),a.newPass?!0==a.sysSettings.warnBeforeChanges?a.togglePage(2):a.saveUserCfm():a.saveUserCfm())};a.keyPress=function(b){13==b.keyCode&&"/advanced/users"==d.path()&&(1!=a.pageToShow||a.showErrorPage?a.save():a.saveUser())}}]).controller("schedulerCtrl",["$scope","$route","Schedules","DateTimeSettings","$location","AccessControl","PortForward",function(a,b,c,d,e,f,g){a.tab=
b.current.tab;a.section=b.current.section;a.runFocus=!1;init_schedules(a);a.pageToShow=0;a.editing=!1;a.showErrorPage=!1;a.errorPageOk=function(){a.showErrorPage=!1;window.scrollTo(0,0)};a.save=function(){a.schedules.$update(function(){void 0!=window?window.location.reload(!0):location.reload(!0)})};a.refresh=function(){a.schedules=c.query(function(){for(var b=0;b<a.schedules.length;b++)a.schedules[b].name=unescape(a.schedules[b].name);a.printSchedArr=[];printSchedule(0,a.schedules,a.printSchedArr);
a.dateTimeSettings=d.get(function(){a.localTimeMoment=findTimeNow(a.dateTimeSettings);for(var b=0;b<a.schedules.length;b++)a.schedules[b].status=isScheduleActive(a.schedules[b],a.localTimeMoment)})})};a.refresh();a.togglePage=function(b){a.pageToShow=b;window.scrollTo(0,0)};a.removeSchedule=function(b){var c=!1;a.toDeleteIndex=b;a.portForward=g.query(function(){a.accessControl=f.query(function(){for(var b=0;b<a.accessControl.length;b++)if(null!=a.accessControl[b].schedule&&unescape(a.accessControl[b].schedule.name)==
a.schedules[a.toDeleteIndex].name){c=!0;break}for(b=0;b<a.portForward.length;b++)if(null!=a.portForward[b].schedule&&unescape(a.portForward[b].schedule)==a.schedules[a.toDeleteIndex].name){c=!0;break}c?a.togglePage(8):a.removeScheduleNow(a.toDeleteIndex)})})};a.updateAccessControlBeforeRemove=function(){var b=!0;a.portForward=g.query(function(){a.accessControl=f.query(function(){for(var c=0;c<a.accessControl.length;c++)if(null!=a.accessControl[c].schedule&&unescape(a.accessControl[c].schedule.name)==
a.schedules[a.toDeleteIndex].name){a.accessControl[c].schedule="";a.accessControl[c].schedule1={};f.update({id:a.accessControl[c].id},a.accessControl[c],function(){a.updateAccessControlBeforeRemove()});b=!1;break}if(b)for(c=0;c<a.portForward.length;c++)if(null!=a.portForward[c].schedule&&unescape(a.portForward[c].schedule)==a.schedules[a.toDeleteIndex].name){a.portForward[c].schedule="Always";g.remove({id:a.portForward[c].id},function(){a.portForward[c].id="";g.save(a.portForward[c],function(){a.updateAccessControlBeforeRemove()})});
b=!1;break}b&&a.removeScheduleNow(a.toDeleteIndex)})})};a.removeScheduleNow=function(b){c.remove({id:a.schedules[b].id},function(){a.togglePage(0);a.refresh()})};a.applyRuleScheduler=function(){a.errorMessages=[];""===a.schedule.name&&a.errorMessages.push("Description: A string must be specified.");for(var b=0;b<a.schedules.length;b++)if(b!=a.editingRuleSchedule&&a.schedules[b].name==a.schedule.name){a.errorMessages.push("Name: This name is already used by another scheduler rule.");break}0==a.schedule.timeSegments.length&&
a.errorMessages.push("Rule Schedule: At least one time segment should be defined.");if(0<a.errorMessages.length)a.showErrorPage=!0,window.scrollTo(0,0);else{var d={};copySchedule(d,a.schedule);for(b=0;b<d.timeSegments.length;b++)for(var e=0;e<d.timeSegments[b].hourRanges.length;e++)d.timeSegments[b].hourRanges[e].startTimeHour=parseInt(d.timeSegments[b].hourRanges[e].startTimeHour),d.timeSegments[b].hourRanges[e].startTimeMinute=parseInt(d.timeSegments[b].hourRanges[e].startTimeMinute),d.timeSegments[b].hourRanges[e].endTimeHour=
parseInt(d.timeSegments[b].hourRanges[e].endTimeHour),d.timeSegments[b].hourRanges[e].endTimeMinute=parseInt(d.timeSegments[b].hourRanges[e].endTimeMinute);d.name=escape(a.schedule.name).replace(/\+/g,"%2B");-1==a.editingRuleSchedule?c.save(d,function(){a.refresh()}):(d.id=a.schedules[a.editingRuleSchedule].id,c.update(d,function(){a.refresh()}));a.togglePage(0);init_schedules(a)}};a.formatTimeString=function(a,b){return(10>a?"0"+a:a)+":"+(10>b?"0"+b:b)};a.applyTimeSegment=function(){a.errorMessages=
[];for(var b=!1,c=0;c<a.daysOfTheWeek.length;c++)a.daysOfTheWeek[c]&&(b=!0);b||a.errorMessages.push("Days of Week: At least one day must be selected.");0==a.hourRanges.length&&a.errorMessages.push("Hours Range: At least one hour range must be set.");0<a.errorMessages.length?a.showErrorPage=!0:applyTimeSegment(a);window.scrollTo(0,0)};a.applyHourRange=function(){a.runFocus=!0;a.errorMessages=[];var b=a.hourRange.startTimeHour,c=a.hourRange.endTimeHour,d=a.hourRange.startTimeMinute,e=a.hourRange.endTimeMinute;
(""===b||isNaN(b)||0>b||23<b||!checksForWholeNumber(b))&&a.errorMessages.push("Start Time Hour: Please enter a numeric value between 0 and 23.");(""===c||isNaN(c)||0>c||23<c||!checksForWholeNumber(c))&&a.errorMessages.push("End Time Hour: Please enter a numeric value between 0 and 23.");(""===d||isNaN(d)||0>d||59<d||!checksForWholeNumber(d))&&a.errorMessages.push("Start Time Minutes: Please enter a numeric value between 0 and 59.");(""===e||isNaN(e)||0>e||59<e||!checksForWholeNumber(e))&&a.errorMessages.push("End Time Minutes: Please enter a numeric value between 0 and 59.");
...
b>=c&&a.errorMessages.push("Source Ports: The From value must be lower than the To value in a range of ports (from="+b+", to="+c+").")}switch(a.protocols.outgoingPorts){case 1:(null==d||""===d||1>d||65535<d||isNotaInt(d))&&a.errorMessages.push("Destination Port: Please enter a numeric value between 1 and 65535.");4567==d&&!1==a.protocols.outgoingExclude&&1==a.protocols.protocol&&a.errorMessages.push("Invalid rule: Port 4567 cannot be blocked");break;case 2:(null==d||""===d||1>d||65535<d||isNotaInt(d))&&
a.errorMessages.push("Start Destination Port: Please enter a numeric value between 1 and 65535."),(null==e||""===e||1>e||65535<e||isNotaInt(e))&&a.errorMessages.push("End Destination Port: Please enter a numeric value between 1 and 65535."),0==a.errorMessages.length&&d>=e&&a.errorMessages.push("Destination Ports: The From value must be lower than the To value in a range of ports (from="+d+", to="+e+").")}break;case 3:if(10==a.protocols.icmpMessage){var f=a.protocols.icmpType,q=a.protocols.icmpCode;
(null==f||""===f||0>f||255<f||isNotaInt(f))&&a.errorMessages.push("ICMP Type:  Please enter a numeric value between 0 and 255.");(null==q||""===q||0>q||255<q||isNotaInt(q))&&a.errorMessages.push("ICMP Code:  Please enter a numeric value between 0 and 255.")}}0==a.errorMessages.length&&(0==a.protocols.incomingPorts&&(a.protocols.incomingExclude=!1),0==a.protocols.outgoingPorts&&(a.protocols.outgoingExclude=!1),1==a.protocols.protocol||2==a.protocols.protocol)&&(q=f=!1,0==a.protocols.incomingPorts?
f=!0:2==a.protocols.incomingPorts&&!1==a.protocols.incomingExclude&&1==b&&65535==c&&(f=!0),0==a.protocols.outgoingPorts?q=!0:2==a.protocols.outgoingPorts&&!1==a.protocols.outgoingExclude&&1==d&&65535==e&&(q=!0),f&&q&&a.errorMessages.push('Port forwarding rules can not be Any-to-Any (or "source: 1-65535" with "destination: 1-65535").'));0==a.errorMessages.length&&(b=getTempObjectContent(a.protocols),a.errorMessages=compareNewCondAndCurrentList(a.tempPortForward.protocols,b,a.editingProtocol));0<a.errorMessages.length?
(a.showErrorPage=!0,window.scrollTo(0,0)):(a.protocols.protocolNumber=parseInt(a.protocols.protocolNumber),a.protocols.icmpType=parseInt(a.protocols.icmpType),a.protocols.icmpCode=parseInt(a.protocols.icmpCode),applyProtocol(a,a.tempPortForward.protocols),printProtocol(2,a.tempPortForward.protocols,a.printPortsArr))};a.cancelPortForwardRule=function(){cancelService(a);a.tempPortForward={name:"Global Application",description:"",protocols:[]}};a.cancelProtocol=function(){cancelProtocol(a)};a.editPortForwardRule=
function(b){a.tempPortForward.name=a.portForwardRules[b].name;a.tempPortForward.description=a.portForwardRules[b].description;a.tempPortForward.servType=a.portForwardRules[b].servType;a.tempPortForward.protocols=[];for(var c=0;c<a.portForwardRules[b].protocols.length;c++){var d={};copyProtocol(d,a.portForwardRules[b].protocols[c]);a.tempPortForward.protocols.push(d)}a.printPortsArr=[];printProtocol(2,a.tempPortForward.protocols,a.printPortsArr);a.editing=b;a.togglePage(3)};a.editProtocol=function(b){editProtocol(a,
b,a.tempPortForward.protocols)};a.removePortForwardRule=function(b){a.errorMessages=[];for(var d=a.portForwardRules[b].name,e=0;e<a.nat.length;e++)for(var f=0;f<a.nat[e].services.length;f++)if(d===a.nat[e].services[f].name){a.errorMessages.push("Protocol: Protocol is in use.");break}if(0==a.errorMessages.length)for(e=0;e<a.forwards.length;e++)for(f=0;f<a.forwards[e].protocols.length;f++)if(d===a.forwards[e].name){a.errorMessages.push("Protocol: Protocol is in use.");break}0<a.errorMessages.length?
(a.showErrorPage=!0,window.scrollTo(0,0)):c.remove({id:a.portForwardRules[b].id},function(){a.portForwardRules=c.query(function(){a.portForwardRules=sortListItems(a.portForwardRules);a.printProtArr=[];printProtocol(1,a.portForwardRules,a.printProtArr)})})};a.removeProtocol=function(b){removeObject(a.tempPortForward.protocols,b);a.printPortsArr=[];printProtocol(2,a.tempPortForward.protocols,a.printPortsArr)};a.keyPress=function(b){13==b.keyCode&&"/advanced/portforward"==f.path()&&(0!=a.pageToShow||
a.showErrorPage?3!=a.pageToShow||a.showErrorPage?4!=a.pageToShow||a.showErrorPage||a.applyProtocol():a.applyPortForwardRule():f.path("/advanced"))}}]).controller("dnsServerCtrl",["$scope","$route","DnsServer","securityTips","$location",function(a,b,c,d,e){a.tab=b.current.tab;a.section=b.current.section;a.dnsServer=c.query();a.showErrorPage=!1;a.showAdd=!1;a.editing=-1;a.runFocus=!1;a.errorPageOk=function(){a.showErrorPage=!1;window.scrollTo(0,0)};a.validateBeforeSave=function(){a.errorMessages=[];
validateHostname(a.tempDnsServer.hostname)||a.errorMessages.push("Host Name: Name may not contain spaces. Only letters, digits and the special characters dash (-), underscore (_) and dot (.) may be used. These special characters may not appear at the beginning or at the end of a name. The maximum length of a label (text between two dots) is 63.");validateIp(a.tempDnsServer.ipAddress)?"0.0.0.0"===getIpString(a.tempDnsServer.ipAddress)?a.errorMessages.push("IP Address: IP address may not be 0.0.0.0. IP address must consist of 4 fields. Each field's value must be between 0 and 255, without any spaces. At least one of the values must be greater than 0."):
("255.255.255.255"===getIpString(a.tempDnsServer.ipAddress)||224<=Number(a.tempDnsServer.ipAddress[0])&&239>=Number(a.tempDnsServer.ipAddress[0]))&&a.errorMessages.push("Invalid IP address."):a.errorMessages.push("IP Address: IP address must consist of 4 fields. Each field's value must be between 0 and 255, without any spaces.");for(var b=0;b<a.dnsServer.length;b++)a.editing!=b&&(a.tempDnsServer.hostname==a.dnsServer[b].hostname&&a.errorMessages.push("Host Name: This name is already used by another DNS object."),
getIpString(a.tempDnsServer.ipAddress)==a.dnsServer[b].ipAddress&&a.errorMessages.push("IP Address:You have entered an IP address that already exists. Please enter new IP address."));0<a.errorMessages.length?(a.showErrorPage=!0,window.scrollTo(0,0)):hasADot(a.tempDnsServer.hostname)?d.showWarningMsgBeforeSaving("DNS Entry","Hostnames including a period (.) may not resolve correctly.",[""]).then(function(b){"cancel"!=b&&a.save()}):a.save()};a.save=function(){var b={hostname:a.tempDnsServer.hostname,
type:a.tempDnsServer.type};1==b.type&&(b.ipAddress=getIpString(a.tempDnsServer.ipAddress));-1==a.editing?c.save(b,function(){a.dnsServer=c.query()}):c.update({id:a.dnsServer[a.editing].id},b,function(){a.dnsServer=c.query()});a.tempDnsServer={};a.toggleShow()};a.toggleShow=function(){a.showAdd=!a.showAdd};a.addDnsEntry=function(){a.tempDnsServer={hostname:"new-host",type:1,ipAddress:[0,0,0,0]};a.runFocus=!0;a.editing=-1;a.toggleShow()};a.editDnsEntry=function(b){a.editing=b;a.tempDnsServer={hostname:a.dnsServer[a.editing].hostname,
type:a.dnsServer[a.editing].type};1==a.tempDnsServer.type&&(a.tempDnsServer.ipAddress=splitIpArr(a.dnsServer[a.editing].ipAddress));a.showAdd=!a.showAdd};a.removeDnsEntry=function(b){c.remove({id:a.dnsServer[b].id},function(){a.dnsServer=c.query()})};a.keyPress=function(b){13==b.keyCode&&"/advanced/dnsserver"==e.path()&&(a.showAdd?a.showAdd&&!a.showErrorPage&&a.save():e.path("/advanced"))}}]).controller("ipv6Ctrl",["$scope","$route","$location","IPv6",function(a,b,c,d){a.tab=b.current.tab;a.section=
b.current.section;a.ipv6=d.get(function(){a.ipv6.addressStart=a.ipv6.addressStart.toString(16);a.ipv6.addressEnd=a.ipv6.addressEnd.toString(16);for(var b=0;b<a.ipv6.interfaces.length;b++)a.ipv6.interfaces[b].name=unescape(a.ipv6.interfaces[b].name);a.autoDns=1==a.ipv6.mode?a.ipv6.autoDns:0});a.errorPageOk=function(){a.showErrorPage=!1;window.scrollTo(0,0)};a.save=function(){if(a.ipv6.enabled){a.errorMessages=[];a.ipv6.autoDns=a.autoDns;if(2==a.ipv6.mode)if(a.ipv6.staticAddress=a.ipv6.staticAddress.replace(/^\s+|\s+$/g,
""),a.ipv6.staticDefaultGateway=a.ipv6.staticDefaultGateway.replace(/^\s+|\s+$/g,""),a.ipv6.staticPrimaryDns=a.ipv6.staticPrimaryDns.replace(/^\s+|\s+$/g,""),a.ipv6.staticSecondaryDns=a.ipv6.staticSecondaryDns.replace(/^\s+|\s+$/g,""),validateIPv6(a.ipv6.staticAddress)||a.errorMessages.push("IPv6 WAN Address: Invalid IP Address."),(isNaN(a.ipv6.staticPrefixLength)||16>a.ipv6.staticPrefixLength||128<a.ipv6.staticPrefixLength)&&a.errorMessages.push("Prefix Length: Please enter a numeric value between 16 and 128."),
validateIPv6(a.ipv6.staticDefaultGateway)||a.errorMessages.push("IPv6 Default Gateway: Invalid IP Address."),validateIPv6(a.ipv6.staticPrimaryDns)||""!=a.ipv6.staticPrimaryDns&&a.errorMessages.push("Primary DNS Server: Invalid IP Address."),validateIPv6(a.ipv6.staticSecondaryDns)||""!=a.ipv6.staticSecondaryDns&&a.errorMessages.push("Secondary DNS Server: Invalid IP Address."),""!=a.ipv6.staticPrimaryDns&&a.ipv6.staticPrimaryDns==a.ipv6.staticSecondaryDns&&a.errorMessages.push("Secondary DNS Server: Same As DNS1."),
validateIPv6(a.ipv6.lanPrefix)){var b=getPrefixLength(a.ipv6.lanPrefix);16>b||128==b?a.errorMessages.push("LAN Prefix: Invalid LAN prefix."):a.ipv6.lanPrefix=a.ipv6.lanPrefix+"/"+b}else validatePrefixLength(a.ipv6.lanPrefix)||a.errorMessages.push("LAN Prefix: Invalid LAN prefix.");else 1==a.ipv6.mode&&1==a.ipv6.autoDns&&(a.ipv6.staticPrimaryDns=a.ipv6.staticPrimaryDns.replace(/^\s+|\s+$/g,""),a.ipv6.staticSecondaryDns=a.ipv6.staticSecondaryDns.replace(/^\s+|\s+$/g,""),validateIPv6(a.ipv6.staticPrimaryDns)||
a.errorMessages.push("Primary DNS Server: Invalid IP Address."),validateIPv6(a.ipv6.staticSecondaryDns)||a.errorMessages.push("Secondary DNS Server: Invalid IP Address."));if(1==a.ipv6.lanMode)if(""===a.ipv6.addressStart||""===a.ipv6.addressEnd)a.errorMessages.push("LAN IPv6 Address Range: Please enter a numeric value between 2 and ffff");else if(/^[0-9A-F]+$/i.test(a.ipv6.addressStart)&&/^[0-9A-F]+$/i.test(a.ipv6.addressEnd)){var f=parseInt(a.ipv6.addressStart,16),g=parseInt(a.ipv6.addressEnd,16);
2>f||65535<f||2>g||65535<g?a.errorMessages.push("LAN IPv6 Address Range: Please enter a numeric value between 2 and ffff"):f>g&&a.errorMessages.push("LAN IPv6 Address Range: Start addr must be little or equal to end addr.")}else a.errorMessages.push("LAN IPv6 Address Range: Please enter hex digit as LAN range.");(""===a.ipv6.advertisementLifetime||isNaN(a.ipv6.advertisementLifetime)||0>a.ipv6.advertisementLifetime||150<a.ipv6.advertisementLifetime||!checksForDecimalNumber(a.ipv6.advertisementLifetime)||
!checksForWholeNumber(a.ipv6.advertisementLifetime))&&a.errorMessages.push("Router Advertisement Lifetime: Please enter a numeric value between 0 and 150.");0!=a.ipv6.mode&&1==a.ipv6.lanMode&&(""===a.ipv6.addressLifetime||isNaN(a.ipv6.addressLifetime)||3>a.ipv6.addressLifetime||150<a.ipv6.addressLifetime||!checksForDecimalNumber(a.ipv6.addressLifetime)||!checksForWholeNumber(a.ipv6.addressLifetime))&&a.errorMessages.push("IPv6 Address Lifetime: Please enter a numeric value between 3 and 150.");if(0<
a.errorMessages.length)a.showErrorPage=!0,window.scrollTo(0,0);else{a.ipv6.mode=parseInt(a.ipv6.mode);switch(a.ipv6.mode){case 0:a.ipv6.autoDns=2;break;case 1:a.ipv6.autoDns=parseInt(a.autoDns);break;case 2:a.ipv6.autoDns=1}a.ipv6.staticPrefixLength=parseInt(a.ipv6.staticPrefixLength);a.ipv6.lanMode=parseInt(a.ipv6.lanMode);a.ipv6.advertisementLifetime=parseInt(a.ipv6.advertisementLifetime);a.ipv6.addressLifetime=parseInt(a.ipv6.addressLifetime);a.ipv6.addressStart=f;a.ipv6.addressEnd=g;for(b=0;b<
a.ipv6.interfaces.length;b++)a.ipv6.interfaces[b].name=escape(a.ipv6.interfaces[b].name).replace(/\+/g,"%2B");a.ipv6.$update(function(){a.ipv6.addressStart=a.ipv6.addressStart.toString(16);a.ipv6.addressEnd=a.ipv6.addressEnd.toString(16);for(var b=0;b<a.ipv6.interfaces.length;b++)a.ipv6.interfaces[b].name=unescape(a.ipv6.interfaces[b].name);c.path("/advanced")})}}else d.update({enabled:!1},function(){c.path("/advanced")})};a.keyPress=function(b){13==b.keyCode&&"/advanced/ipv6"==c.path()&&a.save()}}]).controller("ArpCtrl",
["$scope","$route","ARPTable","NetworkConnections","$location",function(a,b,c,d,e){a.tab=b.current.tab;a.section=b.current.section;a.interfaces=d.query(function(){for(var b=0;b<a.interfaces.length;b++)a.interfaces[b].name=unescape(a.interfaces[b].name)});a.refresh=function(){a.connections=c.query()};a.keyPress=function(a){13==a.keyCode&&"/advanced/arptable"==e.path()&&e.path("/advanced")};a.refresh()}]).controller("PortConfigCtrl",["$scope","$route","$location","Ethernet",function(a,b,c,d){a.tab=
b.current.tab;a.section=b.current.section;a.runFocus=!1;var e=d.get(function(){a.ports=[];a.ports.push({name:"WAN Port",currentMode:e.wan.currentMode,desiredMode:e.wan.desiredMode,status:e.wan.status});a.ports.push({name:"LAN Port 1",currentMode:e.lan1.currentMode,desiredMode:e.lan1.desiredMode,status:e.lan1.status});a.ports.push({name:"LAN Port 2",currentMode:e.lan2.currentMode,desiredMode:e.lan2.desiredMode,status:e.lan2.status});a.ports.push({name:"LAN Port 3",currentMode:e.lan3.currentMode,desiredMode:e.lan3.desiredMode,
status:e.lan3.status});a.ports.push({name:"LAN Port 4",currentMode:e.lan4.currentMode,desiredMode:e.lan4.desiredMode,status:e.lan4.status})});a.checkSave=function(){systemSettings.warnBeforeChanges?a.isOK=!0:(a.isOK=!1,a.save())};a.cancelOK=function(){a.isOK=!1;c.path("/advanced")};a.save=function(){d.update({wan:{desiredMode:parseInt(a.ports[0].desiredMode)},lan1:{desiredMode:parseInt(a.ports[1].desiredMode)},lan2:{desiredMode:parseInt(a.ports[2].desiredMode)},lan3:{desiredMode:parseInt(a.ports[3].desiredMode)},
lan4:{desiredMode:parseInt(a.ports[4].desiredMode)}},function(){c.path("/advanced")})};a.selectOpt=function(){a.runFocus=!0};a.keyPress=function(b){13==b.keyCode&&"/advanced/portconfig"==c.path()&&(a.isOK?a.save():a.checkSave())}}]).controller("RoutesCtrl",["$scope","$route","$location","Routes","NetworkConnections","SystemSettings",function(a,b,c,d,e,f){a.tab=b.current.tab;a.section=b.current.section;a.runFocus=!1;a.editingRoute=!1;a.showErrorPage=!1;a.routesConnections=[];a.routingObject=d.get(function(){a.routes=
a.routingObject.routes});a.sysSettings=f.get(function(){});a.connections=e.query(function(){for(var b=0,c=0;c<a.connections.length;c++){if(null!=a.connections[c].bridged)for(var d=0;d<a.connections[c].bridged.length;d++)a.connections[c].bridged[d]&&(a.connections[a.connections[c].availableBridges[d]].name=2==a.connections[c].availableBridges[d]?unescape(a.connections[c].name)+" Wireless 802.11 2.4GHz Access Point":3==a.connections[c].availableBridges[d]?unescape(a.connections[c].name)+" Wireless 802.11 5.0GHz Access Point":
unescape(a.connections[c].name+" "+a.connections[a.connections[c].availableBridges[d]].name));a.connections[c].name=unescape(a.connections[c].name);a.connections[c].hasOwnProperty("connectionUsed")||(a.routesConnections[b]=a.connections[c],b++)}});a.errorIGMPPageOk=function(){a.showIGMPwarning=!1;window.scrollTo(0,0);d.update({domainRouting:a.routingObject.domainRouting,igmpRouting:a.routingObject.igmpRouting},function(){c.path("/advanced")})};a.errorIGMPPageNo=function(){a.showIGMPwarning=!1};a.errorPageOk=
function(){a.showErrorPage=!1;window.scrollTo(0,0)};a.addNewRoute=function(){a.runFocus=!0;a.routeIndex=-1;a.routeEditing={type:0,destination:[0,0,0,0],netmask:[255,255,255,255],gateway:[0,0,0,0],metric:0};a.editingRoute=!0};a.save=function(b){b||!0!==a.sysSettings.warnBeforeChanges?d.update({domainRouting:a.routingObject.domainRouting,igmpRouting:a.routingObject.igmpRouting},function(){c.path("/advanced")}):(a.errorMessages=[],a.errorMessages.push("Internet Group Management Protocol (IGMP): You are attempting to disable Internet Group Management Protocol (IGMP). Please note that Internet Group Management Protocol (IGMP) will be disabled for the following connections:"),
a.showIGMPwarning=!0,window.scrollTo(0,0))};a.warningMsgGen=function(){return null!=a.routes&&0<a.routes.length?a.connections[a.routes[0].type].name:a.connections[0].name};a.editRoute=function(b){a.editingRoute=!0;b=a.routes[b];a.routeIndex=b.id;a.routeEditing={type:parseInt(a.translateRouteEnum(b.type,!0)),destination:splitIpArr(b.destination),netmask:splitIpArr(b.netmask),gateway:splitIpArr(b.gateway),metric:b.metric}};a.deleteRoute=function(b){d.remove({id:a.routes[b].id},function(){a.routingObject=
d.get(function(){a.routes=a.routingObject.routes})})};a.cancelRouteEdit=function(){a.editingRoute=!1};a.saveRoute=function(){a.runFocus=!0;a.errorMessages=[];validateIp(a.routeEditing.destination)||a.errorMessages.push("Destination: IP address must consist of 4 fields. Each field's value must be between 0 and 255, without any spaces.");validateIp(a.routeEditing.netmask)||a.errorMessages.push("Netmask: Subnet mask must consist of 4 fields. Each field's value must be between 0 and 255, constructing a contiguous string of binary ones. The default mask value is 255.255.255.0.");
"0.0.0.0"==getIpString(a.routeEditing.netmask)&&a.errorMessages.push("Netmask: Subnet mask may not be 0.0.0.0. Subnet mask must consist of 4 fields. Each field's value must be between 0 and 255, constructing a contiguous string of binary ones. The default mask value is 255.255.255.0.");(null==a.routeEditing.metric||""===a.routeEditing.metric||isNaN(a.routeEditing.metric)||0>a.routeEditing.metric||255<a.routeEditing.metric||isNotaInt(a.routeEditing.metric)||!checksForWholeNumber(a.routeEditing.metric))&&
a.errorMessages.push("Metric: Please enter a numeric value between 0 and 255.");"0.0.0.0"==getIpString(a.routeEditing.gateway)&&a.errorMessages.push("Gateway: IP Address may not be 0.0.0.0.");validateIp(a.routeEditing.gateway)||a.errorMessages.push("Gateway : IP address must consist of 4 fields. Each field's value must be between 0 and 255, without any spaces.");for(var b=!0,c=!1,e=0;4>e;e++){for(var f=7;-1<f;f--){var p=1==(a.routeEditing.netmask[e]>>f&1);if(p&&c){b=!1;break}p||(c=!0)}if(!b)break}b||
a.errorMessages.push("Netmask: Subnet mask must consist of 4 fields. Each field's value must be between 0 and 255, constructing a contiguous string of binary ones. The default mask value is 255.255.255.0.");0<a.errorMessages.length?(a.showErrorPage=!0,window.scrollTo(0,0)):(a.editingRoute=!1,a.routeEditing.type=parseInt(a.translateRouteEnum(a.routeEditing.type,!1)),a.routeEditing.destination=getIpString(a.routeEditing.destination),a.routeEditing.netmask=getIpString(a.routeEditing.netmask),a.routeEditing.gateway=
getIpString(a.routeEditing.gateway),a.routeEditing.metric=parseInt(a.routeEditing.metric),-1!=a.routeIndex?d.update({id:a.routeIndex},a.routeEditing,function(){a.routingObject=d.get(function(){a.routes=a.routingObject.routes})}):d.save(a.routeEditing,function(){a.routingObject=d.get(function(){a.routes=a.routingObject.routes})}))};a.translateRouteEnum=function(a,b){if(b)switch(parseInt(a)){case 0:return 0;case 2:return 1;case 3:return 2}else switch(parseInt(a)){case 0:return 0;case 1:return 2;case 2:return 3}};
a.selectOpt=function(){a.runFocus=!0};a.keyPress=function(b){13==b.keyCode&&"/advanced/routing"==c.path()&&(a.editingRoute||a.showErrorPage||a.showIGMPwarning?a.editingRoute&&!a.showErrorPage&&a.saveRoute():a.save(a.routingObject.igmpRouting))}}]).controller("UPnPCtrl",["$scope","$route","$location","UPnP",function(a,b,c,d){a.tab=b.current.tab;a.section=b.current.section;a.upnpSettings=d.get();a.save=function(){a.upnpSettings.$update(function(){c.path("/advanced")})}}]).controller("dynamicDnsCtrl",
["$scope","$route","$location","DynamicDNS","DynamicDNSProvider","$window","securityTips","SystemSettings",function(a,b,c,d,e,f,g,h){a.sysSettings=h.get();a.pageToShow=0;a.toggleWarningPage=function(b){a.pageToShow=b;window.scrollTo(0,0)};a.tab=b.current.tab;a.section=b.current.section;a.showErrorPage=!1;a.showAddEdit=!1;a.editing=-1;a.ddnsObjects=[];a.ddnsObject={};a.runFocus=!1;a.getTitle=function(a){return getStatusTitle(a)};a.show=function(a){g.showMessage(getStatusTitle(a.status),a.errorMessage)};
a.init_DDNSObject=function(){a.ddnsObject={hostname:"",provider:"",username:"",password:"",useSsl:!0,status:"",lastUpdate:""}};a.loadProviders=function(){a.ddnsProviders=e.query(function(){0<a.ddnsProviders.length&&(a.ddnsObject.provider=a.ddnsProviders[0])})};a.refresh=function(){a.ddnsObjects=[];a.dynamicDNSObjects=d.query(function(){for(var b,c=0;c<a.dynamicDNSObjects.length;c++)b={},b.id=a.dynamicDNSObjects[c].id,b.hostname=unescape(a.dynamicDNSObjects[c].hostname),b.lastUpdate=unescape(a.dynamicDNSObjects[c].updateTime),
b.status=unescape(a.dynamicDNSObjects[c].status),b.errorMessage=unescape(a.dynamicDNSObjects[c].errorMessage),b.hyperLink=0===a.dynamicDNSObjects[c].status||1===a.dynamicDNSObjects[c].status?!1:!0,void 0==b&&b.hostname==b||a.ddnsObjects.push(b)});a.loadProviders()};a.refresh();a.togglePage=function(b){a.showAddEdit=!1};a.errorPageOk=function(){a.showErrorPage=!1;window.scrollTo(0,0)};a.addDDNS=function(){a.loadProviders();a.showErrorPage=!1;a.showAddEdit=!0;a.editing=-1;a.init_DDNSObject();a.runFocus=
!0};a.openSub=function(){""!=a.ddnsObject.provider&&(a.DnsLink=a.ddnsObject.provider.url,f.open(a.DnsLink))};a.editDDNSObj=function(b){a.showAddEdit=!0;a.showErrorPage=!1;a.editing=b;a.ddnsObject=a.dynamicDNSObjects[b];for(b=0;b<a.ddnsProviders.length;b++)if(a.ddnsProviders[b].name==a.ddnsObject.provider){a.ddnsObject.provider=a.ddnsProviders[b];break}};a.saveDynamicDnsObj=function(){a.errorMessages=[];if(0!==a.ddnsObject.hostname.length){for(var b=0;b<a.dynamicDNSObjects.length;b++)if(a.editing!=
b&&a.dynamicDNSObjects[b].hostname===a.ddnsObject.hostname){a.errorMessages.push("Duplicate Hostname: An identical hostname already exists in the system.");break}validateLocalDomains(a.ddnsObject.hostname)||a.errorMessages.push("Host Name: Name may not contain spaces. Only letters, digits and the special characters dash (-), underscore (_) and dot (.) may be used. These special characters may not appear at the beginning or at the end of a name. The maximum length of a label (text between two dots) is 63.")}else a.errorMessages.push("Host Name: Name may not contain spaces. Only letters, digits and the special characters dash (-), underscore (_) and dot (.) may be used. These special characters may not appear at the beginning or at the end of a name. The maximum length of a label (text between two dots) is 63.");
...
d.replace()};a.keyPress=function(b){13==b.keyCode&&"/firewall"==d.path()&&a.save()}}]).controller("AccessControlCtrl",["$scope","$route","AccessControl","NetworkObjects","DnsServer","PortForwardRules","Schedules","Devices","$routeParams","$location","DateTimeSettings","General","SystemSettings",function(a,b,c,d,e,f,g,h,k,m,p,q,z){a.tab=b.current.tab;a.section=b.current.section;var w=q.get(function(){a.showAllow=2==w.ipv4SecurityLevel});a.showAdd=!1;a.isSame=!1;a.pageToShow=0;a.netObj=-2;a.prot=-2;
a.sched=-2;a.showNetObj=[];a.runFocus=!1;a.sysSettings=z.get();a.initVars=function(){init_networkObjs(a);init_services(a);init_schedules(a);a.networkObjects=[];a.services=[];a.editing=-1};a.errorPageOk=function(){0!=a.pageToShow||a.showAdd||a.refresh();a.isSame=!1;a.showErrorPage=!1;window.scrollTo(0,0)};a.cancel=function(){window.scrollTo(0,0);null!=k.fromnetwork?m.url("/network"):m.url("/firewall")};a.setPrintArr=function(b){a.printAddArr=[];a.printProtArr=[];a.netObjShowArr=[];switch(b){case 0:printAccessControlAddresses(a.accessControl,
...
return b};a.getDeviceIp=function(b,c){for(var d=0;d<a.devices.length;d++)if(b==a.devices[d].name&&c===a.devices[d].ipAddress)return a.devices[d].ipAddress;return b};a.netObjsIPArr=[];a.showNetObj=[];a.netObjsArr=[];a.refresh=function(){a.active=[];a.dateTimeSettings=p.get(function(){a.localTimeMoment=findTimeNow(a.dateTimeSettings);a.devices=h.query(function(){a.devices=filterOutGuest(a.devices);a.accessControl=c.query(function(){a.netObjs=d.query(function(){a.netObjsIPArr=[];a.showNetObj=[];a.netObjsArr=
...
a.errorMessages.push("Invalid rule: Port 4567 cannot be blocked");break;case 2:(null==d||""===d||1>d||65535<d)&&a.errorMessages.push("Start Destination Port: Please enter a numeric value between 1 and 65535."),(null==e||""===e||1>e||65535<e)&&a.errorMessages.push("End Destination Port: Please enter a numeric value between 1 and 65535."),0==a.errorMessages.length&&d>=e&&a.errorMessages.push("Destination Ports: The From value must be lower than the To value in a range of ports (from="+d+", to="+e+").")}break;
case 3:if(10==a.protocols.icmpMessage){var g=a.protocols.icmpType,f=a.protocols.icmpCode;(null==g||""===g||0>g||255<g)&&a.errorMessages.push("ICMP Type:  Please enter a numeric value between 0 and 255.");(null==f||""===f||0>f||255<f)&&a.errorMessages.push("ICMP Code:  Please enter a numeric value between 0 and 255.")}}a.protocols.protocolNumber=parseInt(a.protocols.protocolNumber);0==a.errorMessages.length&&(0==a.protocols.incomingPorts&&(a.protocols.incomingExclude=!1),0==a.protocols.outgoingPorts&&
(a.protocols.outgoingExclude=!1),1==a.protocols.protocol||2==a.protocols.protocol)&&(f=g=!1,0==a.protocols.incomingPorts?g=!0:2==a.protocols.incomingPorts&&!1==a.protocols.incomingExclude&&1==b&&65535==c&&(g=!0),0==a.protocols.outgoingPorts?f=!0:2==a.protocols.outgoingPorts&&!1==a.protocols.outgoingExclude&&1==d&&65535==e&&(f=!0),g&&f&&a.errorMessages.push('Port forwarding rules can not be Any-to-Any (or "source: 1-65535" with "destination: 1-65535").'));0<a.errorMessages.length?(a.showErrorPage=
!0,window.scrollTo(0,0)):(a.protocols.protocolNumber=parseInt(a.protocols.protocolNumber),a.protocols.icmpType=parseInt(a.protocols.icmpType),a.protocols.icmpCode=parseInt(a.protocols.icmpCode),applyProtocol(a,a.service.protocols))};a.cancelProtocol=function(){cancelProtocol(a)};a.editProtocol=function(b){editProtocol(a,b,a.service.protocols)};a.removeProtocol=function(b){removeObject(a.service.protocols,b);a.setPrintArr(1)};a.scheduleChange=function(){a.runFocus=!0;-2!=a.sched&&-1==a.sched&&a.addSchedule()};
a.addSchedule=function(){init_schedule(a);a.editingSchedule=!1;a.togglePage(5)};a.applyRuleScheduler=function(){a.runFocus=!0;a.errorMessages=[];""===a.schedule.name&&a.errorMessages.push("Description: A string must be specified.");for(var b=0;b<a.schedArr.length;b++)if(b!=a.editingRuleSchedule&&a.schedArr[b].name==a.schedule.name){a.errorMessages.push("Name: This name is already used by another scheduler rule.");break}0==a.schedule.timeSegments.length&&a.errorMessages.push("Rule Schedule: At least one time segment should be defined.");
if(0<a.errorMessages.length)a.showErrorPage=!0,window.scrollTo(0,0);else{var c={};copySchedule(c,a.schedule);for(b=0;b<c.timeSegments.length;b++)for(var d=0;d<c.timeSegments[b].hourRanges.length;d++)c.timeSegments[b].hourRanges[d].startTimeHour=parseInt(c.timeSegments[b].hourRanges[d].startTimeHour),c.timeSegments[b].hourRanges[d].startTimeMinute=parseInt(c.timeSegments[b].hourRanges[d].startTimeMinute),c.timeSegments[b].hourRanges[d].endTimeHour=parseInt(c.timeSegments[b].hourRanges[d].endTimeHour),
c.timeSegments[b].hourRanges[d].endTimeMinute=parseInt(c.timeSegments[b].hourRanges[d].endTimeMinute);a.schedule=c;a.schedule.name=unescape(a.schedule.name);a.togglePage(0);a.printSchedArr=[];printSchedule(1,[a.schedule],a.printSchedArr)}};a.cancelRuleScheduler=function(){cancelRuleScheduler(a);a.editingSchedule||(a.sched=-2)};a.editRuleScheduler=function(){a.editingSchedule=!0;var b={};copySchedule(b,a.schedule);editRuleScheduler(a,[b],0)};a.removeRuleScheduler=function(b){a.sched=-2;a.printSchedArr=
[]};a.applyTimeSegment=function(){a.errorMessages=[];for(var b=!1,c=0;c<a.daysOfTheWeek.length;c++)a.daysOfTheWeek[c]&&(b=!0);b||a.errorMessages.push("Days of Week: At least one day must be selected.");0==a.hourRanges.length&&a.errorMessages.push("Hours Range: At least one hour range must be set.");0<a.errorMessages.length?a.showErrorPage=!0:applyTimeSegment(a);window.scrollTo(0,0)};a.cancelTimeSegment=function(){cancelTimeSegment(a)};a.editTimeSegment=function(b){editTimeSegment(a,b)};a.removeTimeSegment=
function(b){removeObject(a.schedule.timeSegments,b)};a.applyHourRange=function(){a.runFocus=!0;a.errorMessages=[];var b=a.hourRange.startTimeHour,c=a.hourRange.endTimeHour,d=a.hourRange.startTimeMinute,e=a.hourRange.endTimeMinute;(""===b||isNaN(b)||0>b||23<b)&&a.errorMessages.push("Start Time Hour: Please enter a numeric value between 0 and 23.");(""===c||isNaN(c)||0>c||23<c)&&a.errorMessages.push("End Time Hour: Please enter a numeric value between 0 and 23.");(""===d||isNaN(d)||0>d||59<d)&&a.errorMessages.push("Start Time Minutes: Please enter a numeric value between 0 and 59.");
(""===e||isNaN(e)||0>e||59<e)&&a.errorMessages.push("End Time Minutes: Please enter a numeric value between 0 and 59.");0<a.errorMessages.length?(a.showErrorPage=!0,window.scrollTo(0,0)):(a.hourRange.startTimeHour=parseInt(a.hourRange.startTimeHour),a.hourRange.startTimeMinute=parseInt(a.hourRange.startTimeMinute),a.hourRange.endTimeHour=parseInt(a.hourRange.endTimeHour),a.hourRange.endTimeMinute=parseInt(a.hourRange.endTimeMinute),applyHourRange(a))};a.cancelHourRange=function(){cancelHourRange(a)};
a.editHourRange=function(b){editHourRange(a,b)};a.removeHourRange=function(b){removeObject(a.hourRanges,b)};a.formatTimeString=function(a,b){return(10>a?"0"+a:a)+":"+(10>b?"0"+b:b)};a.keyPress=function(b){13==b.keyCode&&"/firewall/accesscontrol"==m.path()&&(a.showAdd&&0==a.pageToShow&&!a.isSame?a.applyMain():8!=a.pageToShow||a.showErrorPage?1!=a.pageToShow&&2!=a.pageToShow||a.showErrorPage?5!=a.pageToShow&&6!=a.pageToShow&&7!=a.pageToShow||a.showErrorPage?3!=a.pageToShow||a.showErrorPage?4!=a.pageToShow||
a.showErrorPage?a.showAdd||0!=a.pageToShow||a.save():a.applyProtocol():a.applyPortForwardRule():6!=a.pageToShow||a.showErrorPage?7!=a.pageToShow||a.showErrorPage?a.showErrorPage||a.applyRuleScheduler():a.applyHourRange():a.applyTimeSegment():2==a.pageToShow?a.applyAddress():a.applyNetworkObj():a.createAccessControl(null))}}]).controller("PortForwardCtrl",["$scope","$route","PortForward","Devices","PortForwardRules","Schedules","DateTimeSettings",function(a,b,c,d,e,f,g){a.tab=b.current.tab;a.section=
b.current.section;a.sourcePortType=0;a.destPortType=0;a.showAdvanced=!1;a.ipSelect=-2;a.appToForward=-2;a.showErrorPage=!1;a.portsUsed=[];a.hasPrevValue=null;a.errorPageOk=function(){a.showErrorPage=!1;window.scrollTo(0,0)};a.pf={specifiedIp:[0,0,0,0],servicePortType:0,servicePort:0,schedule:"Always",protocol:{protocol:1,incomingPorts:0,incomingPortStart:65535,incomingPortEnd:0,outgoingPorts:0,outgoingPortStart:65535,outgoingPortEnd:0}};a.removeBox=[];for(var h=0;h<a.numItems;h++)a.removeBox.push(!1);
a.isScheduleActive=isScheduleActive;a.appForward=function(){if(0<=a.appToForward){a.portsUsed=[];for(var b=null,c=0;c<a.pfRules.length;c++)if(a.pfRules[c].id==a.appToForward){b=a.pfRules[c];break}null!=b&&printProtocol(6,b.protocols,a.portsUsed)}};a.save=function(){for(var b=0,d=0,e=0;e<a.portForward.length;e++)a.active[e]!=a.portForward[e].enabled&&(b++,c.update({id:a.portForward[e].id},{enabled:a.active[e]},function(){d++;d==b&&a.refresh()}))};a.toggleShow=function(){a.showAdvanced=!a.showAdvanced};
a.contains=function(a,b){for(var c=0;c<a.length;c++)if(a[c]===b)return!0;return!1};a.refresh=function(){a.portForward=c.query(function(){a.numItems=a.portForward.length;a.printPortsArr=[];printProtocol(1,a.portForward,a.printPortsArr);a.active=[];for(var b=0;b<a.portForward.length;b++)if(a.active.push(a.portForward[b].enabled),/^PortForward\_.*$/.test(a.portForward[b].name)){a.portForward[b].displayName="Destination Ports ";for(var c=[],h=0;h<a.portForward[b].protocols.length;h++){var q=a.portForward[b].protocols[h];
if(1==q.outgoingPorts){if(a.contains(c,q.outgoingPortStart))continue}else if(2==q.outgoingPorts&&a.contains(c,q.outgoingPortStart+"-"+q.outgoingPortEnd))continue;0<h&&(a.portForward[b].displayName+=",");0==q.outgoingPorts?a.portForward[b].displayName+="Any":1==q.outgoingPorts?(a.portForward[b].displayName+=q.outgoingPortStart,c.push(q.outgoingPortStart)):(a.portForward[b].displayName+=q.outgoingPortStart+"-"+q.outgoingPortEnd,c.push(q.outgoingPortStart+"-"+q.outgoingPortEnd))}}else a.portForward[b].displayName=
unescape(a.portForward[b].name);a.dateTimeSettings=g.get(function(){a.localTimeMoment=findTimeNow(a.dateTimeSettings);a.schedules=f.query(function(){for(var b=0;b<a.portForward.length;b++){a.portForward[b].schedActive="Active";for(var c=0;c<a.schedules.length;c++)if(a.portForward[b].schedule==a.schedules[c].name){a.portForward[b].schedActive=isScheduleActive(a.schedules[c],a.localTimeMoment);break}}for(c=0;c<a.schedules.length;c++)a.schedules[c].name=unescape(a.schedules[c].name)})});null==a.hasPrevValue?
a.devices=d.query(function(){for(var b=0;b<a.devices.length;b++)a.devices[b].displayName=a.devices[b].ipAddress!=a.devices[b].name?a.devices[b].ipAddress+" - "+a.devices[b].name:a.devices[b].name;for(b=0;b<a.portForward.length;b++){for(var c=0;c<a.devices.length;c++)if(a.portForward[b].deviceIp==a.devices[c].ipAddress&&a.devices[c].name!==a.devices[c].ipAddress){a.portForward[b].hostname=a.devices[c].name;a.portForward[b].showIp=!0;break}if(null==a.portForward[b].hostname||""===a.portForward[b].hosname)a.portForward[b].showIp=
!1,a.portForward[b].hostname=a.portForward[b].deviceIp}a.devices=filterOutStb(a.devices);a.devices=filterOutGuest(a.devices);a.pfRules=e.query(function(){a.pfRules=sortListItems(a.pfRules);a.pfRulesCopy=copyArrayFromSource(a.pfRules)})}):!0==a.hasPrevValue&&(a.hasPrevValue=null,a.devices1=d.query(function(){for(var b=0;b<a.devices1.length;b++)a.devices1[b].displayName=a.devices1[b].ipAddress!=a.devices1[b].name?a.devices1[b].ipAddress+" - "+a.devices1[b].name:a.devices1[b].name;for(b=0;b<a.portForward.length;b++){for(var c=
0;c<a.devices1.length;c++)if(a.portForward[b].deviceIp==a.devices1[c].ipAddress&&a.devices1[c].name!==a.devices1[c].ipAddress){a.portForward[b].hostname=a.devices1[c].name;a.portForward[b].showIp=!0;break}if(null==a.portForward[b].hostname||""===a.portForward[b].hosname)a.portForward[b].showIp=!1,a.portForward[b].hostname=a.portForward[b].deviceIp}}))})};a.refresh();a.resolveNow=function(){};a.reset=function(){a.ipSelect=-2;a.appToForward=-2;a.showAdvanced=!1;a.pf={specifiedIp:[0,0,0,0],servicePortType:0,
servicePort:0,schedule:"Always",protocol:{protocol:1,incomingPorts:0,incomingPortStart:65535,incomingPortEnd:0,outgoingPorts:0,outgoingPortStart:65535,outgoingPortEnd:0}}};a.add=function(){a.errorMessages=[];-2==a.ipSelect?a.errorMessages.push("Error: Please select the IP Address you want to forward to, or select an already connected item from the list"):-1==a.ipSelect&&(validateIp(a.pf.specifiedIp)&&"0.0.0.0"!=getIpString(a.pf.specifiedIp)&&"255.255.255.255"!=getIpString(a.pf.specifiedIp)||a.errorMessages.push("Error: The IP address specified is invalid"));
a.pf.protocol.protocol=parseInt(a.pf.protocol.protocol);var b=[],d=[],e=[],g=0,f=0,h=65535,r=0,y=0,v=65535;if(-2==a.appToForward)a.errorMessages.push("Error: Please select a pre-configured application or choose the option Custom ports to create a new appliction for port forwarding");else if(-1==a.appToForward)if(!a.showAdvanced&&0==a.errorMessages.length)for(var s=(""+a.pf.protocol.outgoingPortStart).split(","),u=0;u<s.length;u++){var n=s[u].split("-");if(2<n.length)a.errorMessages.push("Error: The input of port has illegal characters. Source port should be 111 or 111-222; Destination port should be 111, or 111-222, or 111,222-333 and so on.");
else for(var l=0;l<n.length;l++)""===n[l]||isNaN(n[l])||checkForWholeNumbers(n[l])||!checksForWholeNumber(n[l])?a.errorMessages.push("Error: The input of port has illegal characters. Source port should be 111 or 111-222; Destination port should be 111, or 111-222, or 111,222-333 and so on."):(1>n[l]||65535<n[l])&&a.errorMessages.push("Error: Port range should be between [1-65535]");1==n.length?(r=1,y=v=parseInt(n[0])):2==n.length&&(r=2,y=parseInt(n[0]),v=parseInt(n[1]));b.push(r);d.push(y);e.push(v)}else if(0==
a.errorMessages.length){if(1==a.sourcePortType){n=(""+a.pf.incomingPortStart).split("-");if(2<n.length)a.errorMessages.push("Error: The input of port has illegal characters. Source port should be 111 or 111-222; Destination port should be 111, or 111-222, or 111,222-333 and so on.");else for(l=0;l<n.length;l++)""===n[l]||isNaN(n[l])||checkForWholeNumbers(n[l])||!checksForWholeNumber(n[l])?a.errorMessages.push("Error: The input of port has illegal characters. Source port should be 111 or 111-222; Destination port should be 111, or 111-222, or 111,222-333 and so on."):
(1>n[l]||65535<n[l])&&a.errorMessages.push("Error: Port range should be between [1-65535]");1==n.length?(g=1,f=h=parseInt(n[0])):2==n.length&&(g=2,f=parseInt(n[0]),h=parseInt(n[1]))}if(1==a.destPortType&&0==a.errorMessages.length)for(s=(""+a.pf.outgoingPortStart).split(","),u=0;u<s.length;u++){n=s[u].split("-");if(2<n.length)a.errorMessages.push("Error: The input of port has illegal characters. Destination port should be 111 or 111-222; Destination port should be 111, or 111-222, or 111,222-333 and so on.");
else for(l=0;l<n.length;l++)""===n[l]||isNaN(n[l])||checkForWholeNumbers(n[l])||!checksForWholeNumber(n[l])?a.errorMessages.push("Error: The input of port has illegal characters. Destination port should be 111 or 111-222; Destination port should be 111, or 111-222, or 111,222-333 and so on."):(1>n[l]||65535<n[l])&&a.errorMessages.push("Error: Port range should be between [1-65535]");1==n.length?(r=1,y=v=parseInt(n[0])):2==n.length&&(r=2,y=parseInt(n[0]),v=parseInt(n[1]));b.push(r);d.push(y);e.push(v)}}a.showAdvanced&&
1==a.pf.servicePortType&&0==a.errorMessages.length&&(""===a.pf.servicePort||isNaN(a.pf.servicePort)||0>a.pf.servicePort||65535<a.pf.servicePort||!checksForWholeNumber(a.pf.servicePort))&&a.errorMessages.push("Forward to Port: Please enter a numeric value between 0 and 65535.");0==b.length&&(b.push(r),d.push(y),e.push(v));0==a.errorMessages.length&&-1==a.appToForward&&(r=s=!1,!0==a.showAdvanced?0==a.sourcePortType?s=!0:1==a.sourcePortType&&(l=(""+a.pf.incomingPortStart).split("-"),2==l.length&&1==
l[0]&&65535==l[1]&&(s=!0)):s=!0,!0==a.showAdvanced?0==a.destPortType?r=!0:1==a.destPortType&&(l=(""+a.pf.outgoingPortStart).split("-"),2==l.length&&1==l[0]&&65535==l[1]&&(r=!0)):(l=(""+a.pf.protocol.outgoingPortStart).split("-"),2==l.length&&1==l[0]&&65535==l[1]&&(r=!0)),s&&r&&a.errorMessages.push('Port forwarding rules can not be Any-to-Any (or "source: 1-65535" with "destination: 1-65535").'));if(0==a.errorMessages.length&&0<=a.appToForward){u=null;for(l=0;l<a.pfRules.length;l++)if(a.pfRules[l].id==
a.appToForward){u=a.pfRules[l];break}if(null==u)return;for(l=0;l<u.protocols.length;l++)if(!u.protocols[l].protocolExclude&&!u.protocols[l].outgoingExclude){var r=u.protocols[l].outgoingPortStart,v=u.protocols[l].outgoingPortEnd,y=u.name,x=r,s=u.protocols[l].protocol,B=!1;void 0!==v&&r!=v&&(x+="-"+v);void 0===v&&(v=r);for(x=0;x<a.portForward.length;x++)for(n=0;n<a.portForward[x].protocols.length;n++){var t=a.portForward[x].protocols[n];if(7!=s||!(1>t.protocol||2<t.protocol))if(7==s||s==t.protocol){void 0!=
r||s!=t.protocol||0!=s&&3!=s&&4!=s&&5!=s&&6!=s||(B=!0);var A="";if(0==t.outgoingPorts)A="Any";else if(1==t.outgoingPorts&&r<=t.outgoingPortStart&&v>=t.outgoingPortStart)A=t.outgoingPortStart;else if(2==t.outgoingPorts&&r<=t.outgoingPortStart&&v>=t.outgoingPortStart||r<=t.outgoingPortEnd&&v>=t.outgoingPortEnd||r>=t.outgoingPortStart&&v<=t.outgoingPortEnd)A=t.outgoingPortStart+" -> "+t.outgoingPortEnd;if(""!=A||B){t="Conflicting Port Forwarding Rules: The "+y+" service conflict with an existing port forwarding rule offering "+
y+" service.";A=!0;if(0<a.errorMessages.length)for(l=0;l<a.errorMessages.length;l++)if(a.errorMessages[l]==t){A=!1;break}A&&a.errorMessages.push(t)}}}}}if(0==a.errorMessages.length&&-1==a.appToForward)for(u=0;u<b.length;u++)if(r=b[u],y=d[u],v=e[u],s=parseInt(a.pf.protocol.protocol),1!=s&&7!=s||4567!=y||4567!=v||a.errorMessages.push("Invalid rule: Port 4567 cannot be blocked"),0==r)for(l=0;l<a.portForward.length;l++)for(n=0;n<a.portForward[l].protocols.length;n++){if(t=a.portForward[l].protocols[n],
7!=s||!(1>t.protocol||2<t.protocol))if(7==s||s==t.protocol)A="",A=0==t.outgoingPorts?"Any":1==t.outgoingPorts?t.outgoingPortStart:t.outgoingPortStart+" -> "+t.outgoingPortEnd,a.errorMessages.push("Conflicting Port Forwarding Rules: The Destination Ports Any service conflict with an existing port forwarding rule offering Destination Ports "+A+" service.")}else for(x=r=y,s=a.pf.protocol.protocol,r!=v&&(x+="-"+v),l=0;l<a.portForward.length;l++)for(n=0;n<a.portForward[l].protocols.length;n++)if(t=a.portForward[l].protocols[n],
y=a.portForward[l].name,7!=s||!(1>t.protocol||2<t.protocol))if(7==s||s==t.protocol){A="";if(0==t.outgoingPorts)A="Any";else if(1==t.outgoingPorts&&r<=t.outgoingPortStart&&v>=t.outgoingPortStart)A=t.outgoingPortStart;else if(2==t.outgoingPorts&&r<=t.outgoingPortStart&&v>=t.outgoingPortStart||r<=t.outgoingPortEnd&&v>=t.outgoingPortEnd||r>=t.outgoingPortStart&&v<=t.outgoingPortEnd)A=t.outgoingPortStart+" -> "+t.outgoingPortEnd;""!=A&&(0<=y.indexOf("_")?a.errorMessages.push("Conflicting Port Forwarding Rules: The Destination Ports "+
x+" service conflict with an existing port forwarding rule offering  Destination Ports "+A+" service."):a.errorMessages.push("Conflicting Port Forwarding Rules: The Destination Ports "+x+" service conflict with an existing port forwarding rule offering "+y+" service."))}if(0<a.errorMessages.length)a.showErrorPage=!0,window.scrollTo(0,0);else{y=v=u=r=!1;l=parseInt(a.pf.protocol.protocol);s={enabled:!0};-1==a.ipSelect?s.deviceIp=getIpString(a.pf.specifiedIp):0<=a.ipSelect&&(s.deviceIp=a.devices[a.ipSelect].ipAddress);
if(-1==a.appToForward)if(s.name="PortForward_"+(new Date).getTime(),s.protocols=[],7==l)for(l=0;l<b.length;l++)y=v=u=r=!1,n={protocol:1},n.incomingPorts=g,n.incomingPortStart=f,!1==v&&(n.incomingPortEnd=h),n.incomingExclude=r,n.outgoingPorts=b[l],n.outgoingPortStart=d[l],!1==y&&(n.outgoingPortEnd=e[l]),n.outgoingExclude=u,s.protocols.push(n),n={protocol:2},n.incomingPorts=g,n.incomingPortStart=f,!1==v&&(n.incomingPortEnd=h),n.incomingExclude=r,n.outgoingPorts=b[l],n.outgoingPortStart=d[l],!1==y&&
(n.outgoingPortEnd=e[l]),n.outgoingExclude=u,s.protocols.push(n);else for(l=0;l<b.length;l++)y=v=u=r=!1,n={},n.protocol=a.pf.protocol.protocol,n.incomingPorts=g,n.incomingPortStart=f,!1==v&&(n.incomingPortEnd=h),n.incomingExclude=r,n.outgoingPorts=b[l],n.outgoingPortStart=d[l],!1==y&&(n.outgoingPortEnd=e[l]),n.outgoingExclude=u,s.protocols.push(n);else if(0<=a.appToForward){b=null;for(l=0;l<a.pfRules.length;l++)if(a.pfRules[l].id==a.appToForward){b=a.pfRules[l];break}null!=b&&(s.protocols=b.protocols,
s.name=escape(b.name).replace(/\+/g,"%2B"))}a.showAdvanced?(s.schedule=a.pf.schedule,s.schedule=escape(s.schedule).replace(/\+/g,"%2B"),s.servicePort=0==a.pf.servicePortType?0:parseInt(a.pf.servicePort)):(s.servicePort=0,s.schedule="Always");c.save(s,function(){a.refresh()});a.reset()}};a.remove=function(){for(var b=[],c=0;c<a.portForward.length;c++)a.removeBox[c]&&b.push(a.portForward[c].id);if(0<b.length){for(c=0;c<b.length;c++)b[c]-=c;a.removeListItems(b)}a.removeBox=[];for(c=0;c<a.portForward.length;c++)a.removeBox.push(!1)};
a.removeListItems=function(b){0<b.length?c.remove({id:b[h]},function(){b.splice(0,1);a.removeListItems(b)}):(a.hasPrevValue=!0,a.refresh())};a.filterFn=function(a){return/^PortForward\_.*$/.test(a.name)?!1:!0}}]).controller("PortTriggerCtrl",["$scope","$route","PortTrigger","$location",function(a,b,c,d){a.tab=b.current.tab;a.section=b.current.section;a.pageToShow=0;a.protNameArr="Other TCP UDP ICMP GRE ESP AH".split(" ");a.prot=0;a.incoming=!1;a.runFocus=!1;a.initVars=function(){init_services(a);
a.tempTrigger={enabled:!0,name:"Application",outgoing:[],incoming:[]};a.editing=-1};a.errorPageOk=function(){a.showErrorPage=!1;window.scrollTo(0,0)};a.setPrintArr=function(b){a.printOutArr=[];a.printInArr=[];switch(b){case 3:printProtocol(3,a.portTrigger,a.printOutArr,a.printInArr);break;case 2:printProtocol(2,a.tempTrigger.outgoing,a.printOutArr),printProtocol(2,a.tempTrigger.incoming,a.printInArr)}};a.refresh=function(){a.portTrigger=c.query(function(){a.active=[];for(var b=0;b<a.portTrigger.length;b++)a.active.push(a.portTrigger[b].enabled);
a.setPrintArr(3);a.initVars()})};a.refresh();a.save=function(){for(var b=0,d=0,g=0;g<a.portTrigger.length;g++)a.active[g]!=a.portTrigger[g].active&&(b++,c.update({id:a.portTrigger[g].id},{enabled:a.active[g]},function(){d++;d==b&&a.refresh()}))};a.togglePage=function(b){a.pageToShow=b;3==b?a.setPrintArr(2):4==b?a.incoming=!1:5==b&&(a.incoming=!0);a.prot=0;window.scrollTo(0,0)};a.applyTrigger=function(){a.errorMessages=[];var b={outgoing:[],incoming:[]};copyTrigger(b,a.tempTrigger);""==b.name?a.errorMessages.push("Service Name: A name must be specified."):
...
break;case 2:(null==d||""===d||1>d||65535<d)&&a.errorMessages.push("Start Destination Port: Please enter a numeric value between 1 and 65535."),(null==h||""===h||1>h||65535<h)&&a.errorMessages.push("End Destination Port: Please enter a numeric value between 1 and 65535."),0==a.errorMessages.length&&d>=h&&a.errorMessages.push("Destination Ports: The From value must be lower than the To value in a range of ports (from="+d+", to="+h+").")}break;case 3:if(10==a.protocols.icmpMessage){var k=a.protocols.icmpType,
m=a.protocols.icmpCode;(null==k||""===k||0>k||255<k)&&a.errorMessages.push("ICMP Type:  Please enter a numeric value between 0 and 255.");(null==m||""===m||0>m||255<m)&&a.errorMessages.push("ICMP Code:  Please enter a numeric value between 0 and 255.")}}a.protocols.protocolNumber=parseInt(a.protocols.protocolNumber);0==a.errorMessages.length&&(0==a.protocols.incomingPorts&&(a.protocols.incomingExclude=!1),0==a.protocols.outgoingPorts&&(a.protocols.outgoingExclude=!1),1==a.protocols.protocol||2==a.protocols.protocol)&&
(m=k=!1,0==a.protocols.incomingPorts?k=!0:2==a.protocols.incomingPorts&&!1==a.protocols.incomingExclude&&1==b&&65535==c&&(k=!0),0==a.protocols.outgoingPorts?m=!0:2==a.protocols.outgoingPorts&&!1==a.protocols.outgoingExclude&&1==d&&65535==h&&(m=!0),k&&m&&a.errorMessages.push('Port forwarding rules can not be Any-to-Any (or "source: 1-65535" with "destination: 1-65535").'));var p={outgoing:[],incoming:[]};copyTrigger(p,a.tempTrigger);var q=m=k="",z=!1,b=!1,w,r,c=!1;r=w="";var y=!1,v=!1,s,u,n=!1;if(1==
a.protocols.protocol||2==a.protocols.protocol){1==a.protocols.protocol?k="TCP":2==a.protocols.protocol&&(k="UDP");for(var c=a.protocols.incomingPortEnd,b=a.protocols.incomingPortStart,h=a.protocols.outgoingPortEnd,d=a.protocols.outgoingPortStart,y=2==a.protocols.incomingPorts?!0:!1,z=2==a.protocols.outgoingPorts?!0:!1,p=!1==a.incoming?p.outgoing:p.incoming,l=0;l<p.length;l++)if(a.protocols.protocol==p[l].protocol&&a.tempIndex!=l&&(s=p[l].incomingPortEnd,u=p[l].incomingPortStart,v=null==s||""===s?
!1:!0,n=!1,y?v?Number(b)>=Number(u)&&Number(b)<=Number(s)?(w=u+"-"+s,r=b+"-"+c,n=!0):Number(c)>=Number(u)&&Number(c)<=Number(s)?(w=u+"-"+s,r=b+"-"+c,n=!0):Number(u)>=Number(b)&&Number(u)<=Number(c)?(w=u+"-"+s,r=b+"-"+c,n=!0):Number(s)>=Number(b)&&Number(s)<=Number(c)&&(w=u+"-"+s,r=b+"-"+c,n=!0):Number(u)>=Number(b)&&Number(u)<=Number(c)&&(w=u,r=b+"-"+c,n=!0):v?Number(b)>=Number(u)&&Number(b)<=Number(s)&&(w=u+"-"+s,r=d,n=!0):Number(b)==Number(u)&&(w=u,r=b,n=!0),n)){a.errorMessages.push("Overlapping Port Ranges: The "+
k+" source ports ("+r+") is overlapping with already defined "+k+" source ports ("+w+").");break}for(l=0;l<p.length;l++)if(a.protocols.protocol==p[l].protocol&&a.tempIndex!=l&&(w=p[l].outgoingPortEnd,r=p[l].outgoingPortStart,b=null==w||""===w?!1:!0,c=!1,z?b?Number(d)>=Number(r)&&Number(d)<=Number(w)?(m=r+"-"+w,q=d+"-"+h,c=!0):Number(h)>=Number(r)&&Number(h)<=Number(w)?(m=r+"-"+w,q=d+"-"+h,c=!0):Number(r)>=Number(d)&&Number(r)<=Number(h)?(m=r+"-"+w,q=d+"-"+h,c=!0):Number(w)>=Number(d)&&Number(w)<=
Number(h)&&(m=r+"-"+w,q=d+"-"+h,c=!0):Number(r)>=Number(d)&&Number(r)<=Number(h)&&(m=r,q=d+"-"+h,c=!0):b?Number(d)>=Number(r)&&Number(d)<=Number(w)&&(m=r+"-"+w,q=d,c=!0):Number(d)==Number(r)&&(m=r,q=d,c=!0),c)){a.errorMessages.push("Overlapping Port Ranges: The "+k+" destination ports ("+q+") is overlapping with already defined "+k+" destination ports ("+m+").");break}}0<a.errorMessages.length?(a.showErrorPage=!0,window.scrollTo(0,0)):(a.protocols.protocolNumber=parseInt(a.protocols.protocolNumber),
a.protocols.icmpType=parseInt(a.protocols.icmpType),a.protocols.icmpCode=parseInt(a.protocols.icmpCode),a.incoming?applyProtocol(a,a.tempTrigger.incoming):applyProtocol(a,a.tempTrigger.outgoing))};a.cancelTrigger=function(){a.refresh();a.togglePage(0)};a.cancelProtocol=function(){cancelProtocol(a)};a.editTrigger=function(b){a.editing=b;a.tempTrigger={enabled:a.portTrigger[b].enabled,name:a.portTrigger[b].name,outgoing:[],incoming:[]};copyTrigger(a.tempTrigger,a.portTrigger[b]);a.togglePage(3)};a.editOutgoingProtocol=
function(b){editProtocol(a,b,a.tempTrigger.outgoing);a.incoming=!1;a.tempIndex=b};a.editIncomingProtocol=function(b){editProtocol(a,b,a.tempTrigger.incoming,5);a.incoming=!0;a.tempIndex=b};a.removeTrigger=function(b){c.remove({id:a.portTrigger[b].id},function(){a.refresh()})};a.removeOutgoingProtocol=function(b){removeObject(a.tempTrigger.outgoing,b)};a.removeIncomingProtocol=function(b){removeObject(a.tempTrigger.incoming,b)};a.keyPress=function(b){13==b.keyCode&&"/firewall/porttrigger"==d.path()&&
(4!=a.pageToShow&&5!=a.pageToShow||a.showErrorPage?3!=a.pageToShow||a.showErrorPage?0==a.pageToShow&&a.save():a.applyTrigger():a.applyProtocol())}}]).controller("RemoteAdminCtrl",["$scope","$route","$routeParams","$location","RemoteAdmin","SystemSettings",function(a,b,c,d,e,f){a.tab=b.current.tab;a.section=b.current.section;a.sysSettings=f.get();a.remAdmin=e.get();a.fromString="?from=fw";a.prev="";a.oldFrom="";a.back="/firewall";if(null!=c.from){a.fromString="?from="+c.from+"-fw";a.back="/advanced/settings";
b=c.from.split("-");c="";for(e=0;e<b.length-1;e++)0!=e&&(c+="-"),c+=b[e];""!==c&&(a.prev="?from="+c)}else a.back="/firewall";a.save=function(){a.remAdmin.$update(function(){""==a.oldFrom?location.reload():d.path(a.back).search({from:a.oldFrom})})};a.keyPress=function(b){13==b.keyCode&&"/firewall/remoteadmin"==d.path()&&a.save()}}]).controller("StaticNATCtrl",["$scope","$route","StaticNAT","Devices","PortForwardRules","$location",function(a,b,c,d,e,f){a.tab=b.current.tab;a.section=b.current.section;
a.showErrorPage=!1;a.runFocus=!1;a.errorPageOk=function(){a.showErrorPage=!1;window.scrollTo(0,0)};a.pageToShow=0;a.prot=-2;a.initVars=function(){init_services(a);a.statNat={localHost:-1,host:"192.168.1.0",publicIp:splitIpArr("0.0.0.0"),portForwarding:!1,services:[]};a.editing=-1;a.checkEdit=-1};a.initVars();a.setPrintArr=function(b){a.printProtArr=[];switch(b){case 1:printProtocol(1,a.statNat.services,a.printProtArr)}};a.devices=d.query(function(){a.devices=filterOutStb(a.devices);a.devices=filterOutGuest(a.devices)});
a.save=function(){for(var b=0,d=0,e=0;e<a.staticNAT.length;e++)a.enabled[e]!=a.staticNAT[e].active&&(b++,c.update({id:a.staticNAT[e].id},{active:a.enabled[e]},function(){d++;d==b&&a.refresh()}))};a.togglePage=function(b){a.runFocus=!0;a.pageName="Add";a.pageToShow=b;1==b&&(a.prot=-2,a.setPrintArr(1));window.scrollTo(0,0)};a.saveRule=function(){a.errorMessages=[];var b=a.statNat.host.split(".");-1==a.statNat.localHost&&(validateIpDomain(a.statNat.host)&&validateIp(b)||a.errorMessages.push("Local Host: The value must be a valid IP address."));
if(validateIp(a.statNat.publicIp))if(240<=a.statNat.publicIp[0])a.errorMessages.push("Public IP Address: IP Address must be within the range of 0.0.0.0 to 239.255.255.255.");else{var d=getIpString(a.statNat.publicIp);if("0.0.0.0"===d)a.errorMessages.push("NAT/NAPT configuration: IP conflicts between this rule and previous NAT/NAPT rules");else for(b=0;b<a.staticNAT.length;b++)if(a.checkEdit!=b&&a.staticNAT[b].publicIp===d){a.errorMessages.push("NAT/NAPT configuration: IP conflicts between this rule and previous NAT/NAPT rules");
break}}else a.errorMessages.push("Public IP Address: IP address must consist of 4 fields. Each field's value must be between 0 and 255, without any spaces.");if(0<a.errorMessages.length)a.showErrorPage=!0,window.scrollTo(0,0);else{-1!=a.statNat.localHost&&(a.statNat.host=a.statNat.localHost);d={host:a.statNat.host,publicIp:getIpString(a.statNat.publicIp),portForwarding:a.statNat.portForwarding,services:[],active:!0};if(-1==a.statNat.localHost)d.host=a.statNat.host;else for(b=0;b<a.devices.length;b++)a.statNat.localHost==
a.devices[b].id&&(d.host=a.devices[b].ipAddress);copyServices(d.services,a.statNat.services);if(0<d.services.length)for(b=0;b<d.services.length;b++)for(var e=0;e<a.pfRulesCopy.length;e++)d.services[b].name==a.pfRulesCopy[e].name&&(d.services[b].id=a.pfRulesCopy[e].id,d.services[b].serviceType=a.pfRulesCopy[e].serviceType);d.portForwarding||(d.services=[]);for(b=0;b<d.services.length;b++)d.services[b].name=escape(d.services[b].name).replace(/\+/g,"%2B");-1!=a.checkEdit?c.update({id:a.staticNAT[a.checkEdit].id},
d,function(){a.refresh()}):c.save(d,function(){a.refresh()});a.initVars();a.togglePage(0)}};a.refresh=function(){a.staticNAT=c.query(function(){a.enabled=[];for(var b=0;b<a.staticNAT.length;b++){a.enabled.push(a.staticNAT[b].active);for(var c=0;c<a.staticNAT[b].services.length;c++)a.staticNAT[b].services[c].name=unescape(a.staticNAT[b].services[c].name),a.staticNAT[b].services[c].description=unescape(a.staticNAT[b].services[c].description)}});a.pfRules=e.query(function(){a.pfRules=sortListItems(a.pfRules);
a.pfRulesCopy=copyArrayFromSource(a.pfRules)})};a.cancelRule=function(){a.refresh();a.initVars();a.togglePage(0)};a.editRule=function(b){a.statNat={localHost:-1,host:a.staticNAT[b].host,publicIp:splitIpArr(a.staticNAT[b].publicIp),portForwarding:a.staticNAT[b].portForwarding,services:[]};copyServices(a.statNat.services,a.staticNAT[b].services);for(var c=0;c<a.statNat.services.length;c++){a.statNat.services[c].displayName=a.statNat.services[c].name;a.statNat.services[c].description&&0<a.statNat.services[c].description.length&&
(a.statNat.services[c].displayName+=" - "+a.statNat.services[c].description);for(var d=0;d<a.pfRules.length;d++)a.statNat.services[c].name==a.pfRules[d].name&&(a.statNat.services[c].id=a.pfRules[d].id,a.statNat.services[c].serviceType=a.pfRules[d].serviceType,a.pfRules.splice(d,1))}a.editing=b;a.checkEdit=b;a.togglePage(1);a.pageName="Edit"};a.removeRule=function(b){c.remove({id:a.staticNAT[b].id},function(){a.refresh()})};a.ruleChanged=function(){-2!=a.prot&&(-1==a.prot?a.addService():a.addPfProt(a.findRuleIndex(a.prot)))};
a.findRuleIndex=function(b){for(var c=0;c<a.pfRules.length;c++)if(a.pfRules[c].id==b)return c};a.filterFn=function(a){return/^PortForward\_.*$/.test(a.name)?!1:!0};a.addPfProt=function(b){var c={name:"",type:2,protocols:[]};copyService(c,a.pfRules[b]);c.displayName=a.pfRules[b].name;c.id=a.pfRules[b].id;c.serviceType=a.pfRules[b].serviceType;a.pfRules[b].description&&0<a.pfRules[b].description.length&&(c.displayName+=" - "+a.pfRules[b].description);a.statNat.services.push(c);a.setPrintArr(1);a.prot=
-2;b=[];for(var d=0;d<a.pfRules.length;d++)c.id!=a.pfRules[d].id&&b.push(a.pfRules[d]);a.pfRules=b};a.applyPortForwardRule=function(){a.errorMessages=[];""==a.service.name?a.errorMessages.push("Service Name: A name must be specified."):/^[0-9A-Z\!\-\_]+$/i.test(a.service.name)||a.errorMessages.push('Service Name: Rule name should consist of standard characters only (ASCII 32-126) excluding the special character space and any of :@"|\\/=+<>()[]{}&%^$*?,;');for(var b=0;b<a.pfRules.length;b++)b!=a.editing&&
a.pfRules[b].name==a.service.name&&a.errorMessages.push("Service Name: This name is already used by another protocol.");0==a.service.protocols.length&&a.errorMessages.push("Server Ports: At least one server ports entry must be defined.");if(0<a.errorMessages.length)a.showErrorPage=!0,window.scrollTo(0,0);else{applyService(a,a.statNat.services);for(b=0;b<a.statNat.services.length;b++)a.statNat.services[b].displayName=a.statNat.services[b].name,a.statNat.services[b].description&&0<a.statNat.services[b].description.length&&
(a.statNat.services[b].displayName+=" - "+a.statNat.services[b].description);a.setPrintArr(1);a.togglePage(1)}};a.cancelPortForwardRule=function(){cancelService(a);a.togglePage(1)};a.addService=function(){a.editing=-1;a.togglePage(3)};a.editService=function(b){a.editing=b;editService(a,a.statNat.services,b)};a.removeService=function(b){for(var c=0;c<a.pfRulesCopy.length;c++)if(a.pfRulesCopy[c].id==a.statNat.services[b].id){a.pfRules.push(a.pfRulesCopy[c]);break}a.pfRules=sortListItems(a.pfRules);
removeObject(a.statNat.services,b)};a.applyProtocol=function(){a.errorMessages=[];a.protocols.protocol=parseInt(a.protocols.protocol);a.protocols.incomingPorts=parseInt(a.protocols.incomingPorts);a.protocols.outgoingPorts=parseInt(a.protocols.outgoingPorts);a.protocols.icmpMessage=parseInt(a.protocols.icmpMessage);a.protocols.protocolNumber=parseInt(a.protocols.protocolNumber);var b=a.protocols.incomingPortStart,c=a.protocols.incomingPortEnd,d=a.protocols.outgoingPortStart,e=a.protocols.outgoingPortEnd;
switch(a.protocols.protocol){case 0:var f=a.protocols.protocolNumber;(null==f||""===f||isNaN(f)||0>f||255<f)&&a.errorMessages.push("Protocol Number: Please enter a numeric value between 0 and 255.");break;case 1:case 2:switch(a.protocols.incomingPorts){case 1:(null==b||""===b||1>b||65535<b)&&a.errorMessages.push("Source Port: Please enter a numeric value between 1 and 65535.");break;case 2:(null==b||""===b||1>b||65535<b)&&a.errorMessages.push("Start Source Port: Please enter a numeric value between 1 and 65535."),
(null==c||""===c||1>c||65535<c)&&a.errorMessages.push("End Source Port: Please enter a numeric value between 1 and 65535."),0==a.errorMessages.length&&b>=c&&a.errorMessages.push("Source Ports: The From value must be lower than the To value in a range of ports (from="+b+", to="+c+").")}switch(a.protocols.outgoingPorts){case 1:(null==d||""===d||1>d||65535<d)&&a.errorMessages.push("Destination Port: Please enter a numeric value between 1 and 65535.");4567==d&&!1==a.protocols.outgoingExclude&&1==a.protocols.protocol&&
a.errorMessages.push("Invalid rule: Port 4567 cannot be blocked");break;case 2:(null==d||""===d||1>d||65535<d)&&a.errorMessages.push("Start Destination Port: Please enter a numeric value between 1 and 65535."),(null==e||""===e||1>e||65535<e)&&a.errorMessages.push("End Destination Port: Please enter a numeric value between 1 and 65535."),0==a.errorMessages.length&&d>=e&&a.errorMessages.push("Destination Ports: The From value must be lower than the To value in a range of ports (from="+d+", to="+e+").")}break;
case 3:if(10==a.protocols.icmpMessage){var f=a.protocols.icmpType,q=a.protocols.icmpCode;(null==f||""===f||0>f||255<f)&&a.errorMessages.push("ICMP Type:  Please enter a numeric value between 0 and 255.");(null==q||""===q||0>q||255<q)&&a.errorMessages.push("ICMP Code:  Please enter a numeric value between 0 and 255.")}}0==a.errorMessages.length&&(0==a.protocols.incomingPorts&&(a.protocols.incomingExclude=!1),0==a.protocols.outgoingPorts&&(a.protocols.outgoingExclude=!1),1==a.protocols.protocol||2==
a.protocols.protocol)&&(q=f=!1,0==a.protocols.incomingPorts?f=!0:2==a.protocols.incomingPorts&&!1==a.protocols.incomingExclude&&1==b&&65535==c&&(f=!0),0==a.protocols.outgoingPorts?q=!0:2==a.protocols.outgoingPorts&&!1==a.protocols.outgoingExclude&&1==d&&65535==e&&(q=!0),f&&q&&a.errorMessages.push('Port forwarding rules can not be Any-to-Any (or "source: 1-65535" with "destination: 1-65535").'));0<a.errorMessages.length?(a.showErrorPage=!0,window.scrollTo(0,0)):applyProtocol(a,a.service.protocols)};
a.cancelProtocol=function(){cancelProtocol(a);a.togglePage(3)};a.editProtocol=function(b){editProtocol(a,b,a.service.protocols)};a.removeProtocol=function(b){removeObject(a.service.protocols,b,2,a)};a.keyPress=function(b){13==b.keyCode&&"/firewall/staticnat"==f.path()&&(0!=a.pageToShow||a.showErrorPage?1!=a.pageToShow||a.showErrorPage?3!=a.pageToShow||a.showErrorPage?4!=a.pageToShow||a.showErrorPage||a.applyProtocol():a.applyPortForwardRule():a.saveRule():a.save())};a.refresh()}]).controller("FirewallSecurityLogCtrl",
["$scope","$route","$http","SecuritySettings","SecurityCsv","SecurityHazard","Log","$location",function(a,b,c,d,e,f,g,h){a.tab=b.current.tab;a.section=b.current.section;a.showSettings=0;a.FDisplayNum=50;a.securitySettings=d.get();a.enableLogging=systemSettings.enableLogging;setupLoggingCtrl(a,1,c,g);a.loadMore=function(b){a.FDisplayNum+=50};a.clearSecLog=function(){a.clearLog();a.showSettings=0};a.clearLogScreen=function(){systemSettings.warnBeforeChanges?a.showSettings=3:a.clearLog()};a.getTime=
function(a){return moment.unix(a).format("MMM DD HH:mm:ss YYYY")};a.save=function(){a.securitySettings.$update(function(){a.showSettings=0;a.securitySettings=d.get()})};a.togglePage=function(b){0==b&&(a.securitySettings=d.get());a.showSettings=b;window.scrollTo(0,0)};a.markViewed=function(){f.update(function(){location.reload()})};a.keyPress=function(a){13==a.keyCode&&"/firewall/log"==h.path()&&h.path("/firewall")}}]).controller("dmzHostCtrl",["$scope","$route","securityTips","FirewallDMZ","$location",
"NetworkConnections","PortForward",function(a,b,c,d,e,f,g){a.tab=b.current.tab;a.section=b.current.section;a.portForwardingDisabled=!1;a.ipAddrDisabled=[!1,!1,!1,!1];a.subNetID=[];a.refresh=function(){a.connection=f.get({id:0},function(){a.dmz=d.get(function(){a.listOfNetworkIPAddress=[];var b={};b.id=a.connection.connectionId;b.ip=a.connection.ipAddress;b.name=unescape(a.connection.name);a.listOfNetworkIPAddress.push(b);a.subNetID=splitIpArr(a.connection.subnetMask);for(b=0;b<a.subNetID.length;b++)"255"==
a.subNetID[b]||255==parseInt(a.subNetID[b])?a.ipAddrDisabled[b]=!0:a.ipAddrDisabled[b]=!1;""==a.dmz.ip||"0.0.0.0"==a.dmz.ip?(a.ipAddr=splitIpArr(angular.copy(a.connection.ipAddress)),a.ipAddr[3]=""):a.ipAddr=splitIpArr(a.dmz.ip);a.enabled=a.dmz.enabled});a.portForward=g.query()})};a.refresh();a.toggleOnOff=function(){a.enabled=!a.enabled};a.save=function(){var b={};b.enabled=a.enabled;b.enabled&&(b.ip=getIpString(a.ipAddr));d.update(b,function(){a.refresh()})};a.checkThenSave=function(){a.errorMessages=
[];if(a.enabled)if(validateIp(a.ipAddr)){var b=splitIpArr(a.connection.ipAddress),d=splitIpArr(a.connection.subnetMask);checkAllNetworks(a.listOfNetworkIPAddress,a.ipAddr,1)&&a.errorMessages.push("Local IP Address: The FiOS Quantum Gateway's own IP address("+a.listOfNetworkIPAddress[0].ip+") cannot be used as a DMZ host");checkHostVsSubmask(a.ipAddr,d)&&("0"==checkBits(a.ipAddr)?a.errorMessages.push("The DMZ Host address must not match the network address of the subnet (.0 if the netmask is 255.255.255.0)."):
a.errorMessages.push("The DMZ Host address must not match the broadcast address (.255 if the netmask is 255.255.255.0)."));checkIf2IPAreInTheSameSubnetRange(a.ipAddr,b,d)||a.errorMessages.push("The DMZ Host address must be in the same subnet as the router's LAN address.")}else a.errorMessages.push("IP Address: IP address must consist of 4 fields. Each field's value must be between 0 and 255, without any spaces.");if(0!=a.errorMessages.length)a.showErrorPage=!0,window.scrollTo(0,0);else{var e;a.enabled&&
(a.portForwardingDisabled=!1,checkPortForwarding(a.portForward,a.ipAddr)&&(a.portForwardingDisabled=!0,e="The device with IP address "+getNumIpString(a.ipAddr)+" has an existing port forwarding rule. That port forwarding rule will become redundant with DMZ Host enabled for that IP address."));a.enabled&&a.portForwardingDisabled?c.showDmzSaving("",e,[""]).then(function(b){"cancel"!=b&&a.save()}):a.enabled?c.showDmzSaving1("",[""]).then(function(b){"cancel"!=b&&a.save()}):a.save()}};a.errorPageOk=function(){a.showErrorPage=
!1;window.scrollTo(0,0)};a.cancel=function(){window.scrollTo(0,0);e.url("/firewall")}}]);function init_networkObjs(a){init_networkObj(a);init_address(a)}function init_networkObj(a){a.networkObj={name:"Global Object",type:1,rules:[]};a.editingNetworkObj=-1}function init_address(a){a.rule={networkObjType:0,dhcpType:0,ip:splitIpArr("0.0.0.0"),ipTwo:splitIpArr("0.0.0.0"),mac:splitMacArr("00:00:00:00:00:00"),macMask:splitMacArr("ff:ff:ff:ff:ff:ff"),hostname:""};a.editingAddress=-1}
...
h.subnetMask);e.push(h.ipSubnet+" / "+h.subnetMask);break;case 2:a[d].displayText.push(h.ipStart+" - "+h.ipEnd);e.push(h.ipStart+" - "+h.ipEnd);break;case 3:a[d].displayText.push(h.mac);e.push(h.mac);break;case 4:a[d].displayText.push(h.hostname);for(var k=!1,m=0;m<b.length;m++)if(b[m].name==h.hostname){e.push(b[m].ipAddress);k=!0;break}k||e.push("");break;case 5:switch(h.dhcpType){case 0:e.push("DHCP Option: Vendor Class ID = "+h.option);a[d].displayText.push("DHCP Option: Vendor Class ID = "+h.option);
break;case 1:e.push("DHCP Option: Client ID = "+h.option);a[d].displayText.push("DHCP Option: Client ID = "+h.option);break;case 2:e.push("DHCP Option: User Class ID = "+h.option),a[d].displayText.push("DHCP Option: User Class ID = "+h.option)}break;case 6:e.push(h.hostname);a[d].displayText.push(h.hostname);break;case 7:a[d].displayText.push(h.name);k=!1;for(m=0;m<b.length;m++)if(b[m].name==h.name&&b[m].ipAddress===h.ipAddress){e.push(b[m].ipAddress);k=!0;break}k||e.push("")}}}c.push(e)}}
...
function overlapsPort(a,b,c,d,e,f,g){var h="",k=h="";1==g&&(0==a&&(a=1,b=4567),0==d&&(d=1,e=4567));1==g?h=overlapsSubPort(a,b,c,d,e,f,"TCP"):0==a||0==d?(0==a?h="Any":1==a?h=b:2==a&&(h=b+"-"+c),0==d?k="Any":1==d?k=e:2==d&&(k=e+"-"+f),h="Overlapping Port Ranges: The UDP destination ports ("+h+") is overlapping with already defined UDP destination ports ("+k+")."):h=overlapsSubPort(a,b,c,d,e,f,"UDP");return h}
function overlapsSubPort(a,b,c,d,e,f,g){var h="",k="",m="",p=!1;1==a?1==d?Number(b)==Number(e)&&(k=b,m=e,p=!0):Number(f)>=Number(b)&&Number(e)<=Number(b)&&(k=b,m=e+"-"+f,p=!0):1==d?Number(c)>=Number(e)&&Number(b)<=Number(e)&&(k=b+"-"+c,m=e,p=!0):Number(c)>=Number(e)&&Number(b)<=Number(e)?(k=b+"-"+c,m=e+"-"+f,p=!0):Number(c)>=Number(f)&&Number(b)<=Number(f)?(k=b+"-"+c,m=e+"-"+f,p=!0):Number(f)>=Number(c)&&Number(e)<=Number(c)?(k=b+"-"+c,m=e+"-"+f,p=!0):Number(f)>=Number(c)&&Number(e)<=Number(c)&&(k=
b+"-"+c,m=e+"-"+f,p=!0);p&&(h="Overlapping Port Ranges: The "+g+" destination ports ("+k+") is overlapping with already defined "+g+" destination ports ("+m+").");return h};angular.module("igmp.controllers",[]).controller("IgmpProxyCtrl",["$scope","$route","$location","IGMPProxy",function(a,b,c,d){a.tab=b.current.tab;a.section=b.current.section;a.showErrorPage=!1;a.errorPageOk=function(){a.showErrorPage=!1;window.scrollTo(0,0)};a.igmp=d.get();a.save=function(){a.errorMessages=[];a.igmp.enabled="true"===""+a.igmp.enabled;a.igmp.fastLeave="true"===""+a.igmp.fastLeave;(null==a.igmp.robustness||isNaN(a.igmp.robustness)||1>a.igmp.robustness||!checksForDecimalNumber(a.igmp.robustness))&&
...
angular.module("main.controllers",[]).controller("MainCtrl",["$scope","$route","Devices","NetworkConnections","FirmwareUpgrade","Logout","$location","SystemSettings","securityTips",function(a,b,c,d,e,f,g,h,k){a.tab=b.current.tab;a.hasFailedLogin=!1;a.failedLogin=0;a.hasList0=!1;a.hasList1=!1;a.showList0=!1;a.showList1=!1;a.list0Count=0;a.list1Count=0;a.devices=c.query(function(){a.list0Count=0;a.list1Count=0;a.hasList0=!1;a.hasList1=!1;for(var b=0;b<a.devices.length;b++)6!=a.devices[b].connectionType?
a.list0Count++:6==a.devices[b].connectionType&&a.devices[b].approved&&a.list1Count++,setDeviceStyle(a.devices[b],""),a.devices[b]=checkDeviceIP(a.devices[b]);3<a.list0Count&&(a.hasList0=!0);3<a.list1Count&&(a.hasList1=!0)});a.showDevice=function(b,c){return 0==b?a.showList0?!0:3>c?!0:!1:a.showList1?!0:3>c?!0:!1};a.toggleList=function(b,c){0==b?a.showList0=0==c?!1:!0:a.showList1=0==c?!1:!0};a.fwInfo=e.get();var m="";a.ipString2="";a.ArrStr=[];a.broadband=null;a.hasMoreLen=!1;var p=d.query(function(){for(var b=
0;b<p.length;b++)if(1==p[b].type){a.hasMoreLen=!1;a.broadband=p[b];for(var b=a.broadband.ipv6Address.split(":"),c=0;c<b.length;c++)m=m+b[c]+":",4>c&&a.ArrStr.push(m);4<b.length&&(a.hasMoreLen=!0);for(c=4;c<b.length;c++)a.ipString2=a.ipString2+b[c]+":";a.ipString2=a.ipString2.substring(0,a.ipString2.length-1);break}});a.systemSettings=h.get(function(){a.hasFailedLogin=!1;a.failedLogin=a.systemSettings.previousFailedLogins;0<a.failedLogin&&(a.hasFailedLogin=!0)});a.noConnection=function(){k.showCustomMessage3("Warning",
["You need to be connected to the internet to view the User Guide"])};a.nonGuestDevice=function(a){return 6==Number(a.connectionType)?!1:!0};a.guestDevice=function(a){return 6==Number(a.connectionType)&&a.approved?!0:!1}}]).controller("empty",["$scope","$route","Login",function(a,b,c){a.tab=b.current.tab;a.section=b.current.section;c.get()}]).controller("LoginCtrl",["$scope","$rootScope","$location","$routeParams","Login","SystemSettings","$timeout",function(a,b,c,d,e,f,g){function h(){var a=window.navigator.userAgent,
b=a.indexOf("MSIE "),c=a.indexOf("Trident/");return 0<b?parseInt(a.substring(b+5,a.indexOf(".",b)),10):0<c?(b=a.indexOf("rv:"),parseInt(a.substring(b+3,a.indexOf(".",b)),10)):!1}a.cookiesEnabled="cookie"in document&&(0<document.cookie.length||-1<(document.cookie="test").indexOf.call(document.cookie,"test"));if(!a.cookiesEnabled&&h()){var k;k=-1!=c.$$host.indexOf("_")?!0:!1;k?a.isUrlError=!0:a.isUrlError=!1}a.cookiesEnabled?(a.username="admin",a.password="",a.showError=!1,a.showTimeout=!1,a.loggingIn=
!1,a.showPassword=!1,a.setupRedirect=!0,a.requirePassword=!0,a.isWireless=!1,a.showPasswordForm=!0,d.setup&&(a.setupRedirect=d.setup),null!=d.fromlogout&&(a.showError=!0,a.errorMessage="User has logged out, please login again."),e.get(function(){null==systemSettings?systemSettings=f.get(function(){b.loggedIn=!0;b.nextLocation&&"/login"!=b.nextLocation?c.path(b.nextLocation):c.path("/main");c.replace()}):(b.loggedIn=!0,b.nextLocation&&"/login"!=b.nextLocation?c.path(b.nextLocation):c.path("/main"),
c.replace())},function(b){a.loggingIn=!0;a.timeout1=!1;a.timeout24=!1;switch(b.data.error){case 1:a.showError=!0,a.errorMessage=null!=d.rebooting?"Wireless Broadband Router is up again, please login:":"Connection has expired, please login again:";default:a.passwordSalt=b.data.passwordSalt;if(!0==b.data.requirePassword||!1==b.data.requirePassword)a.requirePassword=b.data.requirePassword;if(!0==b.data.isWireless||!1==b.data.isWireless)a.isWireless=b.data.isWireless;a.showPasswordForm=a.isWireless||
a.requirePassword?!0:!1;if(b.data.doSetupWizard&&"false"!==a.setupRedirect)c.path("/setup");else switch(b.data.denyState){case 1:a.timeout1=!0;g(function(){a.timeout1=!1;a.showError=!1},6E4*Number(b.data.denyTimeout));a.showError=!0;a.errorMessage="Login has been disabled for 1 minute. If you have not changed your password, you can find it on the router\u2019s sticker.";a.password="";break;case 2:a.timeout24=!0,g(function(){a.timeout24=!1;a.showError=!1},6E4*Number(b.data.denyTimeout)),a.showError=
!0,a.errorMessage="Login has been disabled for 24 hours. Please wait 24 hours before trying again, or call Verizon for a password reset",a.password=""}}}),a.handleKeyPress=function(b){13==b&&a.login()},a.login=function(){var d={};d.password=CryptoJS.SHA512(a.password+a.passwordSalt).toString();e.save({},d,function(){systemSettings=f.get(function(){b.loggedIn=!0;b.nextLocation&&"/login"!=b.nextLocation?c.path(b.nextLocation):c.path("/main");c.search({});c.replace()})},function(b){a.showError=!0;a.timeout24=
!1;a.timeout1=!1;switch(b.data.denyState){case 1:a.timeout1=!0;g(function(){a.timeout1=!1;a.showError=!1},6E4*Number(b.data.denyTimeout));a.errorMessage="Login has been disabled for 1 minute. If you have not changed your password, you can find it on the router\u2019s sticker.";a.password="";break;case 2:a.timeout24=!0;g(function(){a.timeout24=!1;a.showError=!1},6E4*Number(b.data.denyTimeout));a.errorMessage="Login has been disabled for 24 hours. Please wait 24 hours before trying again, or call Verizon for a password reset";
a.password="";break;default:a.errorMessage=2==b.data.error?"Sessions Number: No more than "+b.data.maxUsers+" sessions at a time are allowed. Please wait until open sessions expire.":"Login failed, please try again:",a.password=""}})}):a.retry=function(){location.reload()}}]).controller("LogoutCtrl",["$scope","$location","Logout",function(a,b,c){c.get(function(){b.path("/login").search({fromlogout:"true"});b.replace()})}]).controller("SetupCtrl",["$scope","$location","Login","Logout","Users","Wireless",
"SetupWizard","SystemSettings","$timeout",function(a,b,c,d,e,f,g,h,k){a.page=0;a.dontShowAgain=!1;a.notConnected=!1;var m=0;d.get(function(){c.get(function(){},function(b){a.passwordSalt=b.data.passwordSalt;a.isWireless=b.data.isWireless;a.timeout1=!1;a.timeout24=!1;switch(b.data.denyState){case 1:a.timeout1=!0;k(function(){a.timeout1=!1;a.errorMessage=""},6E4*Number(b.data.denyTimeout));break;case 2:a.timeout24=!0,k(function(){a.timeout24=!1;a.errorMessage=""},6E4*Number(b.data.denyTimeout))}})});
a.print=function(){window.print()};a.handlePassKeyUp=function(b){13==b&&a.login()};a.doLater=function(){a.dontShowAgain?c.update({doSetupWizard:!1},function(){b.path("/login")}):b.path("/login").search({setup:"false"})};a.getStarted=function(){a.setLoginSection()};a.loginCancelSetup=function(){b.path("/login").search({setup:"false"})};a.setLoginSection=function(){a.page=1;a.password="";a.showPassword=!1;a.errorMessage="";a.timeout24&&(a.errorMessage="Login has been disabled for 24 hours. Please wait 24 hours before trying again, or call Verizon for a password reset");
a.timeout1&&(a.errorMessage="Login has been disabled for 1 minute. If you have not changed your password, you can find it on the router\u2019s sticker.")};a.login=function(){var b={};b.password=CryptoJS.SHA512(a.password+a.passwordSalt).toString();c.save({},b,function(){systemSettings=h.get(function(){a.setEditSection()})},function(b){a.timeout1=!1;a.timeout24=!1;switch(b.data.denyState){case 1:a.timeout1=!0;k(function(){a.timeout1=!1;a.errorMessage=""},6E4*Number(b.data.denyTimeout));a.errorMessage=
"Login has been disabled for 1 minute. If you have not changed your password, you can find it on the router\u2019s sticker.";a.password="";break;case 2:a.timeout24=!0;k(function(){a.timeout24=!1;a.errorMessage=""},6E4*Number(b.data.denyTimeout));a.errorMessage="Login has been disabled for 24 hours. Please wait 24 hours before trying again, or call Verizon for a password reset";a.password="";break;default:a.errorMessage=2==b.data.error?"Sessions Number: No more than "+b.data.maxUsers+" sessions at a time are allowed. Please wait until open sessions expire.":
"Login failed, please try again:",a.password=""}})};a.setEditSection=function(){k(function(){a.showErrorPage=!1;a.page=2;a.isNewPassword=!1;a.isNewSsid=!1;a.isNewWifiPw=!1;a.newPassword=a.password;a.rePassword="";a.wireless=f.get({id:0},function(){a.wireless.ssid=unescape(a.wireless.ssid);a.oldSsid=a.wireless.ssid;a.dispSecurity="WPA2 Key";a.secKey=a.wireless.security.wpa.key;a.oldKey=a.secKey});a.modifyWifi=!1;a.secType=3},200)};a.setMaxlength=function(a){return 1===Number(a)?13:63};a.secChanged=
function(){0==a.secType?(a.dispSecurity="WPA2 Key",a.secKey=a.wireless.security.wpa.key):1==a.secType?(a.dispSecurity="WEP Key",a.secKey=a.wireless.security.wep.settings[0].key):3==a.secType?(a.dispSecurity="WPA2 Key",a.secKey=a.wireless.security.wpa.key):(a.dispSecurity="None",a.secKey="");a.oldKey=a.secKey;a.isNewWifiPw=!1};a.handleKeyPress=function(){a.isNewPassword=a.password!==a.newPassword};a.handleSsidChange=function(){a.isNewSsid=a.oldSsid!==a.wireless.ssid};a.handleWifiPwChange=function(){a.isNewWifiPw=
a.oldKey!==a.secKey};a.editWifi=function(){a.modifyWifi=!0};a.errorPageOk=function(){a.showErrorPage=!1;window.scrollTo(0,0)};a.acceptSettings=function(){a.errorMessages=[];var b={security:{}};a.isNewSsid&&(b.ssid=escape(a.wireless.ssid),(1>a.wireless.ssid.length||64<a.wireless.ssid.length||!/^[0-9a-zA-Z\!\"\#\$\%\&\u2019\'\(\)\*\+\,\-\.\/\:\;\<\=\>\?\@\[\\\]\^\_\`\{\|\}\~\ ]+$/i.test(a.wireless.ssid))&&a.errorMessages.push("Service Set Identifier must consist of 1 to 64 characters and must not contain any special character."));
b.security.enabled=2!=a.secType;b.security.enabled&&(1==a.secType?b.security.type=0:0==a.secType?b.security.type=1:3==a.secType&&(b.security.type=2),0==a.secType||3==a.secType?(b.security.wpa={key:a.secKey},8>a.secKey.length&&a.errorMessages.push("Pre-Shared Key: Pass-Phrase has to be at least 8 characters long.")):(b.security.wep={activeSetting:0,settings:[{key:a.secKey,keyLength:1,entryMethod:1}]},13!=a.secKey.length?a.errorMessages.push("The WEP key must be a string of 13 characters."):/[\x21-\x7E]{13}$/i.test(a.secKey)||
a.errorMessages.push("Invalid WEP Key. Only ASCII characters are allowed.")));a.isNewPassword&&(a.newPassword!=a.rePassword?a.errorMessages.push("Passwords do not match."):0==a.newPassword.length&&a.errorMessages.push("A password must be specified."));if(0<a.errorMessages.length)a.showErrorPage=!0;else{if(a.isNewPassword){var c=uuid(),d=CryptoJS.SHA512(a.newPassword+c).toString();g.update({},{wifi:b,password:d,salt:c})}else g.update({},{wifi:b});a.page=4}};a.confirmSettings=function(){a.isWireless?
!1===a.isNewSsid&&!1===a.isNewWifiPw?b.path("/main"):a.page=5:b.path("/main")};a.checkConnection=function(){a.page=5;a.notConnected=!0;h.get(function(){k(function(){a.page=7},2E3)},function(b){200!=b.status&&(m++,3==m?(a.page=6,m=0):window.setTimeout(a.checkConnection,2E3))})};a.keyPress=function(c){if(13==c.keyCode)switch(a.page){case 0:a.getStarted();break;case 1:a.login();break;case 2:a.acceptSettings();break;case 4:a.confirmSettings();break;case 5:a.checkConnection();break;case 6:a.checkConnection();
break;case 7:b.path("/main")}}}]);function Route(a){var b={};b.type=a;b.destination="0.0.0.0";b.netmask="0.0.0.0";b.gateway="0.0.0.0";b.metric=3;return b}function setDeviceStyle(a,b){var c;switch(a.icon){case 0:c="computer";break;case 1:c="tablet";break;case 2:c="camera";break;case 3:c="printer";break;case 4:c="ip-stb";break;case 5:c="phone";break;case 6:c="nas";break;default:c="unknown"}c+="-image";a.status||(c+=" inact");a.blocked&&(c+=" blocked");""!=b&&(c+=" "+b);a.styleName=c}
angular.module("network.controllers",[]).controller("NetworkCtrl",["$scope","$route","Devices","$location","IPv6",function(a,b,c,d,e){a.tab=b.current.tab;a.section=b.current.section;a.hasList0=!1;a.hasList1=!1;a.showList0=!1;a.showList1=!1;a.list0Count=0;a.list1Count=0;a.devices=c.query(function(){a.list0Count=0;a.list1Count=0;a.hasList0=!1;a.hasList1=!1;for(var b=0;b<a.devices.length;b++)6!=a.devices[b].connectionType?a.list0Count++:6==a.devices[b].connectionType&&a.devices[b].approved&&a.list1Count++;
3<a.list0Count&&(a.hasList0=!0);3<a.list1Count&&(a.hasList1=!0)});a.showDevice=function(b,c){return 0==b?a.showList0?!0:3>c?!0:!1:a.showList1?!0:3>c?!0:!1};a.toggleList=function(b,c){0==b?a.showList0=0==c?!1:!0:a.showList1=0==c?!1:!0};a.ipv6=e.get();a.deviceTypes=function(b){var c=0;angular.forEach(a.devices,function(a){a.connectionType==b?c+=1:5==b&&6==a.connectionType&&(c+=1);setDeviceStyle(a,"device");checkDeviceIP(a)});return c};a.checkNetworkConnections=function(){d.path("/network/connections")};
a.blockDevice=function(b){for(var e=0;e<a.devices.length;e++)a.devices[e].id==b&&(a.devices[e].blocked=a.devices[e].blocked?!1:!0);c.update({id:b},a.devices[b],function(){d.path("/network");window.scrollTo(0,0)})};a.nonGuestDevice=function(a){return 6==Number(a.connectionType)?!1:!0};a.guestDevice=function(a){return 6==Number(a.connectionType)&&a.approved?!0:!1}}]).controller("NetworkConnectionsCtrl",["$scope","$route","$location","NetworkConnections","$routeParams","SystemSettings","$templateCache",
"Devices","$rootScope",function(a,b,c,d,e,f,g,h,k){a.tab=b.current.tab;a.section=b.current.section;a.detectBroadband=!1;a.restarting=!1;a.rebootingPage=!1;a.getStyleName=function(a){switch(a){case 0:return"Network";case 1:return"Network";case 2:return"Wireless";case 3:return"Wireless";case 4:return"Ethernet";case 5:return"Coax";default:return"Network"}};null!=e.fromdevicenetwork&&c.path("/network/connections/0");a.checkForDevicesNow=function(a,b){var c=!1,d="";switch(String(b)){case "2":d="4";break;
case "3":d="5";break;case "4":d="0";break;default:return!1}if(0<a.length){for(var e=0;e<a.length;e++){if(String(a[e].connectionType)==String(d)&&!0==a[e].status){c=!0;break}if("6"==String(a[e].connectionType)&&a[e].approved&&"5"==String(d)&&!0==a[e].status){c=!0;break}}return c}return!1};var m,p=h.query(function(){m=d.query(function(){a.names=[];for(var b=[],c=0;c<m.length;c++)if(!m[c].done&&(m[c].done=!0,m[c].isChild=!1,m[c].styleName=a.getStyleName(m[c].connectionId),m[c].hasConnectedDeviceNow=
a.checkForDevicesNow(p,m[c].connectionId),b.push(m[c]),a.names.push(unescape(m[c].name)),m[c].availableBridges))for(var d=0;d<m[c].availableBridges.length;d++){var e=m[c].availableBridges[d];1!=e&&m[c].bridged[d]&&(m[e].isChild=!0,m[e].done||(m[e].styleName="BridgedItem "+a.getStyleName(m[e].connectionId),b.push(m[e]),a.names.push(unescape(m[e].name)),m[e].done=!0,m[e].hasConnectedDeviceNow=a.checkForDevicesNow(p,m[e].connectionId)))}a.connections=b})});a.showChildren=k.connectionListIsAdvanced;a.detectConfirm=
function(){systemSettings.warnBeforeChanges?a.detectBroadband=!0:a.doDetectBroadband()};a.detectCancel=function(){a.detectBroadband=!1};a.doDetectBroadband=function(){d.update({id:1},{detectBroadband:!0},function(){a.rebootingPage=!0;window.setTimeout(a.testConnection,0)})};a.testConnection=function(){f.get(function(){window.setTimeout(a.testConnection,3E3)},function(b){401==b.status?(a.restarting=!0,g.removeAll(),window.location="/redirect.html?v=v1.0.125"):window.setTimeout(a.testConnection,
...
a.deviceNames=[];a.checkForDevicesNow=function(a,b){var c=!1,d="";switch(String(b)){case "2":d="4";break;case "3":d="5";break;case "4":d="0";break;default:return!1}if(0<a.length){for(var e=0;e<a.length;e++){if(String(a[e].connectionType)==String(d)&&!0==a[e].status){c=!0;break}if("6"==String(a[e].connectionType)&&a[e].approved&&"5"==String(d)&&!0==a[e].status){c=!0;break}}return c}return!1};a.showBridgeDeviceList=function(b){for(var c=0;c<a.deviceBridge.length;c++)if(a.deviceBridge[c].name==b)return a.deviceBridge[c].toShowOut;
return!0};a.findDeviceNames=function(b){for(var c=0;c<a.deviceBridge.length;c++)if(a.deviceBridge[c].name==b)return a.deviceBridge[c].realDeviceName};var k,m=f.query(function(){k=e.get({id:c.id},function(){a.name=unescape(k.name);a.deviceBridge=[];if(k.availableBridges)for(var b=0;b<k.availableBridges.length;b++){var c={};c.name=k.availableBridges[b];c.toShowOut=k.bridged[b];a.deviceBridge.push(c)}k.hasConnectedDeviceNow=a.checkForDevicesNow(m,k.connectionId);a.connections=e.query(function(){for(var b=
0;b<a.connections.length;b++);if(k.underlyingDevices)for(angular.forEach(k.underlyingDevices,function(b){angular.forEach(a.connections,function(c){c.connectionId==b&&a.deviceNames.push(unescape(c.name))})}),b=0;b<a.deviceBridge.length;b++)for(var c=0;c<a.connections.length;c++)if(a.connections[c].connectionId==a.deviceBridge[b].name){a.deviceBridge[b].realDeviceName=unescape(a.connections[c].name);break}a.connection=k;a.connFromString=""===a.fromString?"?from="+a.connection.connectionId:a.fromString+
"-"+a.connection.connectionId;a.showIpv6=!1;void 0!==k.ipv6Address&&"::"!==k.ipv6Address&&""!==k.ipv6Address&&(a.showIpv6=!0)})})});a.errorPageOk=function(){a.showErrorPage=!1;window.scrollTo(0,0)};a.refreshContentsNow=function(){location.reload()};a.toggleEnabled=function(){e.update({id:k.connectionId},{enabled:!k.enabled},function(){3==k.connectionId||2==k.connectionId?(a.savePopup=g.showSaving1("Save Settings","Saving your settings. Please wait..."),window.setTimeout(a.refreshContentsNow,15E3)):
a.refreshContentsNow()})};a.toggleCoaxEnabled=function(){e.update({id:k.connectionId},{coaxEnabled:!k.coaxEnabled},function(){3==k.connectionId||2==k.connectionId?(a.savePopup=g.showSaving1("Save Settings","Saving your settings. Please wait..."),window.setTimeout(a.refreshContentsNow,15E3)):a.refreshContentsNow()})};a.cancelBtn=function(){window.scrollTo(0,0);null!=c.fromsystemtraffic?d.url("/monitoring/traffic"):null!=c.fromsystemfullstatus?d.url("/monitoring/fullstatus"):null!=c.fromdevicenetwork?
d.url("/network/device/details/"+c.devicenum+"?fromdevicenetwork=true&devicenum="+c.devicenum):null!=c.connEdit?d.url("/network/connections/"+c.connEdit+"/edit"):d.url(a.back+a.prev)};a.saveName=function(b){a.errorMessages=[];a.connection.name=escape(a.name.replace(/^\s+|\s+$/g,"")).replace(/\+/g,"%2B");if(0==a.connection.name.length)a.errorMessages.push("Connection Name: Connection Name cannot be empty.");else for(var f=0;f<a.connections.length;f++)f!=a.connection.connectionId&&unescape(a.connection.name)==
unescape(a.connections[f].name)&&a.errorMessages.push("Connection Name:  Connection Name already in use.");0<a.errorMessages.length?(a.showErrorPage=!0,window.scrollTo(0,0)):e.update({id:b.connectionId},{name:a.connection.name},function(){window.scrollTo(0,0);null!=c.fromsystemtraffic?d.url("/monitoring/traffic"):null!=c.fromsystemfullstatus?d.url("/monitoring/fullstatus"):null!=c.fromdevicenetwork?d.url("/network/device/details/"+c.devicenum+"?fromdevicenetwork=true&devicenum="+c.devicenum):null!=
c.connEdit?d.url("/network/connections/"+c.connEdit+"/edit"):""==a.oldFrom?d.path(a.back).search({}):d.path(a.back).search({from:a.oldFrom})})};a.checkNetworkConnections=function(){d.path("/network/connections")};a.keyPress=function(b){13==b.keyCode&&d.path()=="/network/connections/"+c.id&&a.saveName(a.connection)}}]).controller("NetworkConnectionEditCtrl",["$scope","$route","$routeParams","$location","DhcpOptions","NetworkConnections","DhcpConnections","Routes","Devices","FirewallDMZ",function(a,
b,c,d,e,f,g,h,k,m){a.tab=b.current.tab;a.section=b.current.section;a.confirm=!1;a.messages=[];a.staticConnections=[];a.routesConnections=[];a.oldCoaxPassword="";a.coTempIPAddress="";a.coTempHomeName="";a.tempStatic={};a.listOfNetworkIPAddress=[];a.fromString=c.from?"?from="+c.from+"&connectionEdit="+c.id:"?connectionEdit="+c.id;a.getStyleName=function(a){switch(a){case 0:return"Network";case 1:return"Network";case 2:return"Wireless";case 3:return"Wireless";case 4:return"Ethernet";case 5:return"Coax";
default:return"Network"}};a.checkForDevicesNow=function(a,b){var c=!1,d="";switch(String(b)){case "2":d="4";break;case "3":d="5";break;case "4":d="0";break;default:return!1}if(0<a.length){for(var e=0;e<a.length;e++){if(String(a[e].connectionType)==String(d)&&!0==a[e].status){c=!0;break}if("6"==String(a[e].connectionType)&&a[e].approved&&"5"==String(d)&&!0==a[e].status){c=!0;break}}return c}return!1};var p=k.query(function(){a.connection=f.get({id:c.id},function(){a.connection.styleName=a.getStyleName(c.id);
if(a.connection.dhcpExpires){var b=a.connection.dhcpExpires;a.connection.dhcpExpireTime=0!=b?Math.ceil(b/60)+" minutes":"Expired"}a.name=unescape(a.connection.name);a.connection.name=unescape(a.connection.name);a.connection.hasConnectedDeviceNow=a.checkForDevicesNow(p,a.connection.connectionId);a.oldsubnet=a.connection.subnetMask;a.oldProtocolUsed=a.connection.protocolUsed;switch(a.connection.type){case 3:a.connectionType="Ethernet";break;case 5:a.connectionType="Coax";break;case 4:a.connectionType=
"AP";break;default:a.connectionType="Basic"}1==a.connection.connectionId&&(a.tempStatic={ipAddress:a.connection.ipAddress,defaultGateway:a.connection.defaultGateway,primaryDnsServer:a.connection.primaryDnsServer,secondaryDnsServer:a.connection.secondaryDnsServer,subnetMask:a.connection.subnetMask},a.connection.ipAddress=void 0!=a.connection.staticIpAddress?a.connection.staticIpAddress:a.connection.ipAddress,a.connection.defaultGateway=void 0!=a.connection.staticDefaultGateway?a.connection.staticDefaultGateway:
a.connection.defaultGateway,a.connection.primaryDnsServer=void 0!=a.connection.staticPrimaryDnsServer?a.connection.staticPrimaryDnsServer:a.connection.primaryDnsServer,a.connection.secondaryDnsServer=void 0!=a.connection.staticSecondaryDnsServer?a.connection.staticSecondaryDnsServer:a.connection.secondaryDnsServer,a.connection.subnetMask=void 0!=a.connection.staticSubnetMask?a.connection.staticSubnetMask:a.connection.subnetMask);a.connection.ipAddress&&(a.initialIpAddress=a.connection.ipAddress,a.ipAddress=
splitIpArr(a.connection.ipAddress),a.subnetMask=splitIpArr(a.connection.subnetMask),null!=a.connection.defaultGateway&&(a.defaultGateway=splitIpArr(a.connection.defaultGateway)),a.connection.primaryDnsServer&&(a.primaryDnsServer=splitIpArr(a.connection.primaryDnsServer),a.secondaryDnsServer=splitIpArr(a.connection.secondaryDnsServer)));1==a.connection.connectionId&&(a.oldCoaxPassword=a.connection.coaxPassword);a.bridges=null;a.connection.availableBridges&&(a.bridges=[],a.allConnections=f.query(function(){for(var b=
0;b<a.connection.availableBridges.length;b++)for(var c=0;c<a.allConnections.length;c++)if(a.allConnections[c].connectionId==a.connection.availableBridges[b]){a.bridges.push(a.allConnections[c]);a.bridges[b].fromString=""===a.fromString?"?from=edit&connEdit="+a.connection.connectionId:a.fromString+"-edit&connEdit="+a.connection.connectionId;a.bridges[b].styleName=a.getStyleName(a.bridges[b].connectionId);a.bridges[b].bridged=a.connection.bridged[b];a.bridges[b].name=unescape(a.bridges[b].name);a.bridges[b].hasConnectedDeviceNow=
a.checkForDevicesNow(p,a.bridges[b].connectionId);break}}));a.dmz=m.get(function(){});a.connections=f.query(function(){var b=0;a.listOfNetworkIPAddress=[];for(var d=0;d<a.connections.length;d++){if(null!=a.connections[d].bridged)for(var f=0;f<a.connections[d].bridged.length;f++)a.connections[d].bridged[f]&&(a.connections[a.connections[d].availableBridges[f]].name=2==a.connections[d].availableBridges[f]?unescape(a.connections[d].name)+" Wireless 802.11 2.4GHz Access Point":3==a.connections[d].availableBridges[f]?
unescape(a.connections[d].name)+" Wireless 802.11 5.0GHz Access Point":unescape(a.connections[d].name+" "+a.connections[a.connections[d].availableBridges[f]].name));a.connections[d].name=unescape(a.connections[d].name);0==a.connections[d].connectionId&&(a.coTempHomeName=unescape(a.connections[d].name),a.coTempIPAddress=a.connections[d].ipAddress);if(0==a.connections[d].connectionId||1==a.connections[d].connectionId)f={},f.id=a.connections[d].connectionId,f.ip=a.connections[d].ipAddress,f.name=unescape(a.connections[d].name),
a.listOfNetworkIPAddress.push(f);a.connections[d].hasOwnProperty("connectionUsed")||(a.routesConnections[b]=a.connections[d],b++)}if(a.connection.dhcpSettings){a.dhcpOptions=e.query();a.dhcpStartIpAddress=splitIpArr(a.connection.dhcpSettings.startIpAddress);a.dhcpEndIpAddress=splitIpArr(a.connection.dhcpSettings.endIpAddress);a.initialDhcpEndAddress=a.connection.dhcpSettings.endIpAddress;b=a.connection.dhcpSettings.winsServer;0==b.length&&(b="0.0.0.0");a.dhcpWinsServer=splitIpArr(b);var h=g.query({id:c.id},
function(){for(var b=0;b<h.length;b++)1==h[b].ipAddressSource&&a.staticConnections.push(h[b].ipAddress)})}0!=c.id&&1!=c.id||a.refreshRoutes()})})});a.enableCoax=function(){f.update({id:c.id},{coaxEnabled:!0},function(){a.connection=f.get({id:c.id})})};a.disableCoax=function(){f.update({id:c.id},{coaxEnabled:!1},function(){a.connection=f.get({id:c.id})})};a.dhcpRelease=function(){f.save({id:c.id},{dhcpRelease:!0},function(){a.connection=f.get({id:c.id})})};a.dhcpRenew=function(){f.save({id:c.id},{dhcpRenew:!0},
function(){a.connection=f.get({id:c.id})})};a.errorPageOk=function(){a.showErrorPage=!1;window.scrollTo(0,0)};a.checkMsg=function(a,b){for(var c=!1,d=0;d<a.length;d++)if(a[d]==b){c=!0;break}return c};a.checkMsg=function(a,b){for(var c=!1,d=0;d<a.length;d++)if(a[d]==b){c=!0;break}return c};a.save=function(){a.errorMessages=[];a.messages=[];if(2==a.connection.mtuType){var b=!1;validRange(a.connection.mtu,68,1500)?checkForWholeNumbers(a.connection.mtu)&&(b=!0):b=!0;b&&a.errorMessages.push("MTU: The maximum transfer unit of "+
a.connection.name+" should be between 68 and 1500. ")}var e=b=!1,f=!1,g=!1,h=!1,m=!1,k=!1,p="",n=!0;if(a.connection.ipAddress){if(2==a.connection.protocolUsed){var l=!1,x=!1;validateIp(a.ipAddress)?"0.0.0.0"==getNumIpString(a.ipAddress)&&(a.errorMessages.push("IP address may not be 0.0.0.0. IP address must consist of 4 fields. Each field's value must be between 0 and 255, without any spaces. At least one of the values must be greater than 0."),x=l=!0):(a.errorMessages.push("IP Address: IP address must consist of 4 fields. Each field's value must be between 0 and 255, without any spaces."),
x=l=!0);if(223<a.ipAddress[0]||0==a.ipAddress[0]||l)a.errorMessages.push("IP Address: Device IP address should start with a value between 1-223."),b=!0;0==a.connection.connectionId&&x&&a.errorMessages.push("IP Address already in use: IP address item is already in use by Port Forwarding.");validateIp(a.ipAddress)&&checkAllNetworks(a.listOfNetworkIPAddress,a.ipAddress,a.connection.connectionId)&&(a.errorMessages.push("IP Address Already Exists: IP Address address "+getNumIpString(a.ipAddress)+" is already in use by "+
checkAllNetworks4Name(a.listOfNetworkIPAddress,a.ipAddress,a.connection.connectionId)+"."),e=!0);b||e||!checkFor127(a.ipAddress)||(a.errorMessages.push("IP Address: IP addresses starting with 127 are not valid because they are reserved for loopback addresses."),h=!0)}x=l=!1;if(2==a.connection.protocolUsed||1==a.connection.protocolUsed)if(x=!0,1!=a.connection.protocolUsed||a.connection.overrideSubnetMask||(x=!1),x){if(!validateIp(a.subnetMask)||checkFullListOfSubnetMaskRange(a.subnetMask))"0.0.0.0"!=
getIpString(a.subnetMask)&&(l=!0,1==a.connection.connectionId&&a.connection.overrideSubnetMask&&(l=!1),l?a.errorMessages.push("Subnet Mask: Subnet mask must consist of 4 fields. Each field's value must be between 0 and 255, constructing a contiguous string of binary ones. The default mask value is 255.255.255.0."):a.errorMessages.push("Override Subnet Mask: Subnet mask must consist of 4 fields. Each field's value must be between 0 and 255, constructing a contiguous string of binary ones. The default mask value is 255.255.255.0.")),
l=!0}else validateIp(a.subnetMask)||("0.0.0.0"!=getIpString(a.subnetMask)&&(l=!0,1==a.connection.connectionId&&a.connection.overrideSubnetMask&&(l=!1),l?a.errorMessages.push("Subnet Mask: Subnet mask must consist of 4 fields. Each field's value must be between 0 and 255, constructing a contiguous string of binary ones. The default mask value is 255.255.255.0."):a.errorMessages.push("Override Subnet Mask: Subnet mask must consist of 4 fields. Each field's value must be between 0 and 255, constructing a contiguous string of binary ones. The default mask value is 255.255.255.0.")),
l=!0);0==a.connection.connectionId&&(checkForValidSubnetMaskRange(a.subnetMask)||l)&&(a.errorMessages.push("Subnet Mask: Subnet mask must be between 255.0.0.0 and 255.255.255.252."),l=!0);2==a.connection.protocolUsed&&a.defaultGateway&&!validateIp(a.defaultGateway)&&(a.errorMessages.push("Default Gateway: IP address must consist of 4 fields. Each field's value must be between 0 and 255, without any spaces."),n=!1,m=!0);!b&&!1==l&&2==a.connection.protocolUsed&&validateIp(a.ipAddress)&&"0.0.0.0"!=getNumIpString(a.ipAddress)&&
(checkHostVsSubmask(a.ipAddress,a.subnetMask)?(p="IP Address and Subnet Mask combination: The combination of IP address and subnet mask is invalid. All of the bits in the host address portion of the IP address are set to "+checkBits(a.ipAddress)+".",a.errorMessages.push(p),k=!0):(l=compareIpMaskNetwork(a.ipAddress,a.subnetMask),"2"!=l&&(p="IP Address and Subnet Mask combination: The combination of IP address and subnet mask is invalid. All of the bits in the network address portion of the IP address are set to "+
l+".",a.errorMessages.push(p),k=!0)));if(2==a.connection.protocolUsed&&a.defaultGateway&&n&&!k&&validateIp(a.ipAddress)&&getNumIpString(a.ipAddress)==getNumIpString(a.defaultGateway)){if(""!=p){f=[];for(l=0;l<a.errorMessages.length;l++)p!=a.errorMessages[l]&&f.push(a.errorMessages[l]);a.errorMessages=f}a.errorMessages.push("Incorrect Gateway IP: The Gateway cannot have the same IP address as the specified Wireless Broadband Router IP address.");f=!0}a.connection.primaryDnsServer&&1==a.connection.dnsMode&&
...
0>a.connection.deviceMetric||255<a.connection.deviceMetric)&&a.errorMessages.push("Device Metric: Please enter a numeric value between 0 and 255.");p=getIpString(a.ipAddress);a.connection.dnsMode=parseInt(a.connection.dnsMode);x=n="";a.connection.primaryDnsServer&&(n=getIpString(a.primaryDnsServer),x=getIpString(a.secondaryDnsServer));var B=parseInt(a.connection.protocolUsed),t=a.connection.subnetMask;if(2==B||1==B)t=getIpString(a.subnetMask);var A=a.connection.defaultGateway;a.defaultGateway&&(A=
...
a.errorMessages.push("Coax Link Password:  Only digits can be used for the number.")):a.connection.coaxPasswordManual=a.oldCoaxPassword);if(0==a.connection.type)"0.0.0.0"==t&&a.errorMessages.push((1==B&&a.connection.overrideSubnetMask?"Override ":"Subnet Mask: ")+"Subnet mask may not be 0.0.0.0. Subnet mask must consist of 4 fields. Each field's value must be between 0 and 255, constructing a contiguous string of binary ones. The default mask value is 255.255.255.0.");else if(1==a.connection.type){var C=
a.subnetMask;a.connection.overrideSubnetMask||1!=B||(C=splitIpArr(t));if((2==B||1==B&&a.connection.overrideSubnetMask)&&validateIp(C)){for(var D=!0,F=!1,l=0;4>l;l++){for(var E=7;-1<E;E--){var G=1==(C[l]>>E&1);if(G&&F){D=!1;break}G||(F=!0)}if(!D)break}if(!D)l=!0,1==a.connection.connectionId&&a.connection.overrideSubnetMask&&(l=!1),m="",m=l?"Subnet Mask: Subnet mask must consist of 4 fields. Each field's value must be between 0 and 255, constructing a contiguous string of binary ones. The default mask value is 255.255.255.0.":
"Override Subnet Mask: Subnet mask must consist of 4 fields. Each field's value must be between 0 and 255, constructing a contiguous string of binary ones. The default mask value is 255.255.255.0.",a.checkMsg(a.errorMessages,m)||a.errorMessages.push(m);else if(2==B&&a.connection.defaultRoute&&"0.0.0.0"==A)a.errorMessages.push("Default Gateway: The Default Gateway must not be 0.0.0.0 while the Default Route is set.");else if(2==B&&"0.0.0.0"!=A){if(validateIp(a.ipAddress)&&!(b||e||h||k||m))for(l=0;4>
l;l++)if((C[l]&a.defaultGateway[l])!=(a.subnetMask[l]&a.ipAddress[l])){a.errorMessages.push("Wireless Broadband Router and Default Gateway Are Not on the Same Subnet: Under the given Netmask, the IP Address does not belong to the same Subnet as the Default Gateway.");g=!0;break}g||(checkHostVsSubmask(a.defaultGateway,a.subnetMask)?(a.errorMessages.push("Wireless Broadband Router and Default Gateway Are Not on the Same Subnet: Under the given Netmask, the IP Address does not belong to the same Subnet as the Default Gateway."),
g=!0):(l=compareIpMaskNetwork(a.defaultGateway,a.subnetMask),"2"!=l&&(a.errorMessages.push("Wireless Broadband Router and Default Gateway Are Not on the Same Subnet: Under the given Netmask, the IP Address does not belong to the same Subnet as the Default Gateway."),g=!0)))}"0.0.0.0"==t&&(l=!0,1==a.connection.connectionId&&a.connection.overrideSubnetMask&&(l=!1),l?a.errorMessages.push("Subnet Mask: Subnet mask may not be 0.0.0.0. Subnet mask must consist of 4 fields. Each field's value must be between 0 and 255, constructing a contiguous string of binary ones. The default mask value is 255.255.255.0."):
a.errorMessages.push("Override Subnet Mask: Subnet mask may not be 0.0.0.0. Subnet mask must consist of 4 fields. Each field's value must be between 0 and 255, constructing a contiguous string of binary ones. The default mask value is 255.255.255.0."))}}0==a.connection.protocolUsed||1!=a.connection.dnsMode||1!=a.connection.connectionId||!a.connection.primaryDnsServer||b||e||f||g||h||(p==n&&1!=a.connection.protocolUsed?a.errorMessages.push("Primary DNS Server: DNS Server cannot have the IP address of Wireless Broadband Router "+
unescape(a.connection.name)+"."):n==a.coTempIPAddress&&a.errorMessages.push("Primary DNS Server: DNS Server cannot have the IP address of Wireless Broadband Router "+a.coTempHomeName+"."),p==x&&1!=a.connection.protocolUsed?a.errorMessages.push("Secondary DNS Server: DNS Server cannot have the IP address of Wireless Broadband Router "+unescape(a.connection.name)+"."):x==a.coTempIPAddress&&a.errorMessages.push("Secondary DNS Server: DNS Server cannot have the IP address of Wireless Broadband Router "+
a.coTempHomeName+"."),"0.0.0.0"!=n&&n==x&&a.errorMessages.push("Redundant DNS Entries: 'Primary' and 'Secondary' values may not point to the same DNS server."),"255.255.255.255"==n&&a.errorMessages.push("Primary DNS Server: "+n+" is invalid."),"255.255.255.255"==x&&a.errorMessages.push("Secondary DNS Server: "+x+" is invalid."));if(0!=a.errorMessages.length){a.showErrorPage=!0;window.scrollTo(0,0);return}2==a.connection.protocolUsed?(a.connection.ipAddress=p,a.defaultGateway&&(a.connection.defaultGateway=
A)):a.defaultGateway&&(a.connection.defaultGateway=null);void 0!==a.connection.routingMode&&(a.connection.routingMode=parseInt(a.connection.routingMode));a.connection.primaryDnsServer=n;a.connection.secondaryDnsServer=x;a.connection.protocolUsed=B;a.connection.subnetMask=t;0==c.id&&1==a.connection.protocolUsed&&2==a.oldProtocolUsed&&(a.messages.push({type:"DHCP Client:",data:"Configuring an internal connection to obtain an IP address automatically might make the connection inaccessible in case no IP address is obtained."}),
...
splitIpArr(a.connection.ipAddress);e=splitIpArr(a.connection.subnetMask);a.connection.dhcpSettings.mode=parseInt(a.connection.dhcpSettings.mode);if(2==a.connection.dhcpSettings.mode){if(a.connection.ipAddress!=a.initialIpAddress&&getIpString(a.dhcpEndIpAddress)==a.initialDhcpEndAddress)for(l=0;4>l;l++)a.dhcpStartIpAddress[l]=b[l]&e[l]|a.dhcpStartIpAddress[l]&~e[l],a.dhcpEndIpAddress[l]=b[l]&e[l]|a.dhcpEndIpAddress[l]&~e[l];for(l=0;l<a.staticConnections.length;l++)if(!isInRange(a.dhcpStartIpAddress,
a.dhcpEndIpAddress,splitIpArr(a.staticConnections[l]))){a.messages.push({type:"Static Leases:",data:"The Static lease configured IP's are no longer valid and requires manual UI updating to reflect the new IP configuration."});break}a.errorMessages=validateIPv4DhcpSettings(b,a.dhcpStartIpAddress,a.dhcpEndIpAddress,e,a.dhcpWinsServer,a.connection.dhcpSettings.leaseTime,a.connection.connectionId,a.connection.protocolUsed,a.connection.overrideSubnetMask,a.listOfNetworkIPAddress);if(0!=a.errorMessages.length){a.showErrorPage=
!0;window.scrollTo(0,0);return}}a.connection.dhcpSettings.mode=parseInt(a.connection.dhcpSettings.mode);a.connection.dhcpSettings.startIpAddress=getIpString(a.dhcpStartIpAddress);a.connection.dhcpSettings.endIpAddress=getIpString(a.dhcpEndIpAddress);validateIp(a.dhcpWinsServer)?(b=getIpString(a.dhcpWinsServer),a.connection.dhcpSettings.winsServer="0.0.0.0"!=b?b:""):a.connection.dhcpSettings.winsServer=""}if(a.bridges)for(!a.connection.bridged[0]&&a.bridges[0].bridged&&a.messages.push({type:"Warning:",
data:"Changing this configuration setting will impact router performance, and negatively impact video services and features.When in the 'bridge mode', the router and broadband service cannot be supported by the Service provider. Proceed with caution."}),l=0;l<a.bridges.length;l++)a.connection.bridged[l]=a.bridges[l].bridged;a.connection.mtuType=parseInt(a.connection.mtuType);1==a.connection.connectionId&&(0==a.connection.protocolUsed?(a.connection.staticIpAddress="",a.connection.staticDefaultGateway=
"",a.connection.staticPrimaryDnsServer="",a.connection.staticSecondaryDnsServer="",a.connection.staticSubnetMask=""):(1==a.connection.protocolUsed?1==a.connection.dnsMode?(a.connection.staticIpAddress="",a.connection.staticDefaultGateway="",a.connection.staticPrimaryDnsServer=a.connection.primaryDnsServer,a.connection.staticSecondaryDnsServer=a.connection.secondaryDnsServer):(a.connection.staticIpAddress="",a.connection.staticDefaultGateway="",a.connection.staticPrimaryDnsServer="",a.connection.staticSecondaryDnsServer=
""):2==a.connection.dnsMode?(a.connection.staticIpAddress=a.connection.ipAddress,a.connection.staticDefaultGateway=a.connection.defaultGateway,a.connection.staticPrimaryDnsServer="",a.connection.staticSecondaryDnsServer=""):(a.connection.staticIpAddress=a.connection.ipAddress,a.connection.staticDefaultGateway=a.connection.defaultGateway,a.connection.staticPrimaryDnsServer=a.connection.primaryDnsServer,a.connection.staticSecondaryDnsServer=a.connection.secondaryDnsServer),a.connection.staticSubnetMask=
a.connection.subnetMask),a.connection.ipAddress=a.tempStatic.ipAddress,a.connection.defaultGateway=a.tempStatic.defaultGateway,1!=a.connection.dnsMode&&(a.connection.primaryDnsServer=a.tempStatic.primaryDnsServer,a.connection.secondaryDnsServer=a.tempStatic.secondaryDnsServer),a.connection.subnetMask=a.tempStatic.subnetMask);if(0!=a.messages.length&&systemSettings.warnBeforeChanges)a.confirm=!0,window.scrollTo(0,0);else{var H=a.connection.connectionId;a.connection.$update(function(){d.path("/network/connections/"+
...
break}63<a.device.name.length?a.errorMessages.push("New Name: Name may not contain spaces. Only letters, digits and the special characters dash (-), underscore (_) and dot (.) may be used. These special characters may not appear at the beginning or at the end of a name. The maximum length of a label (text between two dots) is 63."):validateHostname(a.device.name)||/^\d+$/.test(a.device.name)||a.errorMessages.push("New Name: Name may not contain spaces. Only letters, digits and the special characters dash (-), underscore (_) and dot (.) may be used. These special characters may not appear at the beginning or at the end of a name. The maximum length of a label (text between two dots) is 63.");
...
function(){d.path("/network/connections")};a.coax=e.get({id:1})}]).controller("LanCoaxStatsCtrl",["$scope","$route","$routeParams","$location","NetworkConnections",function(a,b,c,d,e){a.tab=b.current.tab;a.section=b.current.section;a.checkNetworkConnections=function(){d.path("/network/connections")};a.coax=e.get({id:5})}]);angular.module("parental.controllers",[]).controller("ParentalAddCtrl",["$scope","$route","$routeParams","$location","Parental","Devices","SystemSettings","GuestDevices",function(a,b,c,d,e,f,g,h){a.tab=b.current.tab;a.section=b.current.section;a.showErrorPage=!1;a.days=[!1,!1,!1,!1,!1,!1,!1];a.activeType=0;a.startHour=1;a.startMinute=0;a.startTime=0;a.endHour=1;a.endMinute=0;a.endTime=0;a.selectedAvailable=[];a.selectedUsed=[];a.availableDevices=[];a.usedDevices=[];a.embeddedKeyword="";a.website=
"";a.blocks=[];a.blockedItems=[];a.pageToShow=0;a.hasParentalRules=!1;a.sysSettings=g.get();a.togglePage=function(b){a.pageToShow=b;window.scrollTo(0,0)};a.changeDisplayNum=function(a){return 1<String(a).length?a:"0"+a};a.$watch("parentalControl.accessType",function(){a.checkAccessValue()});a.checkAccessValue=function(){if("Block_Selected"==a.parentalControl.name||"Allow_Only_Selected"==a.parentalControl.name||"Block_All"==a.parentalControl.name)0==a.parentalControl.accessType?a.parentalControl.name=
...
hourRanges:[{startTimeHour:0,startTimeMinute:0,endTimeHour:0,endTimeMinute:0}]}]},accessType:0,restrictions:[],hosts:[],status:!0};var k=[],m=e.query(function(){k=f.query(function(){for(var b=0;b<k.length;b++)if(6!=k[b].connectionType){for(var d=!1,e=0;e<m.length;e++)for(var f=m[e],g=0;g<f.hosts.length;g++)if(f.hosts[g]==k[b].mac){d=!0;break}d||k[b].isStb||a.availableDevices.push(k[b].name)}for(b=0;b<k.length;b++)if(6==k[b].connectionType&&k[b].approved){d=!1;for(e=0;e<m.length;e++)for(f=m[e],g=0;g<
f.hosts.length;g++)if(f.hosts[g]==k[b].mac){d=!0;break}d||k[b].isStb||a.availableDevices.push(k[b].name)}if(void 0!==c.id&&m.length>c.id){f=m[c.id];a.parentalControl.accessType=f.accessType;a.parentalControl.name=f.name;a.parentalControl.description=f.description;a.parentalControl.schedule.activeDuring=f.schedule.activeDuring;for(g=0;g<f.schedule.timeSegments[0].daysOfTheWeek.length;g++)a.days[f.schedule.timeSegments[0].daysOfTheWeek[g]]=!0;b=f.schedule.timeSegments[0].hourRanges[0].startTimeHour;
...
2!=a.parentalControl.accessType&&0==a.parentalControl.restrictions.length&&a.errorMessages.push("Filter Websites or Keywords: List cannot be empty.");for(var f=!1,g=0;g<a.days.length;g++)a.days[g]&&(f=!0);f||a.errorMessages.push("Days of Week: At least one day must be selected.");if(0<a.errorMessages.length)a.showErrorPage=!0,window.scrollTo(0,0);else{for(g=0;g<a.usedDevices.length;g++)for(f=0;f<k.length;f++)if(k[f].name==a.usedDevices[g]){a.parentalControl.hosts.push(k[f].mac);break}for(g=0;g<a.days.length;g++)a.days[g]&&
...
c.id?e.update({id:c.id},a.parentalControl,function(){d.search({});d.path("/parental/summary");window.scrollTo(0,0)}):a.hasParentalRules?a.cfmApplySettings():!0==a.sysSettings.warnBeforeChanges?a.togglePage(1):a.cfmApplySettings()}};a.keyPress=function(b){13==b.keyCode&&"/parental"==d.path()&&a.applySettings()}}]).controller("ParentalSummaryCtrl",["$scope","$route","$location","Devices","Parental","SystemSettings",function(a,b,c,d,e,f){a.tab=b.current.tab;a.section=b.current.section;a.viewingRule=
-1;a.pageToShow=0;a.selectedIndex=-1;a.enableRuleList=[];a.sysSettings=f.get();a.togglePage=function(b){a.pageToShow=b;window.scrollTo(0,0)};a.refresh=function(){a.enableRuleList=[];a.devices=d.query(function(){a.rules=e.query(function(){for(var b=0;b<a.rules.length;b++){for(var c=0;c<a.rules[b].hosts.length;c++)for(var d=a.rules[b].hosts[c],e=0;e<a.devices.length;e++)if(d==a.devices[e].mac){a.rules[b].hosts[c]=a.devices[e].name;break}a.rules[b].selected=a.rules[b].hosts[0]}})})};a.togglePage=function(b){a.pageToShow=
b;window.scrollTo(0,0)};a.refresh();a.editRule=function(a){c.path("/parental").search({id:a})};a.viewRule=function(b){a.viewingRule=b;a.selected=a.rules[b];a.selectedRestrictions=[];for(b=0;b<a.selected.restrictions.length;b++){var c=a.selected.restrictions[b];switch(c.type){case 0:a.selectedRestrictions.push("Website:"+c.value);break;default:a.selectedRestrictions.push("Keyword:"+c.value)}}b=[];printSchedule(0,[a.selected.schedule],b);a.scheduleString=b[0][0]};a.cfmDeleteControl=function(){e.remove({id:a.selectedIndex},
function(){a.togglePage(0);a.refresh()})};a.deleteRule=function(b){a.selectedIndex=b;!0==a.sysSettings.warnBeforeChanges?a.togglePage(1):a.cfmDeleteControl()};a.close=function(){a.viewingRule=-1};a.setSelected=function(b){var c=!1;a.rules[b].enabled=a.rules[b].enabled?!1:!0;if(null==a.enableRuleList||void 0==a.enableRuleList||0==a.enableRuleList.length)a.enableRuleList.push({id:b,rule:a.getMacHostsRule(a.rules[b])});else{for(var d=0;d<a.enableRuleList.length;d++)if(a.enableRuleList[d].id==b){a.enableRuleList[d].rule=
a.getMacHostsRule(a.rules[b]);c=!0;break}c||a.enableRuleList.push({id:b,rule:a.getMacHostsRule(a.rules[b])})}};a.getMacHostsRule=function(b){b=angular.copy(b);for(var c,d=0;d<b.hosts.length;d++){c=b.hosts[d];for(var e=0;e<a.devices.length;e++)if(c==a.devices[e].name||c==a.devices[e].mac){b.hosts[d]=a.devices[e].mac;break}}return b};a.restSelection=function(){a.refresh();a.togglePage(0)};a.applySettings=function(){if(null!=a.enableRuleList&&0!=a.enableRuleList.length){for(var b=0;b<a.enableRuleList.length;b++)e.update({id:a.enableRuleList[b].id},
...
a[e].approved&&"5"==String(d)&&!0==a[e].status){c=!0;break}}return c}return!1};a.initialLoad=!0;a.refresh=function(){if(a.initialLoad||a.autoRefresh){a.initialLoad=!1;var b,d=k.query(function(){b=c.query(function(){a.connections=b;for(var c=0;c<a.connections.length;c++)if(void 0!==a.connections[c].ipv6Address&&"::"!==a.connections[c].ipv6Address&&""!==a.connections[c].ipv6Address&&(a.showIpv6=!0),a.connections[c].displayName=unescape(a.connections[c].name),a.connections[c].hasConnectedDeviceNow=a.checkForDevicesNow(d,
a.connections[c].connectionId),a.connections[c].underlyingDevicesTempList=[],a.connections[c].underlyingDevices&&a.connections[c].availableBridges)for(var e=0;e<a.connections[c].availableBridges.length;e++)for(var f=0;f<a.connections[c].underlyingDevices.length;f++)if(a.connections[c].availableBridges[e]==a.connections[c].underlyingDevices[f]&&!0==a.connections[c].bridged[e]){a.connections[c].underlyingDevicesTempList.push(a.connections[c].availableBridges[e]);break}a.autoRefresh&&a.valid&&(a.timer=
setTimeout(function(){a.refresh()},1E4))})})}};a.toggleAutoRefresh=function(){clearTimeout(a.timer);a.systmSetngs=g.get();a.autoRefresh=!a.autoRefresh;a.systmSetngs.autoRefreshSystemMonitoring=a.autoRefresh;a.systmSetngs.$update(function(){systemSettings=a.systmSetngs;a.autoRefresh&&a.refresh()})};a.close=function(){null!=f.frommain?d.url(d.path("/main")):(d.path("/monitoring/advanced"),window.scrollTo(0,0))};a.refresh()}]).controller("BandwidthCtrl",["$scope","$route","NetworkConnections","SystemSettings",
"$location",function(a,b,c,d,e){a.tab=b.current.tab;a.section=b.current.section;a.valid=!0;a.timer=null;a.autoRefresh=systemSettings.autoRefreshSystemMonitoring;a.$on("$destroy",function(){a.valid=!1;clearInterval(a.timer)});a.initialLoad=!0;a.refresh=function(){if(a.initialLoad||a.autoRefresh){a.initialLoad=!1;var b=c.get({id:1},function(){a.bb=b;a.autoRefresh&&a.valid&&(a.timer=setTimeout(function(){a.refresh()},1E4))})}};a.toggleAutoRefresh=function(){clearTimeout(a.timer);a.systmSetngs=d.get();
a.autoRefresh=!a.autoRefresh;a.systmSetngs.autoRefreshSystemMonitoring=a.autoRefresh;a.systmSetngs.$update(function(){systemSettings=a.systmSetngs;a.autoRefresh&&a.refresh()})};a.refresh()}]).controller("SystemLogCtrl",["$scope","$route","$http","Log","SystemSettings",function(a,b,c,d,e){a.sysSettings=e.get();a.tab=b.current.tab;a.section=b.current.section;a.canClearLog=!0;a.logType="System";a.enableLogging=systemSettings.enableLogging;setupLoggingCtrl(a,0,c,d)}]).controller("SecurityLogCtrl",["$scope",
"$route","$http","Log","SystemSettings",function(a,b,c,d,e){a.sysSettings=e.get();a.tab=b.current.tab;a.section=b.current.section;a.logType="Security";a.canClearLog=!0;a.enableLogging=systemSettings.enableLogging;setupLoggingCtrl(a,1,c,d)}]).controller("AdvancedLogCtrl",["$scope","$route","$http","Log","SystemSettings",function(a,b,c,d,e){a.sysSettings=e.get();a.tab=b.current.tab;a.section=b.current.section;a.logType="Advanced";a.canClearLog=!0;a.enableLogging=systemSettings.enableLogging;setupLoggingCtrl(a,
2,c,d)}]).controller("FirewallLogCtrl",["$scope","$route","$http","Log","SystemSettings",function(a,b,c,d,e){a.tab=b.current.tab;a.section=b.current.section;a.logType="Firewall";a.enableLogging=systemSettings.enableLogging;setupLoggingCtrl(a,3,c,d)}]).controller("WandhcpLogCtrl",["$scope","$route","$http","Log","SystemSettings",function(a,b,c,d,e){a.tab=b.current.tab;a.section=b.current.section;a.logType="WAN DHCP";a.enableLogging=systemSettings.enableLogging;setupLoggingCtrl(a,4,c,d)}]).controller("LandhcpLogCtrl",
...
function validateIp(a){if(4!=a.length)return!1;for(var b=0;b<a.length;b++)if(""==String(a[b])||null==a[b]||isNaN(a[b])||isNotaInt(a[b])||0>a[b]||255<a[b])return!1;return!0}function isNotaInt(a){return null!=a&&void 0!=a?Number(a)===parseInt(Number(a))?!1:!0:!0}function validateMulticastAddr(a){return validateIp(a)?224<=a[0]&&239>=a[0]:!1}
function validateSubnetMask(a){if(!validateIp(a)||0==a[0])return!1;for(var b=!0,c=!1,d=0;4>d;d++){for(var e=7;-1<e;e--){var f=1==(a[d]>>e&1);if(f&&c){b=!1;break}f||(c=!0)}if(!b)break}return b}function validateMac(a){return/^[0-9A-F]{12}$/i.test(a.replace(RegExp(":","g"),""))}
function validateWepSettings(a,b,c,d){if(b||""!==a.key)switch(a.keyLength){case 0:switch(a.entryMethod){case 0:10==a.key.length?/[0-9A-F]{10}$/i.test(a.key)||d.push(c+": Invalid WEP Key. Allowable values are 0-9 and a-f."):d.push(c+": The WEP key must be a hexadecimal number of 10 digits.");break;case 1:5!=a.key.length?d.push(c+": The WEP key must be a string of 5 characters."):/[\x21-\x7E]{5}$/i.test(a.key)||d.push(c+": Invalid WEP Key. Only ASCII characters are allowed.")}break;case 1:switch(a.entryMethod){case 0:26==
a.key.length?/[0-9A-F]{26}$/i.test(a.key)||d.push(c+": Invalid WEP Key. Allowable values are 0-9 and a-f."):d.push(c+": The WEP key must be a hexadecimal number of 26 digits.");break;case 1:13!=a.key.length?d.push(c+": The WEP key must be a string of 13 characters."):/[\x21-\x7E]{13}$/i.test(a.key)||d.push(c+": Invalid WEP Key. Only ASCII characters are allowed.")}}}function validRange(a,b,c){return!(isNaN(a)||a<b||a>c)}
function validateTransmissionSettings(a,b,c){!isNaN(a.beaconInterval)&&null!=a.beaconInterval&&checksForDecimalNumber(a.beaconInterval)&&checksForWholeNumber(a.beaconInterval)?(20>a.beaconInterval||1E3<a.beaconInterval)&&c.push(b+" Beacon Interval:  Beacon interval out of range (from=20, to=1000)"):c.push(b+" Beacon Interval: Please enter a numeric value.");!isNaN(a.dtimInterval)&&null!=a.dtimInterval&&checksForDecimalNumber(a.dtimInterval)&&checksForWholeNumber(a.dtimInterval)?(1>a.dtimInterval||
255<a.dtimInterval)&&c.push(b+" DTIM Interval:  DTIM interval out of range (from=1, to=255)"):c.push(b+" DTIM Interval: Please enter a numeric value.");!isNaN(a.fragmentationThreshold)&&null!=a.fragmentationThreshold&&checksForDecimalNumber(a.fragmentationThreshold)&&checksForWholeNumber(a.fragmentationThreshold)?(256>a.fragmentationThreshold||2346<a.fragmentationThreshold)&&c.push(b+" Fragmentation Threshold:  Fragmentation threshold out of range (from=256, to=2346)"):c.push(b+" Fragmentation Threshold: Please enter a numeric value.");
null!=a.rtsThreshold&&""!==a.rtsThreshold&&!isNaN(a.rtsThreshold)&&checksForDecimalNumber(a.rtsThreshold)&&checksForWholeNumber(a.rtsThreshold)?(0>a.rtsThreshold||2347<a.rtsThreshold)&&c.push(b+" RTS Threshold:  RTS Threshold out of range (from=0, to=2347)"):c.push(b+" RTS Threshold: Please enter a numeric value.");!isNaN(a.frameBurstMax)&&checksForDecimalNumber(a.frameBurstMax)&&checksForWholeNumber(a.frameBurstMax)?(2>a.frameBurstMax||255<a.frameBurstMax)&&c.push(b+" Frame Burst - Max Number: Max number of frames in a burst is out of range (from=2, to=255)"):
c.push(b+" Frame Burst - Max Number: Please enter a numeric value.");null!=a.frameBurstTime&&""!==a.frameBurstTime&&!isNaN(a.frameBurstTime)&&checksForDecimalNumber(a.frameBurstTime)&&checksForWholeNumber(a.frameBurstTime)?(0>a.frameBurstTime||1023<a.frameBurstTime)&&c.push(b+" Frame Burst - Burst Time: Burst time out of range (from=0, to=1023)"):c.push(b+" Frame Burst - Burst Time: Please enter a numeric value.");!isNaN(a.power)&&checksForDecimalNumber(a.power)&&checksForWholeNumber(a.power)?(1>
a.power||100<a.power)&&c.push(b+" Transmit Power: Please enter a numeric value between 1 and 100."):c.push(b+" Transmit Power: Please enter a numeric value.")}function isInRange(a,b,c){for(var d=0;4>d;d++)if(a[d]>c[d]||b[d]<c[d])return!1;return!0}
function validateIPv4DhcpSettings(a,b,c,d,e,f,g,h,k,m){var p=[],q=getSubnetID(a,d),z=angular.copy(q);z[3]=100;q=angular.copy(q);q[3]=150;validateIp(a)||p.push("Invalid IP Address provided. Values must be between 0 and 255");validateIp(b)||p.push("Start IP Address: IP address must consist of 4 fields. Each field's value must be between 0 and 255, without any spaces.");validateIp(c)||p.push("End IP Address: IP address must consist of 4 fields. Each field's value must be between 0 and 255, without any spaces.");
validateIp(a)&&null!=m&&checkAllNetworks(m,a,g)&&$scope.errorMessages.push("IP Address Already Exists: IP Address address "+getNumIpString(a)+" is already in use by "+checkAllNetworks4Name(m,a,g)+".");var w=m=!1;if(2==h||1==h)if(w=!0,1!=h||k||(w=!1),w){if(!validateIp(d)||checkFullListOfSubnetMaskRange(d))"0.0.0.0"!=getIpString(d)&&p.push("Subnet Mask: Subnet mask must consist of 4 fields. Each field's value must be between 0 and 255, constructing a contiguous string of binary ones. The default mask value is 255.255.255.0."),
m=!0}else validateIp(d)||("0.0.0.0"!=getIpString(d)&&p.push("Subnet Mask: Subnet mask must consist of 4 fields. Each field's value must be between 0 and 255, constructing a contiguous string of binary ones. The default mask value is 255.255.255.0."),m=!0);isIPWithinRange(b,z,q)&&isIPWithinRange(c,z,q)&&p.push("Start IP Address, End IP Address: IP addresses between "+getIpString(z)+" and "+getIpString(q)+" are used for FiOS TV. Please select a start and/or end IP address outside of this reserved range.");
0!=g||!checkForValidSubnetMaskRange(d)&&!0!=m||(p.push("Subnet Mask: Subnet mask must be between 255.0.0.0 and 255.255.255.252."),m=!0);!1==m&&2==h&&validateIp(a)&&"0.0.0.0"!=getNumIpString(a)&&(checkHostVsSubmask(a,d)?p.push("IP Address and Subnet Mask combination: The combination of IP address and subnet mask is invalid. All of the bits in the host address portion of the IP address are set to "+checkBits(a)+"."):(g=compareIpMaskNetwork(a,d),"2"!=g&&p.push("IP Address and Subnet Mask combination: The combination of IP address and subnet mask is invalid. All of the bits in the network address portion of the IP address are set to "+
g+".")));(!validateIp(e)||255<e[0]||255<e[1]||255<e[2]||255<e[3])&&p.push("WINS Server: IP address must consist of 4 fields. Each field's value must be between 0 and 255, without any spaces.");(isNaN(f)||1>f||43200<f||0!==f%1)&&p.push("Lease Time in Minutes: Please enter a numeric value between 1 and 43200.");"0.0.0.0"==getIpString(b)?p.push("Start IP Address: IP address may not be 0.0.0.0. IP address must consist of 4 fields. Each field's value must be between 0 and 255, without any spaces. At least one of the values must be greater than 0."):
checkHostVsSubmask(b,d)&&"1"==checkBits(b)&&p.push("Start IP Address: "+b[0]+"."+b[1]+"."+b[2]+"."+b[3]+" isn't a valid address");"0.0.0.0"==getIpString(c)?p.push("End IP Address:  IP address may not be 0.0.0.0. IP address must consist of 4 fields. Each field's value must be between 0 and 255, without any spaces. At least one of the values must be greater than 0."):checkHostVsSubmask(c,d)&&"1"==checkBits(c)&&p.push("End IP Address: "+c[0]+"."+c[1]+"."+c[2]+"."+c[3]+" isn't a valid address");if(0<
p.length)return p;h=g=f=!0;e=!1;if((a[0]&d[0])!=(b[0]&d[0])||(a[1]&d[1])!=(b[1]&d[1])||(a[2]&d[2])!=(b[2]&d[2])||(a[3]&d[3])!=(b[3]&d[3]))g=!1;if((a[0]&d[0])!=(c[0]&d[0])||(a[1]&d[1])!=(c[1]&d[1])||(a[2]&d[2])!=(c[2]&d[2])||(a[3]&d[3])!=(c[3]&d[3]))h=!1;g&&h?validateDHCPRange(b,c,a,d)||p.push("Start IP Address, End IP Address: The DHCP IP address range does not conform to the connection IP address and the DHCP subnet mask."):p.push((g?"":"Start IP Address")+(g||h?"":", ")+(h?"":"End IP Address")+
":  The DHCP IP address range does not conform to the connection IP address and the DHCP subnet mask.");if(255==d[0]&&255==d[1]&&255==d[2]&&255==d[3]||d[0]<d[1]||d[1]<d[2]||d[2]<d[3])f=!1;g=!1;for(h=0;4>h;h++){for(k=7;-1<k;k--){if((z=1==(d[h]>>k&1))&&g){f=!1;break}z||(g=!0)}if(!f)break}if(f){f=f=d=!1;for(g=0;4>g;g++){h=b[g];k=c[g];h=Number(h);k=Number(k);k>h&&(f=!0);if(h>k&&!f)break;h!=k&&(d=!0)}getIpString(b)===getIpString(c)&&(d=!0);d||(e=!0,p.push("IP Addresses: Start IP address is greater than End IP address."));
if(f=isIPWithinRange(a,b,c))e=!0,p.push("IP Addresses: The Device's IP address should not be in the range of the IP pool");e||checkIPRange(b,c)||p.push("The number of IP Addresses in the DHCP IP address range must not be more than 256.")}else p.push("Subnet Mask: Subnet mask must consist of 4 fields. Each field's value must be between 0 and 255, constructing a contiguous string of binary ones. The default mask value is 255.255.255.0.");return p}
function validateIPv6(a){a=a.split(":");if(3>a.length||8<a.length)return!1;for(var b=0,c=0;c<a.length;c++)if(""===a[c])b++;else if(!/^[a-f0-9]{1,4}$/i.test(a[c]))return!1;return 1==b?""!==a[a.length-1]&&""!==a[0]:2==b?""===a[a.length-1]&&""===a[a.length-2]||""===a[1]&&""===a[0]:3==b&&3==a.length?!0:2<b?!1:8==a.length}
...
function isScheduleActive(a,b){if(null===a||"Always"===a||""===a)return"Active";var c=["Active","Inactive"];a.activeDuring||(c=["Inactive","Active"]);var d=b.day();0==d&&(d=7);for(var d=d-1,e=b.hour(),f=b.minute(),g=0;g<a.timeSegments.length;g++)if(-1!=String(a.timeSegments[g].daysOfTheWeek).indexOf(d))for(var h=0;h<a.timeSegments[g].hourRanges.length;h++){var k=a.timeSegments[g].hourRanges[h].startTimeHour,m=a.timeSegments[g].hourRanges[h].endTimeHour,p=a.timeSegments[g].hourRanges[h].startTimeMinute,
q=a.timeSegments[g].hourRanges[h].endTimeMinute;if(k==m&&p<=q){if(k==e&&m==e&&p<=f&&q>f)return c[0]}else if(k<m){if(e>k&&e<m||k==e&&f>=p||m==e&&f<q)return c[0]}else if(k<e||k==e&&p<=f||m>e||m==e&&q>f)return c[0]}return c[1]}
...
1==a.wirelessA.security.wpa.type?1:2);a.wirelessB=a.wirelessInterfaces[1];a.w50wType=-1;a.wirelessB.security.enabled&&0!=a.wirelessB.security.type&&(a.w50wType=1==a.wirelessB.security.wpa.type?1:2)})}]).controller("WirelessBasicCtrl",["$scope","$route","Wireless","securityTips","$timeout","$location","WPS","$rootScope",function(a,b,c,d,e,f,g,h){a.tab=b.current.tab;a.section=b.current.section;a.errorMessages=[];a.showErrorPage=!1;a.saveChannel=!1;a.hasApplyChanges=!1;a.isW24WEPOn=!1;a.wpsIsEnabled=
!1;var k=c.query(function(){a.wirelessA=k[0];a.wirelessB=k[1];a.wirelessC=k[2];a.wirelessD=k[3];a.saveChannel=a.wirelessA.saveChannel;a.wepA={keyLength:a.wirelessA.security.wep.settings[a.wirelessA.security.wep.activeSetting].keyLength,entryMethod:a.wirelessA.security.wep.settings[a.wirelessA.security.wep.activeSetting].entryMethod,key:a.wirelessA.security.wep.settings[a.wirelessA.security.wep.activeSetting].key};a.wepB={keyLength:a.wirelessB.security.wep.settings[a.wirelessB.security.wep.activeSetting].keyLength,
entryMethod:a.wirelessB.security.wep.settings[a.wirelessB.security.wep.activeSetting].entryMethod,key:a.wirelessB.security.wep.settings[a.wirelessB.security.wep.activeSetting].key};a.wepChanged(a.wepA);a.wepChanged(a.wepB);a.oldAType=a.wirelessA.security.type;a.oldBType=a.wirelessB.security.type;a.wirelessA.ssid=a.wirelessA.ssid;a.wirelessB.ssid=a.wirelessB.ssid;a.wAtype=a.wirelessA.security.enabled?0==a.wirelessA.security.type?0:1:1;a.wBtype=a.wirelessB.security.enabled?0==a.wirelessB.security.type?
0:1:1;a.isW24WEPOn=a.wAtype;h.wpsLinkEnable=a.wirelessA.radioEnabled||a.wirelessB.radioEnabled?!0:!1;a.hasApplyChanges=!1;var b=g.get({id:0},function(){a.wpsIsEnabled=b.enabled;a.wpsIsEnabled=a.wpsIsEnabled?!0:!1})});a.wireless=k;a.rdbtnChange=function(b){1==b&&d.showWEPMessage().then(function(b){"cancel"!=b&&a.wirelessUpdate()})};a.errorPageOk=function(){a.showErrorPage=!1;window.scrollTo(0,0)};a.wepChanged=function(a){var b=10,b=0==a.keyLength?0==a.entryMethod?10:5:0==a.entryMethod?26:13;a.key.length>
b&&(a.key=a.key.substr(0,b));a.charactersLeft=b-a.key.length};a.validateSsid=function(b,c){(1>b.length||64<b.length||!/^[0-9a-zA-Z\!\"\#\$\%\&\u2019\'\(\)\*\+\,\-\.\/\:\;\<\=\>\?\@\[\\\]\^\_\`\{\|\}\~\ ]+$/i.test(b))&&a.errorMessages.push(c+" Service Set Identifier must consist of 1 to 64 characters and must not contain any special character.")};a.wirelessUpdate=function(){a.wirelessA.ssid=a.wirelessA.ssid;a.wirelessB.ssid=a.wirelessB.ssid;a.hasApplyChanges=!0;var b=a.wirelessA.radioEnabled;a.wirelessA.$update(function(){a.wirelessB.$update(function(){b?
d.showSaving("Save Settings","Saving your settings. Please wait...",15E3):(a.wirelessC.enabled=!1,a.wirelessC.id=2,a.wirelessC.$update(function(){a.wirelessD.enabled=!1;a.wirelessD.id=3;a.wirelessD.$update(function(){d.showSaving("Save Settings","Saving your settings. Please wait...",15E3)})}))})})};a.checkWEP=function(){a.wirelessA.radioEnabled&&0==a.wAtype&&a.wpsIsEnabled&&4==a.wirelessA.mode?a.enableWEP():a.wirelessA.radioEnabled&&0==a.wAtype&&(a.wpsIsEnabled||1==a.wirelessA.mode||8==a.wirelessA.mode)?
a.enableWEP1():a.wirelessA.radioEnabled&&1==a.wAtype&&0==a.isW24WEPOn?a.rdbtnChange(a.wAtype):a.wirelessUpdate()};a.enableWEP=function(){d.showCustomMessage("Warning",["If WEP is enabled for the 2.4 GHz wireless network, then WPS will be disabled for the 2.4 GHz wireless network.","Are you sure you want to enable WEP?"]).then(function(b){"cancel"!=b&&a.wirelessUpdate()})};a.enableWEP1=function(){d.showCustomMessage1("Warning",["Turning on WEP will change your 2.4 GHz wireless mode to Legacy Mode (802.11b/g) and disable Wi-Fi Protected Setup.",
...
!0):0==a.oldAType?a.wirelessA.security.enabled=!1:1==a.wAtype&&(a.wirelessA.security.type=a.oldAType),a.checkWEP()))};a.keyPress=function(b){13==b.keyCode&&"/wireless/basic"==f.path()&&a.save()}}]).controller("WirelessSSIDBroadcast",["$scope","$route","$routeParams","$location","Wireless","securityTips","WPS",function(a,b,c,d,e,f,g){a.tab=b.current.tab;a.section=b.current.section;a.wirelessWpsAreOn=!0;var h=c.id;a.wirelessName=0==h?"2.4 GHz":"5 GHz";var k=g.get({id:0},function(){a.wpsEnabled=k.enabled;
var b=e.query(function(){a.wirelessARadioEnabled=b[0].radioEnabled;a.wirelessAWpsEnabled=b[0].wpsEnabled;a.wirelessBRadioEnabled=b[1].radioEnabled;a.wirelessBWpsEnabled=b[1].wpsEnabled;a.wirelessARadioEnabled||a.wirelessBRadioEnabled||(a.wirelessWpsAreOn=!1);a.wirelessAWpsEnabled||a.wirelessBWpsEnabled||(a.wirelessWpsAreOn=!1);a.wpsEnabled&&!a.wirelessWpsAreOn&&(a.wpsEnabled=!1)})}),m=e.get({id:h},function(){a.ssidBroadcast=m.ssidBroadcast});a.wirelessUpdate=function(){e.update({id:h},{ssidBroadcast:a.ssidBroadcast},
function(){d.path("/wireless/advanced")})};a.save=function(){a.wpsEnabled&&!a.ssidBroadcast?f.showCustomMessage("Warning",["If SSID Broadcast is disabled for the "+a.wirelessName+" wireless network, then WPS will also be disabled for the "+a.wirelessName+" wireless network.","Are you sure you want to disable SSID Broadcast?"]).then(function(b){"cancel"!=b&&a.wirelessUpdate()}):a.wirelessUpdate()}}]).controller("WirelessMode",["$scope","$route","$location","Wireless","securityTips",function(a,b,c,
d,e){a.tab=b.current.tab;a.section=b.current.section;var f=d.query(function(){a.modeA=f[0].mode;a.modeB=f[1].mode;a.wirelessAEnabled=f[0].radioEnabled;a.wAtype=f[0].security.enabled?0==f[0].security.type?0:1:1});a.wirelessUpdate=function(){d.update({id:0},{mode:parseInt(a.modeA)},function(){d.update({id:1},{mode:parseInt(a.modeB)},function(){c.path("/wireless/advanced")})})};a.enableWEP=function(){0!=a.wAtype||!a.wirelessAEnabled||1!=a.modeA&&8!=a.modeA?a.wirelessUpdate():e.showCustomMessage2("Warning",
["Compatibility Mode and Performance Mode are not compatible with WEP security. You must first select WPA/WPA2 Mixed Mode or WPA2 security before enabling Compatibility or Performance mode."])};a.save=function(){a.enableWEP()}}]).controller("WirelessTransmissionCtrl",["$scope","$route","$routeParams","$location","WirelessTransmission",function(a,b,c,d,e){a.tab=b.current.tab;a.section=b.current.section;a.errorMessages=[];a.showErrorPage=!1;a.confirmed=!(null!=c.warn&&"true"==c.warn);a.trans1=e.get({id:0});
a.trans2=e.get({id:1});a.runFocus=!1;a.errorPageOk=function(){a.showErrorPage=!1;window.scrollTo(0,0)};a.isConfirmed=function(){a.confirmed=!0;a.runFocus=!0};a.keyPress=function(b){13==b.keyCode&&!a.showErrorPage&&a.confirmed&&a.save()};a.save=function(){a.errorMessages=[];a.trans1.ctsProtection=parseInt(a.trans1.ctsProtection);a.trans1.ctsProtectionType=parseInt(a.trans1.ctsProtectionType);a.trans2.ctsProtection=parseInt(a.trans2.ctsProtection);a.trans2.ctsProtectionType=parseInt(a.trans2.ctsProtectionType);
validateTransmissionSettings(a.trans1,"2.4 GHz Wireless",a.errorMessages);validateTransmissionSettings(a.trans2,"5.0 GHz Wireless",a.errorMessages);0<a.errorMessages.length?(a.showErrorPage=!0,window.scrollTo(0,0)):(a.trans1.channelWidth=parseInt(a.trans1.channelWidth),a.trans2.channelWidth=parseInt(a.trans2.channelWidth),a.trans1.beaconInterval=parseInt(a.trans1.beaconInterval),a.trans2.beaconInterval=parseInt(a.trans2.beaconInterval),a.trans1.dtimInterval=parseInt(a.trans1.dtimInterval),a.trans2.dtimInterval=
parseInt(a.trans2.dtimInterval),a.trans1.fragmentationThreshold=parseInt(a.trans1.fragmentationThreshold),a.trans2.fragmentationThreshold=parseInt(a.trans2.fragmentationThreshold),a.trans1.rtsThreshold=parseInt(a.trans1.rtsThreshold),a.trans2.rtsThreshold=parseInt(a.trans2.rtsThreshold),a.trans1.frameBurstMax=parseInt(a.trans1.frameBurstMax),a.trans2.frameBurstMax=parseInt(a.trans2.frameBurstMax),a.trans1.frameBurstTime=parseInt(a.trans1.frameBurstTime),a.trans2.frameBurstTime=parseInt(a.trans2.frameBurstTime),
a.trans1.power=parseInt(a.trans1.power),a.trans2.power=parseInt(a.trans2.power),a.trans1.$update({id:0},function(){a.trans2.$update({id:1},function(){d.path("/wireless/advanced")})}))}}]).controller("WirelessWmmCtrl",["$scope","$route","$routeParams","$location","WirelessWmm",function(a,b,c,d,e){a.tab=b.current.tab;a.section=b.current.section;a.wirelessName=0==c.id?"2.4 GHz":"5 GHz";a.qos=e.get({id:c.id});a.enabledClick=function(){a.qos.enabled||(a.qos.powerSave=!1)};a.save=function(){a.qos.$update({id:c.id},
function(){location.reload()})};a.keyPress=function(b){13==b.keyCode&&d.path()=="/wireless/wmm/"+c.id&&a.save()}}]).controller("WirelessWEPCtrl",["$scope","$route","$routeParams","$location","WEP","securityTips","WPS","Wireless",function(a,b,c,d,e,f,g,h){a.tab=b.current.tab;a.section=b.current.section;a.showErrorPage=!1;a.errorMessages=[];var k=c.id;a.wirelessName=0==k?"2.4 GHz":"5 GHz";a.wep=e.get({id:k},function(){var b=g.get({id:0},function(){a.wpsIsEnabled=b.enabled;a.wirelessCmd=h.query(function(){a.wirelessACmd=
a.wirelessCmd[0];a.wpsIsEnabled=a.wpsIsEnabled?!0:!1;a.ipAddress=splitIpArr(a.wep.serverIp)})})});a.errorPageOk=function(){a.showErrorPage=!1;window.scrollTo(0,0)};a.wepChanged=function(b){b=a.wep.settings[b];var c=10,c=0==b.keyLength?0==b.entryMethod?10:5:0==b.entryMethod?26:13;b.key.length>c&&(b.key=b.key.substr(0,c))};a.wirelessUpdate=function(b){e.update({id:k},{activeSetting:a.wep.activeSetting,mode:a.wep.mode,authenticationType:a.wep.authenticationType,settings:a.wep.settings},function(){1==
b?h.update({id:0},{mode:4},function(){d.path("/wireless/advanced")}):d.path("/wireless/advanced")})};a.checkWEP=function(){a.wirelessACmd.radioEnabled&&a.wpsIsEnabled&&4==a.wirelessACmd.mode?a.enableWEP():a.wirelessACmd.radioEnabled&&(a.wpsIsEnabled||1==a.wirelessACmd.mode||8==a.wirelessACmd.mode)?a.enableWEP1():a.wirelessUpdate(0)};a.enableWEP=function(){f.showCustomMessage("Warning",["If WEP is enabled for the 2.4 GHz wireless network, then WPS will be disabled for the 2.4 GHz wireless network.",
"Are you sure you want to enable WEP?"]).then(function(b){"ok"==b&&a.wirelessUpdate(0)})};a.enableWEP1=function(){f.showCustomMessage1("Warning",["Turning on WEP will change your 2.4 GHz wireless mode to Legacy Mode (802.11b/g) and disable Wi-Fi Protected Setup.","Press OK to continue or Cancel."]).then(function(b){"ok"==b&&a.wirelessUpdate(1)})};a.save=function(){a.errorMessages=[];a.wep.authenticationType=parseInt(a.wep.authenticationType);for(var b=0;b<a.wep.settings.length;b++)a.wep.settings[b].entryMethod=
parseInt(a.wep.settings[b].entryMethod),a.wep.settings[b].keyLength=parseInt(a.wep.settings[b].keyLength),validateWepSettings(a.wep.settings[b],b==a.wep.activeSetting,"Key #"+(b+1),a.errorMessages);0<a.errorMessages.length?(a.showErrorPage=!0,window.scrollTo(0,0)):a.checkWEP()}}]).controller("WirelessWPACtrl",["$scope","$route","$routeParams","$location","WPA",function(a,b,c,d,e){a.tab=b.current.tab;a.section=b.current.section;a.whichKey=1;a.keyType=0;a.errorMessages=[];a.showErrorPage=!1;var f=c.id;
a.wirelessName=0==f?"2.4 GHz":"5 GHz";a.wpaType=0==b.current.wpa?"WPA2/WPA mixed mode":"WPA2";a.wpa=e.get({id:f},function(){a.wpa.key=a.wpa.key;a.wpa.type=b.current.wpa});a.errorPageOk=function(){a.showErrorPage=!1;window.scrollTo(0,0)};a.validateWpaKey=function(b){8>b.length&&a.errorMessages.push("Pre-Shared Key: Pass-Phrase has to be at least 8 characters long.")};a.save=function(){a.errorMessages=[];a.wpa.groupKeyUpdateIntervalEnabled&&(isNaN(a.wpa.groupKeyUpdateInterval)||900>a.wpa.groupKeyUpdateInterval||
86400<a.wpa.groupKeyUpdateInterval||!checksForDecimalNumber(a.wpa.groupKeyUpdateInterval))&&a.errorMessages.push("Group Key Update Interval: Please enter a numeric value between 900 and 86400.");a.validateWpaKey(a.wpa.key);0!=a.errorMessages.length?(a.showErrorPage=!0,window.scrollTo(0,0)):(a.wpa.groupKeyUpdateIntervalEnabled||(a.wpa.groupKeyUpdateInterval=0),a.wpa.encryptionAlgorithm=parseInt(a.wpa.encryptionAlgorithm),e.update({id:f},{type:a.wpa.type,mode:a.wpa.mode,encryptionAlgorithm:a.wpa.encryptionAlgorithm,
groupKeyUpdateInterval:a.wpa.groupKeyUpdateInterval,groupKeyUpdateIntervalEnabled:a.wpa.groupKeyUpdateIntervalEnabled,key:a.wpa.key},function(){d.path("/wireless/advanced")}))}}]).controller("WirelessMACFilterCtrl",["$scope","$route","$location","WirelessMACFilter","Wireless","securityTips","WPS",function(a,b,c,d,e,f,g){a.tab=b.current.tab;a.section=b.current.section;a.warningFlg=!1;a.newMac="";a.filter0=d.get({id:0});a.filter1=d.get({id:1});a.errorPageOk=function(){a.showErrorPage=!1;window.scrollTo(0,
0)};a.checkFilters=function(b){for(var c=0;c<b.length;c++)if(17!=b[c].length||!validateMac(b[c])||/[0\:]{17}$/.test(b[c])||/[f\:]{17}$/i.test(b[c])||parseInt(b[c].split(":")[0],16)&1){a.errorMessages.push("Invalid MAC Address format: Please try again.");break}};a.save=function(){a.errorMessages=[];a.checkFilters(a.filter0.macs);0==a.errorMessages.length&&a.checkFilters(a.filter1.macs);0<a.errorMessages.length?(a.showErrorPage=!0,window.scrollTo(0,0)):(a.filter0.mode=parseInt(a.filter0.mode),a.filter1.mode=
parseInt(a.filter1.mode),a.filter0.$update({id:0},function(){a.filter1.$update({id:1},function(){c.path("/wireless/advanced")})}))};a.chkBeforeSave=function(){var b=!0,c=e.query(function(){var d=g.get({id:0},function(){a.wpsEnabled=d.enabled;b=!0;a.wire0=c[0].radioEnabled;a.wire1=c[1].radioEnabled;var e=c[0].wpsEnabled,g=c[1].wpsEnabled;a.wire0||a.wire1||(b=!1);e||g||(b=!1);a.wpsEnabled&&!b&&(a.wpsEnabled=!1);var g=e=!1,z="",w="";a.filter0.enabled&&0==a.filter0.mode&&0==a.filter0.macs.length&&(e=
!0);a.filter1.enabled&&0==a.filter1.mode&&0==a.filter1.macs.length&&(g=!0);a.warningFlg=a.wpsEnabled?e||g?!0:!1:!1;a.warningFlg?(e&&g?(z="2.4GHz and 5.0GHz",w="networks"):g?(z="5.0GHz",w="network"):e&&(z="2.4GHz",w="network"),f.showCustomMessage1("Warning",["If Access List (MAC address filter) is enabled with no accepted devices (an empty white list) for the "+z+" wireless "+w+", then WPS will be disabled for the "+z+" wireless "+w+".","Are you sure you want to enable Access List with no accepted devices?",
"Press OK to continue or Cancel."]).then(function(b){"cancel"!=b&&a.save()})):a.save()})})};a.removeMac=function(b,c){0==b?a.filter0.macs.splice(c,1):a.filter1.macs.splice(c,1)};a.addFilter=function(b){a.errorMessages=[];if(0==b)if(angular.isUndefined(a.newMac0))a.errorMessages.push("MAC Address: MAC address must consist of 6 fields. Each field's value must be between 00 and FF (in hexadecimal representation), without any spaces. The LSB of the first byte must be zero.");else if(0!=a.errorMessages.length||
validateMac(a.newMac0)&&notAcceptedMac(a.newMac0)&&/^([0-9A-Fa-f]{2}[:]){5}([0-9A-Fa-f]{2})$/.test(a.newMac0)){for(b=0;b<a.filter0.macs.length;b++)if(a.newMac0==a.filter0.macs[b])return;a.filter0.macs.push(a.newMac0);a.newMac0=""}else a.errorMessages.push("MAC Address: MAC address must consist of 6 fields. Each field's value must be between 00 and FF (in hexadecimal representation), without any spaces. The LSB of the first byte must be zero.");else if(angular.isUndefined(a.newMac1))a.errorMessages.push("MAC Address: MAC address must consist of 6 fields. Each field's value must be between 00 and FF (in hexadecimal representation), without any spaces. The LSB of the first byte must be zero.");
else if(0!=a.errorMessages.length||validateMac(a.newMac1)&&notAcceptedMac(a.newMac1)&&/^([0-9A-Fa-f]{2}[:]){5}([0-9A-Fa-f]{2})$/.test(a.newMac1)){for(b=0;b<a.filter1.macs.length;b++)if(a.newMac1==a.filter1.macs[b])return;a.filter1.macs.push(a.newMac1);a.newMac1=""}else a.errorMessages.push("MAC Address: MAC address must consist of 6 fields. Each field's value must be between 00 and FF (in hexadecimal representation), without any spaces. The LSB of the first byte must be zero.");0<a.errorMessages.length&&
(a.showErrorPage=!0,window.scrollTo(0,0))}}]).controller("WifiSetupCtrl",["$scope","$route","$location","WPS","Devices","Wireless","securityTips","WirelessMACFilter",function(a,b,c,d,e,f,g,h){a.tab=b.current.tab;a.section=b.current.section;a.valid=!0;a.apLockDown=!1;a.erEnabled=!1;a.apPin="";a.erCanEnabled=!0;a.onlyNumbers=/^\d+$/;a.wirelessWpsAreOn=!0;a.hasSavePopup=!1;a.$on("$destroy",function(){a.valid=!1});var k=0;a.refresh=function(){var b=d.get({id:0},function(){var c=f.query(function(){a.filter0=
h.get({id:0},function(){a.filter1=h.get({id:1},function(){a.wirelessWpsAreOn=!0;a.wire0=c[0].radioEnabled;a.wire1=c[1].radioEnabled;var d=c[0].wpsEnabled,e=c[1].wpsEnabled;a.wire0||a.wire1||(a.wirelessWpsAreOn=!1);d||e||(a.wirelessWpsAreOn=!1);if(!0==b.apLockDown||!1==b.apLockDown)a.apLockDown=b.apLockDown;if(!0==b.erEnabled||!1==b.erEnabled)a.erEnabled=b.erEnabled;if(b.apPin)for(a.apPin=b.apPin;8>String(a.apPin).length;)a.apPin="0"+String(a.apPin);""!=a.apPin&&(a.erCanEnabled=!1);a.enabled=b.enabled;
a.enabled&&!a.wirelessWpsAreOn&&(a.enabled=!1);a.trigger=b.trigger;d=!1;!0==a.isCancelled&&(b.status=0,a.timeString="",a.isCancelled=!1);if(0==a.currentStep)0!=b.status&&(a.currentStep=1,k=(new Date).getTime(),a.timeString="2 minutes",d=!0);else if(3==a.currentStep)3==b.lastStatus?a.currentStep=5:4==b.lastStatus?(a.mac=b.device,a.currentStep=4):0==b.status&&1==b.lastStatus?a.currentStep=0:d=!0;else if(1==a.currentStep||2==a.currentStep){2==b.status?(a.currentStep=3,d=!0):3==b.lastStatus?a.currentStep=
5:4==b.lastStatus?(a.mac=b.device,a.currentStep=4):0==b.status&&1==b.lastStatus?a.currentStep=0:d=!0;if(!d)return;e=120-Math.floor(((new Date).getTime()-k)/1E3);if(0>=e){a.currentStep=5;return}d=Math.floor(e/60);e%=60;a.timeString="";0<d&&(a.timeString=d+" minute"+(1<d?"s":"")+(0<e?", ":""));0<e&&(a.timeString+=e+" second"+(1<e?"s":""));d=!0}d&&a.valid&&window.setTimeout(a.refresh,1E3);a.hasSavePopup&&(a.savePopup.close(),a.hasSavePopup=!1)})})})})};a.refresh();a.currentStep=0;a.clientWpsPin="";a.toggleOnOffSave=
function(){d.update({id:0},{enabled:!a.enabled},function(){a.savePopup=g.showSaving1("Apply Settings","Applying your settings. Please wait...");a.hasSavePopup=!0;a.currentStep=0;window.setTimeout(a.toggleOnOffSaveRefresh,3E3)})};a.toggleOnOffSaveRefresh=function(){a.refresh()};a.toggleOnOff=function(){a.enabled?a.toggleOnOffSave():a.wire0||a.wire1?a.wirelessWpsAreOn?a.toggleOnOffSave():g.showCustomMessage3("Warning",["Wi-Fi Protected Setup (WPS) cannot be used if WEP security is enabled or SSID broadcast is disabled or if MAC address authentication is enabled with an empty white list.",
"To use WPS, change your wireless security mode and/or enable SSID broadcast and/or add a MAC address to the mac authentication table."]):g.showCustomMessage3("Warning",["Wi-Fi Protected Setup (WPS) is disabled because both Wi-Fi radios(2.4GHz and 5GHz) have been turned off.","Please enable Wi-Fi on one or both radios to enable WPS."])};a.pushButtonConfig=function(){d.update({id:0},{trigger:!a.trigger},function(){k=(new Date).getTime();a.currentStep=1;window.setTimeout(a.testConnection,3E3)})};a.testConnection=
function(){a.refresh()};a.errorPageOk=function(){a.showErrorPage=!1;window.scrollTo(0,0)};a.register=function(){a.errorMessages=[];var b=!1;-1==String(a.clientWpsPin).indexOf("-")&&-1==String(a.clientWpsPin).indexOf(" ")?/^[0-9]{8}$/.test(a.clientWpsPin)||/^[0-9]{4}$/.test(a.clientWpsPin)||(b=!0):/^[0-9- ]{9}$/.test(a.clientWpsPin)||/^[0-9- ]{5}$/.test(a.clientWpsPin)||(b=!0);b&&a.errorMessages.push("Error: Invalid PIN. Please enter a valid WPS PIN, which consists of either four or eight numeric digits, and can typically be found printed on your wireless client device or in its documentation.");
if(0<a.errorMessages.length)a.showErrorPage=!0,window.scrollTo(0,0);else if(""==a.clientWpsPin)a.pushButtonConfig();else var b=a.clientWpsPin.replace(/ /g,"").replace(/-/g,""),c=d.update({id:0},{pin:b},function(){"00000000"==c.pin?(a.errorMessages.push("Error: Invalid PIN. Please enter a valid WPS PIN, which consists of either four or eight numeric digits, and can typically be found printed on your wireless client device or in its documentation."),0<a.errorMessages.length&&(a.showErrorPage=!0,window.scrollTo(0,
0))):(k=(new Date).getTime(),a.currentStep=2,window.setTimeout(a.testConnection,3E3))})};a.cancel=function(){d.update({id:0},{trigger:!a.trigger},function(){a.currentStep=0;a.isCancelled=!0;k=0})};a.connect=function(){a.currentStep=4};a.completed=function(){a.currentStep=0;a.clientWpsPin=""};a.sendErEnabled=function(b){d.update({id:0},{erEnabled:b},function(){a.refresh()})}}]).controller("GuestWifiSetupTabCtrl",["$scope","$route","$location","Wireless",function(a,b,c,d){a.pageToShow=0;a.currentTab=
-1;a.bothDisabled=!1;a.basicGuestWifiEnabled=!1;a.guestPassWifiEnabled=!1;a.guestWifiEnum={BASIC_GUEST_WIFI:2,GUEST_PASS:3};a.tabsEnum={1:"partials/wireless/guestDevicesTab.html?v=v1.0.125",2:"partials/wireless/basicGuestWifiTab.html?v=v1.0.125",3:"partials/wireless/guestPassWifiTab.html?v=v1.0.125"};a.section=b.current.section;b=c.path();-1!=b.indexOf("/wireless/guestWifiSetup/devices")?a.currentTab=1:-1!=b.indexOf("/wireless/guestWifiSetup/basic")?a.currentTab=2:-1!=b.indexOf("/wireless/guestWifiSetup/pass")?
a.currentTab=3:a.wireless1=d.query(function(){a.basicGuestWifi1=a.wireless1[a.guestWifiEnum.BASIC_GUEST_WIFI];a.guestPassWifi1=a.wireless1[a.guestWifiEnum.GUEST_PASS];void 0!=a.basicGuestWifi1&&null!=a.basicGuestWifi1&&a.basicGuestWifi1.enabled?c.url("/wireless/guestWifiSetup/basic"):void 0!=a.guestPassWifi1&&null!=a.guestPassWifi1&&a.guestPassWifi1.enabled?c.url("/wireless/guestWifiSetup/pass"):c.url("/wireless/guestWifiSetup/basic")});a.mainRefresh=function(){a.basicGuestWifiEnabled=!1;a.guestPassWifiEnabled=
!1;a.bothDisabled=!1;a.wireless=d.query(function(){a.basicGuestWifi=a.wireless[a.guestWifiEnum.BASIC_GUEST_WIFI];a.guestPassWifi=a.wireless[a.guestWifiEnum.GUEST_PASS];void 0!=a.basicGuestWifi&&null!=a.basicGuestWifi&&a.basicGuestWifi.enabled?(a.basicGuestWifiEnabled=!0,a.bothDisabled=!1):void 0!=a.guestPassWifi&&null!=a.guestPassWifi&&a.guestPassWifi.enabled?(a.guestPassWifiEnabled=!0,a.bothDisabled=!1):a.bothDisabled=!0});a.togglePage(0)};a.togglePage=function(b){a.pageToShow=b;window.scrollTo(0,
0)};a.mainRefresh();a.isActive=function(b){return a.currentTab===b};a.setActive=function(a){switch(a){case 1:c.url("/wireless/guestWifiSetup/devices");break;case 2:c.url("/wireless/guestWifiSetup/basic");break;case 3:c.url("/wireless/guestWifiSetup/pass");break;default:c.url("/wireless/guestWifiSetup/basic")}};a.getActive=function(){return a.currentTab}}]).controller("GuestWifiSetupCtrl",["$scope","$route","$location","Wireless","GuestDevices","GuestPassDevices","Devices","$rootScope","securityTips",
function(a,b,c,d,e,f,g,h,k){a.bothDisable=!1;a.pageToShow=0;a.tab=b.current.tab;a.section=b.current.section;a.refresh=function(){a.guestDevices=null;a.wireless=d.query(function(){a.basicGuestWifi=a.wireless[a.guestWifiEnum.BASIC_GUEST_WIFI];a.guestPassWifi=a.wireless[a.guestWifiEnum.GUEST_PASS];a.basicGuestWifi.enabled||a.guestPassWifi.enabled?a.guestDevices=g.query(function(){for(var b=0;b<a.guestDevices.length;b++)a.guestDevices[b].toDisplay=!1,6==a.guestDevices[b].connectionType&&(a.guestDevices[b].ssid=
a.basicGuestWifi.enabled?a.basicGuestWifi.ssid:a.guestPassWifi.ssid,a.guestDevices[b].toDisplay=!0,setDeviceStyle(a.guestDevices[b],""))}):a.bothDisable=!0});a.togglePage(0)};a.togglePage=function(b){a.pageToShow=b;window.scrollTo(0,0)};a.refresh();a.toggleOnOffSave=function(b){b.blocked=!b.blocked;g.update({id:b.id},b,function(){a.refresh()})}}]).controller("BasicGuestCtrl",["$scope","$route","$location","Wireless","GuestDevices","WPA","securityTips","$sce","$rootScope","Devices",function(a,b,c,
d,e,f,g,h,k,m){a.warningFlag=!1;a.pageToShow=0;a.tab=b.current.tab;a.section=b.current.section;a.keySet=!1;a.showPassword=!0;k.editingGuest=!1;k.guestHasChanges=!1;a.listener=null;a.tempWPAkey="";a.togglePage=function(b){a.pageToShow=b;window.scrollTo(0,0)};a.refresh=function(){a.showPassword=!0;a.wireless=d.query(function(){a.wirelessA=a.wireless[0];a.wirelessB=a.wireless[1];a.basicGuestWifi=a.wireless[a.guestWifiEnum.BASIC_GUEST_WIFI];a.guestPassWifi=a.wireless[a.guestWifiEnum.GUEST_PASS];a.basicGuestWifi.ssid=
a.basicGuestWifi.ssid;a.tempWPAkey="";void 0!=a.basicGuestWifi&&null!=a.basicGuestWifi&&(a.guestDevices=m.query(function(){a.guestDevices=filterOutNonGuest(a.guestDevices)}),a.wpa=f.get({id:a.guestWifiEnum.BASIC_GUEST_WIFI},function(){void 0!=a.wpa.key&&(a.wpa.key=a.wpa.key,a.tempWPAkey=h.trustAsHtml(a.convertKey(a.wpa.key)),a.keySet=""!==a.wpa.key?!0:!1)}));a.warningFlag=void 0!=a.basicGuestWifi&&null!=a.basicGuestWifi&&!a.basicGuestWifi.enabled&&void 0!=a.guestPassWifi&&null!=a.guestPassWifi&&a.guestPassWifi.enabled?
!0:!1});a.mainRefresh();a.togglePage(0)};a.convertKey=function(a){var b="";if(a)for(var c=0;c<a.length;c++)b+="&bull;";return b=unescape(b)};a.refresh();a.toggleOnOffSave=function(){a.wirelessA.radioEnabled?a.basicGuestWifi.enabled?(a.basicGuestWifi.enabled=!1,a.saveToggle(!0)):a.guestPassWifi.enabled?g.showWarningMsgGuestWifi("","Turning on the Basic Guest Wi-Fi will disable the Guest Pass Wi-Fi.",!0,[""]).then(function(b){"cancel"!=b&&(a.keySet?a.saveSettings(!0):a.showKeyForm())}):a.keySet?a.saveSettings(!0):
a.showKeyForm():g.showWirelease24GPrompt("","").then(function(a){})};a.saveSettings=function(b){a.basicGuestWifi.enabled=!0;a.guestPassWifi.enabled=!1;a.saveToggle(b)};a.saveToggle=function(b){a.basicGuestWifi.radioFreq=a.wirelessA.radioEnabled?1:-1;a.guestPassWifi.radioFreq=a.wirelessA.radioEnabled?1:-1;d.update({id:a.guestWifiEnum.BASIC_GUEST_WIFI},a.basicGuestWifi,function(){d.update({id:a.guestWifiEnum.GUEST_PASS},a.guestPassWifi,function(){b?a.refresh():a.saveWifiKey()})})};a.showKeyForm=function(){g.showCreateGuestWifiPassword("Create Password",
"Please create a password to activate your Guest Wi-Fi.",a.wpa).then(function(b){if("cancel"==b.result)a.refresh();else{var c=!1;if(void 0==b.returnObj)c=!0;else if(a.wpa.key=b.returnObj,8>a.wpa.key.length||63<a.wpa.key.length)c=!0;c?a.openValidationPopup():f.update({id:a.guestWifiEnum.BASIC_GUEST_WIFI},{wpaShowKeyInUI:b.wpaShowKeyInUI},function(){a.saveSettings(!1)})}})};a.openValidationPopup=function(){g.showWarningMsgGuestWifi("","Password has to be at least 8 characters long",!1,[""]).then(function(b){"ok"==
b&&a.showKeyForm()})};a.openValidationPopup1=function(){g.showWarningMsgGuestWifi("","Password has to be at least 8 characters long",!1,[""]).then(function(a){})};a.openValidationPopup2=function(){g.showWarningMsgGuestWifi("","SSID must be 1 to 64 characters",!1,[""]).then(function(a){})};a.openValidationPopup3=function(){g.showWarningMsgGuestWifi2("","SSID must be 1 to 64 characters",!1,[""]).then(function(a){})};a.saveWifiKey=function(){a.wpa.encryptionAlgorithm=parseInt(a.wpa.encryptionAlgorithm);
f.update({id:a.guestWifiEnum.BASIC_GUEST_WIFI},{type:a.wpa.type,mode:a.wpa.mode,encryptionAlgorithm:a.wpa.encryptionAlgorithm,groupKeyUpdateInterval:a.wpa.groupKeyUpdateInterval,groupKeyUpdateIntervalEnabled:a.wpa.groupKeyUpdateIntervalEnabled,key:a.wpa.key},function(){k.locationChangeActivate?a.redirect():(a.basicGuestWifi.ssid="",a.refresh(),k.editingGuest=!1)})};a.editWifiSettings=function(){k.editingGuest=!0;k.guestHasChanges=!1};a.saveWifiSettings=function(){var b=!1,c=!1;if(8>a.wpa.key.length||
63<a.wpa.key.length)b=!0;if(1>a.basicGuestWifi.ssid.length||64<a.basicGuestWifi.ssid.length||!/^[0-9a-zA-Z\!\"\#\$\%\&\u2019\'\(\)\*\+\,\-\.\/\:\;\<\=\>\?\@\[\\\]\^\_\`\{\|\}\~\ ]+$/i.test(a.basicGuestWifi.ssid))c=!0;c&&b?(a.cancelRedirect(),a.openValidationPopup3()):c?(a.cancelRedirect(),a.openValidationPopup2()):b?(a.cancelRedirect(),a.openValidationPopup1()):(a.basicGuestWifi.ssid=a.basicGuestWifi.ssid,d.update({id:a.guestWifiEnum.BASIC_GUEST_WIFI},a.basicGuestWifi,function(){f.update({id:a.guestWifiEnum.BASIC_GUEST_WIFI},
{wpaShowKeyInUI:a.wpa.wpaShowKeyInUI},function(){a.saveWifiKey()})}))};a.cancel=function(){a.refresh();k.editingGuest=!1;k.guestHasChanges=!1};a.change=function(){k.guestHasChanges=!0};a.saveContents1Function=function(b){a.saveWifiSettings()};a.listener=k.$on("saveContents1",a.saveContents1Function);a.$on("$destroy",function(){a.listener()});a.redirect=function(){k.locationChangeActivate=!1;k.editingGuest=!1;k.guestHasChanges=!1;""==getNextLocationForLocationChangeSub(k.locationChangeNext)?c.path(getNextLocationForLocationChange(k.locationChangeNext)):
c.path(getNextLocationForLocationChange(k.locationChangeNext)).search(getNextLocationForLocationChangeSub(k.locationChangeNext))};a.toggleShowPassword=function(){f.update({id:a.guestWifiEnum.BASIC_GUEST_WIFI},{wpaShowKeyInUI:a.wpa.wpaShowKeyInUI},function(){})};a.cancelRedirect=function(){k.locationChangeActivate=!1}}]).controller("BasicGuestCtrlPopup",["$scope","WPA","model","dialog",function(a,b,c,d){a.key=c.key;a.wpaPopup=c.wpa;a.buttons=c.buttons;a.title=c.title;a.message=c.message;a.close=function(b){d.close({result:b,
returnObj:a.key,wpaShowKeyInUI:a.wpaPopup.wpaShowKeyInUI})}}]).controller("GuestPassWifiCtrl",["$scope","$route","$location","Wireless","GuestPassDevices","securityTips","Devices","$rootScope",function(a,b,c,d,e,f,g,h){a.warningFlag=!1;a.pageToShow=0;a.tab=b.current.tab;a.section=b.current.section;h.editingGuest=!1;h.guestHasChanges=!1;a.listener=null;a.togglePage=function(b){a.pageToShow=b;window.scrollTo(0,0)};a.refresh=function(){a.wireless=d.query(function(){a.wirelessA=a.wireless[0];a.wirelessB=
a.wireless[1];a.basicGuestWifi=a.wireless[a.guestWifiEnum.BASIC_GUEST_WIFI];a.guestPassWifi=a.wireless[a.guestWifiEnum.GUEST_PASS];a.guestPassWifi.ssid=a.guestPassWifi.ssid;void 0!=a.guestPassWifi&&null!=a.guestPassWifi&&(a.guestDevices=g.query(function(){a.guestDevices=filterOutNonGuest(a.guestDevices)}));a.warningFlag=void 0!=a.guestPassWifi&&null!=a.guestPassWifi&&!a.guestPassWifi.enabled&&void 0!=a.basicGuestWifi&&null!=a.basicGuestWifi&&a.basicGuestWifi.enabled?!0:!1});a.mainRefresh();a.togglePage(0)};
a.refresh();a.toggleOnOffSave=function(){a.wirelessA.radioEnabled?a.guestPassWifi.enabled?(a.guestPassWifi.enabled=!1,a.saveToggle()):a.basicGuestWifi.enabled?f.showWarningMsgGuestWifi("","Turning on the Guest Pass Wi-Fi will disable the Basic Guest Wi-Fi.",!0,[""]).then(function(b){"cancel"!=b&&(a.guestPassWifi.enabled=!0,a.basicGuestWifi.enabled=!1,a.saveToggle())}):(a.guestPassWifi.enabled=!0,a.saveToggle()):f.showWirelease24GPrompt("","").then(function(a){})};a.saveToggle=function(){a.basicGuestWifi.radioFreq=
a.wirelessA.radioEnabled?1:-1;a.guestPassWifi.radioFreq=a.wirelessA.radioEnabled?1:-1;a.guestPassWifi.ssid=a.guestPassWifi.ssid;d.update({id:a.guestWifiEnum.GUEST_PASS},a.guestPassWifi,function(){d.update({id:a.guestWifiEnum.BASIC_GUEST_WIFI},a.basicGuestWifi,function(){h.locationChangeActivate?a.redirect():(a.guestPassWifi.ssid="",a.refresh(),h.editingGuest=!1)})})};a.editWifiSettings=function(){h.editingGuest=!0;h.guestHasChanges=!1};a.openValidationPopup2=function(){f.showWarningMsgGuestWifi("",
"SSID must be 1 to 64 characters",!1,[""]).then(function(a){})};a.saveWifiSettings=function(){var b=!1;if(1>a.guestPassWifi.ssid.length||64<a.guestPassWifi.ssid.length||!/^[0-9a-zA-Z\!\"\#\$\%\&\u2019\'\(\)\*\+\,\-\.\/\:\;\<\=\>\?\@\[\\\]\^\_\`\{\|\}\~\ ]+$/i.test(a.guestPassWifi.ssid))b=!0;b?(a.cancelRedirect(),a.openValidationPopup2()):a.saveToggle()};a.cancel=function(){a.refresh();h.editingGuest=!1;h.guestHasChanges=!1};a.change=function(){h.guestHasChanges=!0};a.saveContents2Function=function(b){a.saveWifiSettings()};
a.listener=h.$on("saveContents2",a.saveContents2Function);a.$on("$destroy",function(){a.listener()});a.redirect=function(){h.locationChangeActivate=!1;h.editingGuest=!1;h.guestHasChanges=!1;""==getNextLocationForLocationChangeSub(h.locationChangeNext)?c.path(getNextLocationForLocationChange(h.locationChangeNext)):c.path(getNextLocationForLocationChange(h.locationChangeNext)).search(getNextLocationForLocationChangeSub(h.locationChangeNext))};a.cancelRedirect=function(){h.locationChangeActivate=!1}}]);
