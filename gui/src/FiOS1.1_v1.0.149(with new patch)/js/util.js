...
function validateIp(ip) {
  ...
  for(var i = 0; i < ip.length; i++){
    if (String(ip[i]) == '')
      return false;
    if(ip[i] == null || isNaN(ip[i]) || isNotaInt(ip[i]) || ip[i] < 0 || ip[i] > 255 || !checksForValidIPInput(ip[i]))
      return false;
  }
  return true;
}

function validateChecksForWholeNumber(ip){
  var result = true;
  for(var i = 0; i < ip.length; i++){
    if (!checksForWholeNumber(ip[i]))
      result = false;
  }
  return result;
}
...
function validateTransmissionSettings(trans, wifiName, errors) {
  if(isNaN(trans.beaconInterval) || trans.beaconInterval == null || !checksForDecimalNumber(trans.beaconInterval) || !checksForWholeNumber(trans.beaconInterval))
  ...
  if(isNaN(trans.dtimInterval) || trans.dtimInterval == null || !checksForDecimalNumber(trans.dtimInterval) || !checksForWholeNumber(trans.dtimInterval))
  ...
  if(isNaN(trans.fragmentationThreshold) || trans.fragmentationThreshold == null || !checksForDecimalNumber(trans.fragmentationThreshold) || !checksForWholeNumber(trans.fragmentationThreshold))
  ...
  if(trans.rtsThreshold == null || trans.rtsThreshold === '' || isNaN(trans.rtsThreshold) || !checksForDecimalNumber(trans.rtsThreshold) || !checksForWholeNumber(trans.rtsThreshold))
  ...
  if(isNaN(trans.frameBurstMax) || !checksForDecimalNumber(trans.frameBurstMax) || !checksForWholeNumber(trans.frameBurstMax))
  ...
  if(trans.frameBurstTime == null || trans.frameBurstTime === '' || isNaN(trans.frameBurstTime) || !checksForDecimalNumber(trans.frameBurstTime) || !checksForWholeNumber(trans.frameBurstTime))
  ...
  if(isNaN(trans.power) || !checksForDecimalNumber(trans.power) || !checksForWholeNumber(trans.power))
...
function validateIPv4DhcpSettings(ip, startAddress, endAddress, subnetMask, winsServer, leaseTime, connectionId, protocolUsed, overrideSubnetMask, listOfNetworkIPAddress, fromAdvanced) {
  var errors = [];
  ...
  var errorMsgForIP = false;
  if(!validateIp(ip)){
    errorMsgForIP = true;
  }else{
    if(fromAdvanced){
      if(!validateChecksForWholeNumber(ip)){
        errorMsgForIP = true;
      }
    }
  }
  ...
  if(!validateIp(startAddress)){
    ...
    if(fromAdvanced){
      if(!validateChecksForWholeNumber(startAddress)){
        errorMsgForIP = true;
      }
    }
  }
  ...
  if(!validateIp(endAddress)){
    ...
    if(fromAdvanced){
      if(!validateChecksForWholeNumber(endAddress)){
        errorMsgForIP = true;
      }
    }
  }
...
function validateHostnameDevice(str) {
  if(!/[a-z0-9]/i.test(str))
    return false;
  if(!(/^[a-z0-9\_\-\.]{0,}$/i.test(str)))
    return false;
  if(!(/^[a-z0-9]$/i.test(str.substr(str.length - 1))))
    return false;
  if(!(/^[a-z0-9]$/i.test(str.substr(0,1))))
    return false;
  var arr = str.split('.');
  for(var i = 0; i < arr.length; i++)
    if(arr[i].length>63)
...
function validateHostnameDNSServer(str) {
  if(!/[a-z0-9]/i.test(str))
    return false;
  if(!(/^[a-z0-9]$/i.test(str.substr(str.length - 1))))
    return false;
  if(str.indexOf(".") != -1){
    ...
    for(var i = 0; i < tempArr.length; i++){
      if(tempArr[i].length == 0){
        return false;
        break;
      }else if(!/^[a-z0-9][a-z0-9\-\_]{0,}$/i.test(tempArr[i])){
        return false;
        break;
      }else if(!(/^[a-z0-9]$/i.test(tempArr[i].substr(tempArr[i].length - 1)))){
        return false;
        break;
      }
    }
  }else{
    if(!/^[a-z0-9][a-z0-9\-]{0,}[\.]{1}[a-z0-9][a-z0-9\-]{0,}[\.]{1}[a-z0-9][a-z0-9\-]{0,}[\.]{1}[a-z0-9][a-z0-9\-]{0,}[\.]{1}[a-z0-9]{0,}$|^[a-z0-9][a-z0-9\-]{0,}[\.]{1}[a-z0-9][a-z0-9\-]{0,}[\.]{1}[a-z0-9][a-z0-9\-]{0,}[\.]{1}[a-z0-9]{0,}$|^[a-z0-9][a-z0-9\-]{0,}[\.]{1}[a-z0-9][a-z0-9\-]{0,}[\.]{1}[a-z0-9]{0,}$|^[a-z0-9]$|^[a-z0-9][a-z0-9\_\-]{0,}[a-z0-9]$|^[a-z0-9][a-z0-9]{0,}[\.]{1}[a-z0-9]{0,}$|^[a-z0-9][a-z0-9]{0,}[\.]{1}[a-z0-9]{0,}[\.]{1}$|^(([0-9]|[1-9][0-9]|1[0-9]{2}|2[0-4][0-9]|25[0-5])\.){3}([0-9]|[1-9][0-9]|1[0-9]{2}|2[0-4][0-9]|25[0-5])$/i.test(str))
      return false;
  }
  if(str.indexOf('--') != -1 || str.indexOf('-_') != -1 || str.indexOf('_-') != -1 || str.indexOf('__') != -1)
    return false;
  var arr = str.split('.');
  for(var i = 0; i < arr.length; i++)
    if(arr[i].length>63)
...
function getNextLocationForLocationChange(urlStr){
  ...
  if(String(tempArr[1]).indexOf("?") != -1){
	tempArr = String(tempArr[1]).split("?");
	return tempArr[0];
  }
  return tempArr[1];
}

function getNextLocationForLocationChangeSub(urlStr){
  var tempArr = String(urlStr).split('?');
  if(tempArr.length < 2){
    return "";
  }
...
function checksForValidIPInput(value){
  var result = true;
  if(!(/^[0-9\.]*$/i.test(value))){
    result = false;
  }
