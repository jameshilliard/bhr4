      ...
      $scope.applyProtocol = function() {
        ...
        switch($scope.protocols.protocol) {
          case 0: {
            ...
            if(pro == null || pro === '' || isNaN(pro) || pro < 0 || pro > 255 || !checksForWholeNumber(pro))
    ...
    function($scope, $route, PortForward, Devices, PortForwardRules, Schedules, DateTimeSettings) {
      ...
      $scope.add = function() {
            ...
            for(var p = 0; p < parts.length; p++) {
              ...
              if(field.length > 2) {
                ...
                for(var i = 0; i < field.length; i++) {
                  if(field[i] === '' || isNaN(field[i]) || checkForWholeNumbers(field[i]) || !checksForWholeNumber(field[i]))
                    $scope.errorMessages.push("Error: The input of port has illegal characters. Source port should be 111 or 111-222; Destination port should be 111, or 111-222, or 111,222-333 and so on.");
        ...
        if($scope.showAdvanced && $scope.pf.servicePortType == 1 && $scope.errorMessages.length == 0) {
          if($scope.pf.servicePort === '' || isNaN($scope.pf.servicePort) || $scope.pf.servicePort < 0 || $scope.pf.servicePort > 65535 || !checksForWholeNumber($scope.pf.servicePort))
          ...
          for(var i = 0; i < service.protocols.length; i++) {
            ...
            for(var j = 0; j < $scope.portForward.length; j++) {
              for(var k = 0; k < $scope.portForward[j].protocols.length; k++) {
                ...
                if(range != '' || nonTCPnUDP){
                  var msg = "Conflicting Port Forwarding Rules: The " + protocolName + " service conflict with an existing port forwarding rule offering " + protocolName + " service.";
                  var result = true;
                  if($scope.errorMessages.length > 0){
                    for(var i = 0; i < $scope.errorMessages.length; i ++){
                      if($scope.errorMessages[i] == msg){
                        result = false;
                        break;
                      }
                    }
