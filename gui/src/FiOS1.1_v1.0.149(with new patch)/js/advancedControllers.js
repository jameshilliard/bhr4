...
angular.module('advanced.controllers', [])
      ...
      $scope.save = function() {
        ...
        if($scope.dhcpSettings.mode == 2) {
          $scope.errorMessages = validateIPv4DhcpSettings(splitIpArr($scope.connection.ipAddress),
            ...
            $scope.dhcpWinsServer, $scope.dhcpSettings.leaseTime, null, null, null, null, true);
  ...
  .controller('fwUpgradeCtrl', ['$scope', '$route', 'FirmwareUpgrade', 'Reboot', 'SystemSettings', '$location', '$templateCache', '$timeout', 'securityTips',
    function($scope, $route, FirmwareUpgrade, Reboot, SystemSettings, $location, $templateCache, $timeout, securityTips) {
      $scope.tab = $route.current.tab;
      $scope.section = $route.current.section;
      $scope.page = 0;
  ...
  .controller('fwRestoreCtrl', ['$scope', '$route', 'FirmwareUpgrade', 'Reboot', 'SystemSettings', '$location', '$templateCache', 'securityTips',
    function($scope, $route, FirmwareUpgrade, Reboot, SystemSettings, $location, $templateCache, securityTips) {
      $scope.tab = $route.current.tab;
      $scope.section = $route.current.section;
      $scope.valid = false;
      $scope.restarting = false;
  ...
  .controller('dateTimeCtrl', ['$scope', '$route', '$routeParams', '$location', 'DateTimeSettings',
    function($scope, $route, $routeParams, $location, DateTimeSettings) {
      ...
      $scope.applySetClock = function() {
        ...
        if(hrs === '' || isNaN(hrs) || hrs < 0 || hrs > 23 || !checksForWholeNumber(hrs))
          $scope.errorMessages.push("Hours: Please enter a numeric value between 0 and 23.");
        if(mns === '' || isNaN(mns) || mns < 0 || mns > 59 || !checksForWholeNumber(mns))
          $scope.errorMessages.push("Minutes: Please enter a numeric value between 0 and 59.");
        if(scs === '' || isNaN(scs) || scs < 0 || scs > 59 || !checksForWholeNumber(scs))
  ...
  .controller('configFileCtrl', ['$scope', '$route', '$location', 'ConfigInfo', 'Reboot', 'Login', 'SystemSettings', '$templateCache', '$timeout', 'securityTips',
    function($scope, $route, $location, ConfigInfo, Reboot, Login, SystemSettings, $templateCache, $timeout, securityTips) {
      $scope.tab = $route.current.tab;
      $scope.section = $route.current.section;
  ...
  .controller('defaultsCtrl', ['$scope', '$route', 'Defaults', 'Login', 'Reboot','SystemSettings', '$location', '$templateCache', 'securityTips',
    function($scope, $route, Defaults, Login, Reboot , SystemSettings, $location, $templateCache, securityTips) {
      $scope.tab = $route.current.tab;
      $scope.section = $route.current.section;
  ...
  .controller('rebootCtrl', ['$scope', '$route', 'Reboot', 'SystemSettings', 'Login', '$location', '$templateCache', 'securityTips',
    function($scope, $route, Reboot, SystemSettings, Login, $location, $templateCache, securityTips) {
      $scope.tab = $route.current.tab;
      $scope.section = $route.current.section;
  ...
  .controller('dnsServerCtrl', ['$scope', '$route', 'DnsServer','securityTips', '$location',
    function($scope, $route, DnsServer,securityTips, $location) {
      ...
      $scope.validateBeforeSave = function() {
        $scope.errorMessages = [];
        if(!validateHostnameDNSServer($scope.tempDnsServer.hostname))
          $scope.errorMessages.push("Host Name: Name may not contain spaces. Only letters, digits and the special characters dash (-), underscore (_) and dot (.) may be used. These special characters may not appear at the beginning or at the end of a name. The maximum length of a label (text between two dots) is 63.");
        ...
        function($scope, $route,$location,DynamicDNS,DynamicDNSProvider, $window, securityTips, SystemSettings) {
            ...
            $scope.saveDynamicDnsObj = function(){
              ...
              if(!$scope.ddnsObject.useSsl){
                securityTips.showWarningMsgBeforeSaving1('DDNS Unsecured Mode',
