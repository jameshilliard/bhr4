      ...
      if($scope.connection.ipAddress) {
        ...
        if($scope.connection.type == 0) {
          if(tempSubMask == '0.0.0.0')
            $scope.errorMessages.push(((tempProtUsed == 1 && $scope.connection.overrideSubnetMask) ? "Override " : "" ) + "Subnet mask may not be 0.0.0.0. Subnet mask must consist of 4 fields. Each field's value must be between 0 and 255, constructing a contiguous string of binary ones. The default mask value is 255.255.255.0.")
          ...
          if((tempProtUsed == 2 || (tempProtUsed == 1 && $scope.connection.overrideSubnetMask)) && validateIp(sub)) {
            ...
            if(!valid){
              var tempMsg = "Subnet Mask: Subnet mask must consist of 4 fields. Each field's value must be between 0 and 255, constructing a contiguous string of binary ones. The default mask value is 255.255.255.0.";
            ...
            if(tempSubMask == '0.0.0.0')
              $scope.errorMessages.push("Subnet mask may not be 0.0.0.0. Subnet mask must consist of 4 fields. Each field's value must be between 0 and 255, constructing a contiguous string of binary ones. The default mask value is 255.255.255.0.")
          }
        }
    ...
    $scope.save = function () {
      ...
      for (var idevice=0 ; idevice<$scope.devices.length ; idevice++) {
        if($scope.device.id != idevice){
          if ($scope.device.name == $scope.devices[idevice].name) {
